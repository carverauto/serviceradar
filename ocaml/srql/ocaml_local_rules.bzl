...
# incompatible toolchain binaries.
LOCAL_ONLY_TAGS = ["no-remote-exec"]

def _with_local_tags(kwargs):
...
def ocaml_module(**kwargs):
    _ocaml_module(**_with_local_tags(kwargs))

def ocaml_library(**kwargs):
    _ocaml_library(**_with_local_tags(kwargs))

def ocaml_binary(**kwargs):
    _ocaml_binary(**_with_local_tags(kwargs))

def ocaml_test(**kwargs):
    _ocaml_test(**_with_local_tags(kwargs))
