...
let () =
  ...
  Lwt_main.run
    (Srql_translator.Proton_client.Client.with_connection config (fun client ->
         ...
         let* _result2 =
           Srql_translator.Proton_client.Client.query client "SELECT version() AS version"
         in
         print_endline "✓ SELECT version() successful";

         let* _result3 =
           Srql_translator.Proton_client.Client.query client "SELECT current_user() AS user"
         in
           ...
           Lwt_list.iter_s
             (fun srql ->
               ...
               let* _result =
                 try
                   let* _res =
                     Srql_translator.Proton_client.SRQL.translate_and_execute client srql
                   in
