...
let () =
  ...
  let test_connection port use_tls desc =
    ...
    Srql_translator.Proton_client.Client.with_connection config (fun client ->
        ...
        (* Test a simple query *)
        let* result =
          Srql_translator.Proton_client.Client.query client "SELECT 1 AS test, version()"
        in
        (match result with
        ...
        let srql_queries =
          [ "SELECT 1"; "SELECT now() AS current_time"; "SELECT version() AS db_version" ]
        in
  ...
  Lwt_main.run
    ((* Try different connection options *)
     ...
     Lwt_list.iter_s
       (fun (port, use_tls, desc) ->
         try test_connection port use_tls desc
         with e ->
