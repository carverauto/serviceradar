...
let tokenize (s:string) : string list =
  ...
  let rec loop i depth in_quotes =
    ...
    | '"', false, _ -> Buffer.add_char buf c; loop (i+1) depth true
    | '"', true, _ -> Buffer.add_char buf c; loop (i+1) depth false
    | '(', _, _ -> Buffer.add_char buf c; loop (i+1) (depth+1) in_quotes
    | ')', _, _ -> Buffer.add_char buf c; loop (i+1) (max 0 (depth-1)) in_quotes
    | (' ' | '\t' | '\n'), false, 0 -> push (); loop (i+1) depth in_quotes
    | _ -> Buffer.add_char buf c; loop (i+1) depth in_quotes
  in
  loop 0 0 false; !parts
...
let strip_neg (k:string) : (bool * string) =
  if String.length k > 0 && k.[0] = '!' then (true, String.sub k 1 (String.length k - 1) |> trim) else (false, k)
...
let split_csv_top (s:string) : string list =
  ...
  for i = 0 to len - 1 do
    ...
    | '"', false -> in_quotes := true; Buffer.add_char buf c
    | '"', true -> in_quotes := false; Buffer.add_char buf c
    | ',', false -> push ()
...
let rec parse_group ~prefix ?(neg=false) (body:string) : Query_ast.search_filter list =
  ...
  if not contains_colon then (
    ...
    match values with
    | [] -> []
    | [_] -> if neg then [AttributeFilter (prefix, Neq, List.hd values)] else [AttributeFilter (prefix, Eq, List.hd values)]
    | vs -> if neg then [AttributeListFilterNot (prefix, vs)] else [AttributeListFilter (prefix, vs)]
  ) else (
    let toks = tokenize body in
    toks |> List.fold_left (fun acc tok ->
      ...
      | Some (k,v) ->
          ...
          if String.length v >= 2 && v.[0] = '(' && v.[String.length v - 1] = ')' then
            ...
            if List.length vs = 1 then acc @ [AttributeFilter (full_k, (if neg_eff then Neq else Eq), List.hd vs)]
            else acc @ [ (if neg_eff then AttributeListFilterNot (full_k, vs) else AttributeListFilter (full_k, vs)) ]
          else
            acc @ [AttributeFilter (full_k, (if neg_eff then Neq else Eq), parse_value v)]
    ) []
  )
...
let parse (input:string) : query_spec =
  ...
  List.iter (fun tok ->
    ...
    | Some (k,v) -> (
        ...
        | "limit" -> (match int_of_string_opt (strip_quotes v) with Some n -> limit_ref := Some n | None -> ())
        | "sort" ->
            let items = v |> strip_quotes |> String.split_on_char ',' |> List.filter ((<>) "") in
        ...
        | key ->
          ...
          if String.length vstr >= 2 && vstr.[0] = '(' && vstr.[String.length vstr - 1] = ')' then
            ...
            filters := !filters @ [ (if neg_key then AttributeListFilterNot (key', vs) else AttributeListFilter (key', vs)) ]
          else
            filters := !filters @ [AttributeFilter (key', (if neg_key then Neq else Eq), parse_value v)]
      )
  ) toks;
  {
    targets = !targets;
