...
let entity_mappings = [
  ...
  ("otel_trace_summaries", {table_name = "otel_trace_summaries_final"; timestamp_field = "timestamp"});
...
let entity_map = List.fold_left (fun acc (entity, config) -> 
  (String.lowercase_ascii entity, config) :: acc
) [] entity_mappings
...
let get_table_name entity =
  ...
  try
    ...
    config.table_name
  with
  | Not_found -> entity (* fallback to entity name if not mapped *)
...
let get_timestamp_field entity =
  ...
  with
  | Not_found -> "timestamp" (* default timestamp field *)
