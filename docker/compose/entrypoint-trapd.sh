...
CONFIG_PATH="${CONFIG_PATH:-/etc/serviceradar/trapd.json}"
if [ ! -f "$CONFIG_PATH" ]; then
    echo "Error: Configuration file not found at $CONFIG_PATH"
    exit 1
fi

echo "Starting ServiceRadar Trapd with config: $CONFIG_PATH"

# Start the Trapd service
exec /usr/local/bin/serviceradar-trapd --config "$CONFIG_PATH"
