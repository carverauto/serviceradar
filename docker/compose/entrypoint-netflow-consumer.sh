...
CNPG_CERT_FILE_VALUE="${CNPG_CERT_FILE:-$CNPG_CERT_DIR_VALUE/netflow-consumer.pem}"
CNPG_KEY_FILE_VALUE="${CNPG_KEY_FILE:-$CNPG_CERT_DIR_VALUE/netflow-consumer-key.pem}"

# Wait for dependencies to be ready
if [ -n "${WAIT_FOR_NATS:-}" ]; then
    NATS_HOST_VALUE=$(resolve_service_host "serviceradar-nats" NATS_HOST "nats.serviceradar")
...
CONFIG_PATH="/etc/serviceradar/consumers/netflow.json"
TEMPLATE_PATH="/etc/serviceradar/templates/netflow.json"
...
echo "Starting ServiceRadar NetFlow Consumer with config: $CONFIG_PATH"

# Start the NetFlow Consumer service
exec /usr/local/bin/serviceradar-netflow-consumer --config "$CONFIG_PATH"
