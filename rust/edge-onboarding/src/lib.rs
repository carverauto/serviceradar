...
pub use bundle::{install_mtls_bundle, MtlsBundle};
pub use config::{generate_checker_config, CheckerConfig, SecurityConfig, SecurityMode};
pub use deployment::{detect_deployment, DeploymentType};
pub use download::{download_package, PackageResponse};
pub use error::{Error, Result};
pub use token::{parse_token, TokenPayload};
...
fn get_storage_path(deployment_type: &DeploymentType, component_type: ComponentType) -> PathBuf {
    match deployment_type {
        DeploymentType::Docker => PathBuf::from("/var/lib/serviceradar").join(component_type.as_str()),
        DeploymentType::Kubernetes => PathBuf::from("/var/lib/serviceradar").join(component_type.as_str()),
        DeploymentType::BareMetal => PathBuf::from("/var/lib/serviceradar").join(component_type.as_str()),
    }
}
