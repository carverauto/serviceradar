...
use crate::{
    ...
    schema::mtr_hops::dsl::{
        ...
        hop_number as col_hop_number, hostname as col_hostname, id as col_id, loss_pct as col_loss_pct,
        mtr_hops, time as col_time, trace_id as col_trace_id,
    },
    time::TimeRange,
};
...
fn build_stats_sql(plan: &QueryPlan, raw: &str) -> Result<HopStatsSql> {
    ...
    let group_col = validate_group_field(group_part.trim())?;
    ...
    let json_kv: Vec<String> = aggs
        .iter()
        .flat_map(|(func, col, alias)| [format!("'{alias}'"), format!("{func}({col})")])
        .chain([format!("'{group_col}'"), group_col.to_string()])
        .collect();
    let payload_expr = format!("jsonb_build_object({})", json_kv.join(", "));

    let mut sql = format!("SELECT {payload_expr} AS payload\nFROM mtr_hops");
    if !clauses.is_empty() {
        sql.push_str("\nWHERE ");
    ...
    // Order: use the first agg alias if no explicit order, else honour plan.order
    let order_sql = build_stats_order_clause(plan, &aggs);
...
fn parse_agg_expressions(part: &str) -> Result<Vec<(String, String, String)>> {
    // Supports comma-separated expressions like:
    //   avg(loss_pct) as avg_loss, avg(avg_us) as avg_latency
    let mut result = Vec::new();
    for expr in part.split(',') {
        let expr = expr.trim();
        let (func, field, alias) = parse_single_agg(expr)?;
        result.push((func, field, alias));
    }
    if result.is_empty() {
...
fn parse_single_agg(expr: &str) -> Result<(String, String, String)> {
    // Accepts structured parser aggregations or the raw "func(field) as alias" form
    let lower = expr.to_ascii_lowercase();

    // Try to match "func(field) as alias"
    let (func_and_field, alias) = if let Some(pos) = lower.find(" as ") {
        (&expr[..pos], expr[pos + 4..].trim())
    } else {
        (expr, "")
    };

    let (func_name, field_name) = parse_func_call(func_and_field.trim())?;
    let alias = if alias.is_empty() {
        format!("{}_{}", func_name.to_ascii_lowercase(), field_name)
    } else {
        sanitize_identifier(alias)?
    };

    Ok((
        agg_func_sql(&func_name)?,
        validate_agg_column(&field_name)?,
        alias,
    ))
}
...
fn parse_func_call(s: &str) -> Result<(String, String)> {
    ...
    let close = lower.rfind(')').ok_or_else(|| {
        ServiceError::InvalidRequest(format!("unmatched parenthesis in '{s}'"))
    })?;
...
fn build_stats_order_clause(
    ...
    let default_expr = aggs
        .first()
        .map(|(func, col, _)| format!("{func}({col})"));
    ...
    for clause in &plan.order {
        let expr = if let Some((func, col, _)) = aggs
            .iter()
            .find(|(_, _, alias)| alias.eq_ignore_ascii_case(&clause.field))
        {
            format!("{func}({col})")
...
fn build_stats_filter_clause(
    filter: &Filter,
) -> Result<Option<(String, Vec<HopStatsBindValue>)>> {
    match filter.field.as_str() {
...
fn build_text_clause(
    col: &str,
    filter: &Filter,
) -> Result<(String, Vec<HopStatsBindValue>)> {
    let mut binds = Vec::new();
    let clause = match filter.op {
        FilterOp::Eq => {
            binds.push(HopStatsBindValue::Text(filter.value.as_scalar()?.to_string()));
            format!("{col} = ?")
        }
        FilterOp::NotEq => {
            binds.push(HopStatsBindValue::Text(filter.value.as_scalar()?.to_string()));
...
mod tests {
    ...
    fn stats_by_asn_is_supported() {
        let plan = plan_for("in:mtr_hops time:last_6h stats:avg(avg_us) as avg_latency by asn limit:20");
        let (sql, _) = to_sql_and_params(&plan).expect("stats SQL should translate");
        let lower = sql.to_lowercase();
