...
use crate::{
    ...
    schema::logs::dsl::{
        body as col_body, event_name as col_event_name, id as col_id, logs,
        ...
        severity_text as col_severity_text, source as col_source, span_id as col_span_id,
...
mod tests {
    ...
    fn data_plan(filters: Vec<Filter>) -> QueryPlan {
        let start = Utc.with_ymd_and_hms(2025, 1, 1, 0, 0, 0).unwrap();
        let end = start + ChronoDuration::hours(24);
        QueryPlan {
            entity: Entity::Logs,
    ...
    fn scalar_filter(field: &str, op: FilterOp, value: &str) -> Filter {
        Filter {
            field: field.into(),
            op,
    ...
    fn event_name_in_filter_generates_any_clause() {
        ...
        assert!(
            sql.contains("\"logs\".\"event_name\" = ANY($3)"),
            "{sql}"
        );
        assert!(
