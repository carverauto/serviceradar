...
type MemoryTable = crate::schema::memory_metrics::table;
type MemoryFromClause = FromClause<MemoryTable>;
type MemoryQuery<'a> =
    BoxedSelectStatement<'a, <MemoryTable as AsQuery>::SqlType, MemoryFromClause, Pg>;
#[derive(Debug, Clone)]
struct MemoryStatsSpec {
    alias: String,
    field: String,
}

#[derive(Debug, Clone)]
struct MemoryStatsSql {
    sql: String,
...
pub(super) async fn execute(conn: &mut AsyncPgConnection, plan: &QueryPlan) -> Result<Vec<Value>> {
    ...
    let rows: Vec<MemoryMetricRow> = query
        .limit(plan.limit)
        .offset(plan.offset)
        .load(conn)
        .await
...
fn ensure_entity(plan: &QueryPlan) -> Result<()> {
    match plan.entity {
        Entity::MemoryMetrics => Ok(()),
        _ => Err(ServiceError::InvalidRequest(
            "entity not supported by memory metrics query".into(),
        )),
    }
}

fn build_query(plan: &QueryPlan) -> Result<MemoryQuery<'static>> {
    let mut query = memory_metrics.into_boxed::<Pg>();
...
fn apply_primary_order<'a>(query: MemoryQuery<'a>, clause: &OrderClause) -> MemoryQuery<'a> {
...
fn apply_secondary_order<'a>(query: MemoryQuery<'a>, clause: &OrderClause) -> MemoryQuery<'a> {
    match clause.field.as_str() {
        "timestamp" => match clause.direction {
...
fn build_stats_query(plan: &QueryPlan, spec: &MemoryStatsSpec) -> Result<MemoryStatsSql> {
    ...
    build_stats_query_with_source(plan, spec, "memory_metrics", "timestamp", &agg_expr, false)
}

fn build_cagg_stats_query(plan: &QueryPlan, spec: &MemoryStatsSpec) -> Result<MemoryStatsSql> {
    let mapped_col = super::cagg_column_for_entity(&Entity::MemoryMetrics, "avg", &spec.field)
        .ok_or_else(|| {
            ServiceError::InvalidRequest(format!(
                "missing CAGG mapping for avg({}) on memory_metrics",
                spec.field
            ))
        })?;
    let agg_expr = format!(
...
fn parse_stats_spec(raw: Option<&str>) -> Result<Option<MemoryStatsSpec>> {
    ...
    if stats_raw.contains(',') {
        return Err(ServiceError::InvalidRequest(
            "memory metrics stats only support a single expression".into(),
        ));
    }
    ...
    if !group_segment.eq_ignore_ascii_case("device_id") {
        return Err(ServiceError::InvalidRequest(
            "memory metrics stats only support grouping by device_id".into(),
        ));
    }
    ...
    if !expr_lower.starts_with("avg(") || !expr_lower.ends_with(')') {
        return Err(ServiceError::InvalidRequest(
            "memory metrics stats only support avg(field) expressions".into(),
        ));
    }
    ...
    if !allowed.contains(&field.as_str()) {
        return Err(ServiceError::InvalidRequest(
            "memory metrics stats only support avg(usage_percent|used_bytes|available_bytes)"
                .into(),
        ));
    }

    Ok(Some(MemoryStatsSpec { alias, field }))
}
...
mod tests {
    ...
    #[test]
    fn stats_query_supports_memory_language_reference() {
        let start = Utc.with_ymd_and_hms(2025, 1, 1, 0, 0, 0).unwrap();
        let end = start + ChronoDuration::hours(1);
        let plan = QueryPlan {
            entity: Entity::MemoryMetrics,
        ...
        assert!(
            sql.sql.contains("FROM memory_metrics")
    ...
    #[test]
    fn stats_query_uses_memory_hourly_cagg_for_large_windows() {
        let start = Utc.with_ymd_and_hms(2025, 1, 1, 0, 0, 0).unwrap();
        ...
        assert!(
            sql.sql.contains("FROM memory_metrics_hourly")
