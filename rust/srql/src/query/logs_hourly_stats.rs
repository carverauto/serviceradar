//! Query module for logs_hourly_stats continuous aggregate.
//!
//! This CAGG provides pre-computed hourly severity counts for efficient dashboard queries.
//! Use `in:logs_hourly_stats` for fast aggregated statistics instead of scanning raw logs.

use super::{BindParam, QueryPlan};
use crate::{
    error::{Result, ServiceError},
    models::LogsHourlyStatsRow,
    parser::{Entity, Filter, FilterOp, OrderClause, OrderDirection},
    schema::logs_hourly_stats::dsl::{
        bucket as col_bucket, error_count as col_error_count,
        logs_hourly_stats, service_name as col_service_name, total_count as col_total_count,
    },
    time::TimeRange,
};
use diesel::pg::Pg;
...
type StatsTable = crate::schema::logs_hourly_stats::table;
...
pub(super) async fn execute(conn: &mut AsyncPgConnection, plan: &QueryPlan) -> Result<Vec<Value>> {
    ...
    let rows: Vec<LogsHourlyStatsRow> = query
        .limit(plan.limit)
        .offset(plan.offset)
        .load(conn)
        .await
    ...
    Ok(rows.into_iter().map(LogsHourlyStatsRow::into_json).collect())
}
...
fn ensure_entity(plan: &QueryPlan) -> Result<()> {
    match plan.entity {
        Entity::LogsHourlyStats => Ok(()),
        _ => Err(ServiceError::InvalidRequest(
            "entity not supported by logs_hourly_stats query".into(),
        )),
    }
}

fn build_query(plan: &QueryPlan) -> Result<StatsQuery<'static>> {
    let mut query = logs_hourly_stats.into_boxed::<Pg>();
    ...
    match filter.field.as_str() {
        ...
        other => {
            return Err(ServiceError::InvalidRequest(format!(
                "unsupported filter field for logs_hourly_stats: '{other}'"
            )));
        }
    }
...
fn collect_filter_params(params: &mut Vec<BindParam>, filter: &Filter) -> Result<()> {
    match filter.field.as_str() {
        "service_name" | "service" => collect_text_params(params, filter),
...
mod tests {
    ...
    fn builds_basic_query() {
        ...
        assert!(sql.to_lowercase().contains("logs_hourly_stats"));
