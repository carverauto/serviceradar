...
use crate::{
    ...
    schema::gateways::dsl::{
        ...
        gateway_id as col_gateway_id, gateways, is_healthy as col_is_healthy,
        last_seen as col_last_seen, registration_source as col_registration_source,
...
fn bind_param<'a>(
    ...
    match param {
        ...
        BindParam::Date(_) => Err(ServiceError::InvalidRequest(
            "unsupported bind type for gateways".into(),
        )),
    }
}
