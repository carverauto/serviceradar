...
struct Args {
    ...
    #[arg(short, long, default_value = "sflow-collector.json")]
    config: String,
}
...
async fn main() -> Result<()> {
    ...
    log::info!("Starting ServiceRadar sFlow Collector");
    ...
    match config.max_samples_per_datagram {
        Some(max) => log::info!("  Max samples per datagram: {}", max),
        None => log::info!("  Max samples per datagram: unlimited"),
    }
    ...
    let listener = Arc::new(Listener::new(listener_config, tx)?);
    ...
    log::info!("sFlow collector started successfully");
    ...
    log::info!("sFlow collector shutting down");
    Ok(())
}
