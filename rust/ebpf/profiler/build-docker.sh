...
case "$DOCKER_TARGET" in
  "dev")
    ...
    docker build -f rust/ebpf/profiler/Dockerfile -t serviceradar-profiler:dev .
    echo "Run with: docker run --privileged --pid=host --network=host -it serviceradar-profiler:dev"
    ;;
  "prod")
    echo "Building production image..."
    docker build -f rust/ebpf/profiler/Dockerfile -t serviceradar-profiler:latest .
  ...
  "standalone")
    ...
    docker build -f rust/ebpf/profiler/Dockerfile -t serviceradar-profiler:standalone .
