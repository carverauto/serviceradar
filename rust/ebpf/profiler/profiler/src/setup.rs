...
pub fn resolve_config_path(args: &CLI) -> PathBuf {
    if let Some(path) = &args.config {
        return PathBuf::from(path);
    }

    let mut candidates = vec![
        PathBuf::from("./profiler.toml"),
        PathBuf::from("/etc/serviceradar/profiler.toml"),
    ];

    if let Ok(home) = std::env::var("HOME") {
        candidates.push(PathBuf::from(home).join(".config/serviceradar/profiler.toml"));
    }
...
pub fn log_configuration_info(config: &Config) {
    ...
    info!(
        ...
        config.profiler.max_concurrent_sessions
    );
    info!(
        "  Max session duration: {}s",
        config.profiler.max_session_duration_seconds
    );
    info!(
        "  Max sampling frequency: {}Hz",
        config.profiler.max_frequency_hz
    );
