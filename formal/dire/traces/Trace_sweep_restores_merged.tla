---- MODULE Trace_sweep_restores_merged ----
...
KnockoutBugs == LifecycleBugs \ {"sweep_restores_merged"}
...
TheLog == <<
  ...
  [status |-> ("d1" :> "live" @@ "d2" :> "live"), reason |-> ("d1" :> "none" @@ "d2" :> "none"), owner |-> ("i1" :> "d2" @@ "i2" :> "d2"), ipOf |-> ("d1" :> "p1" @@ "d2" :> "p2"), audit |-> <<[from |-> "d1", to |-> "d2", kind |-> "merge", ids |-> {}, srcIds |-> {"i1"}, recent |-> TRUE]>>, work |-> {}, act |-> [name |-> "SweepRestore", u |-> "d1", v |-> NoDev, row |-> 0, stale |-> FALSE, bumped |-> {"d1"}]]
>>
====
