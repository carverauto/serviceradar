...
TheLog == <<
  ...
  [status |-> ("d1" :> "live" @@ "d2" :> "live"), reason |-> ("d1" :> "none" @@ "d2" :> "none"), owner |-> ("i1" :> "d1" @@ "i2" :> "d2"), ipOf |-> ("d1" :> "p1" @@ "d2" :> "p2"), audit |-> <<[from |-> "d2", to |-> "d1", kind |-> "merge", ids |-> {"i1", "i2"}, srcIds |-> {"i2"}, recent |-> TRUE], [from |-> "d1", to |-> "d2", kind |-> "unmerge", ids |-> {}, srcIds |-> {}, recent |-> TRUE]>>, work |-> {[target |-> "d1", stale |-> FALSE]}, act |-> [name |-> "Unmerge", u |-> "d2", v |-> "d1", row |-> 1, stale |-> FALSE, bumped |-> {"d1", "d2"}]]
>>
====
