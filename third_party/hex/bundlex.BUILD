...
# Hand-written: bundlex is pinned from git rather than Hex, so scripts/gen_hex_bazel.exs
# does not emit it. Deps mirror its own mix.exs, minus the :dev-only ones.
filegroup(
    name = "sources",
    srcs = glob(
        ["**/*"],
