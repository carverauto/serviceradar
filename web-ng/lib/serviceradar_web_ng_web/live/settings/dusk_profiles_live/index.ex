defmodule ServiceRadarWebNGWeb.Settings.DuskProfilesLive.Index do
  @moduledoc """
  LiveView for managing dusk blockchain node monitoring profiles.

  Provides UI for:
  - Dusk Profiles: Admin-managed configuration for Dusk blockchain node monitoring
  """
  ...
  def mount(_params, _session, socket) do
    ...
    socket =
      socket
      |> assign(:page_title, "Dusk Profiles")
  ...
  defp apply_action(socket, :new_profile, _params) do
    ...
    ash_form =
      Form.for_create(DuskProfile, :create, domain: ServiceRadar.DuskProfiles, scope: scope)

    socket
    |> assign(:page_title, "New Dusk Profile")
  ...
  defp apply_action(socket, :edit_profile, %{"id" => id}) do
    case load_profile(socket.assigns.current_scope, id) do
      nil ->
        ...
        |> push_navigate(to: ~p"/settings/dusk")

      profile ->
        scope = socket.assigns.current_scope

        ash_form =
          Form.for_update(profile, :update, domain: ServiceRadar.DuskProfiles, scope: scope)
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <.settings_shell current_path="/settings/dusk">
        <.settings_nav current_path="/settings/dusk" />
        <.agents_nav current_path="/settings/dusk" />
    ...
    <.ui_panel>
      <:header>
        <div class="flex items-center justify-between w-full">
          <div>
            <div class="text-sm font-semibold">Dusk Profiles</div>
            <p class="text-xs text-base-content/60">
              {length(@profiles)} profile(s) configured
            </p>
          </div>
          <.link navigate={~p"/settings/dusk/new"}>
          ...
          <tbody>
            <tr :if={@profiles == []}>
              <td colspan="6" class="text-center text-base-content/60 py-8">
                No dusk profiles configured. Create one to start monitoring Dusk blockchain nodes.
              </td>
            </tr>
              ...
              <tr class="hover:bg-base-200/40">
                ...
                <td>
                  <div class="flex items-center gap-2">
                    <.link
                      navigate={~p"/settings/dusk/#{profile.id}/edit"}
                ...
                <td>
                  <div class="flex items-center gap-1">
                    ...
                    <.link navigate={~p"/settings/dusk/#{profile.id}/edit"}>
  ...
  defp profile_form(assigns) do
    ...
    <.ui_panel>
      ...
      <form id="dusk-builder-form" phx-change="builder_change" phx-debounce="200"></form>

      <.form
        for={@form}
        id="dusk-profile-form"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <.input
                ...
                placeholder="e.g., Production Dusk Node"
                required
              />
            </div>
            <div>
              ...
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span class="label-text">Enable dusk monitoring</span>
              </label>
            </div>
          </div>
    ...
    <!-- Dusk Node Configuration -->
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="label"><span class="label-text">Node Address</span></label>
              <.input
                type="text"
                field={@form[:node_address]}
                class="input input-bordered w-full"
              ...
              <label class="label">
                <span class="label-text-alt text-base-content/50">
                  WebSocket address of the Dusk node to monitor
                </span>
              </label>
            </div>
            <div>
              <label class="label"><span class="label-text">Timeout</span></label>
              <.input
                type="text"
                field={@form[:timeout]}
                class="input input-bordered w-full"
                placeholder="e.g., 5m, 30s, 10m"
              />
              <label class="label">
                <span class="label-text-alt text-base-content/50">
                  Connection and operation timeout (e.g., 5m, 30s, 10m)
                </span>
              </label>
            </div>
          </div>
                ...
                <div class="flex items-center gap-2">
                  <div class="flex-1">
                    <.input
                      ...
                      placeholder="e.g., tags.role:dusk-node hostname:%dusk%"
                    />
                  </div>
                  <.ui_icon_button
                    active={@builder_open}
  ...
  defp load_profiles(scope) do
    case Ash.read(DuskProfile, scope: scope) do
      {:ok, profiles} ->
  ...
  defp load_profile(scope, id) do
    case Ash.get(DuskProfile, id, scope: scope) do
      {:ok, profile} -> profile
      {:error, _} -> nil
    end
  end
  ...
  # Builder Helper Functions (copied from sysmon for consistency)
  ...
  defp normalize_filter_op(op) do
    case op do
      "contains" -> "contains"
