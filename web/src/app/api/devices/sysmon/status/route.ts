...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (cachedEntry && Date.now() - cachedEntry.timestamp < CACHE_TTL) {
            console.log(`Cache hit for ${deviceIds.length} devices`);
        ...
        if (existingRequest) {
            console.log(`Request deduplication: waiting for existing request for ${deviceIds.length} devices`);
            try {
                const statuses = await existingRequest;
                return NextResponse.json({ statuses });
            } catch {
                // If the existing request failed, we'll proceed to make a new one
                console.log('Existing request failed, making new request');
        ...
        const executeQuery = async (): Promise<Record<string, { hasMetrics: boolean; status: number }>> => {
            // Use SRQL to efficiently query which devices have recent sysmon metrics
            console.log(`Fetching sysmon status for ${deviceIds.length} devices using SRQL`);
            
            try {
                // Create SRQL query to find devices with recent CPU metrics (last 2 hours for faster query)
            ...
            if (queryResponse.ok) {
                ...
                console.log(`SRQL query successful: found ${devicesWithMetrics.length} devices with recent sysmon metrics`);
            } else {
                console.error('SRQL query failed:', queryResponse.status, await queryResponse.text());
                // Fall back to individual API calls if SRQL query fails
                const statusPromises = deviceIds.map(async (deviceId: string) => {
                    try {
                        const url = `${apiUrl}/api/devices/${encodeURIComponent(deviceId)}/sysmon/cpu?hours=1`;
                ...
                console.log(`Fallback to individual calls: ${devicesWithMetrics.length}/${deviceIds.length} devices have metrics`);
            }
            ...
            console.error('Error in SRQL sysmon query:', error);
            ...
            console.log(`Final fallback completed: ${results.filter(r => r.hasMetrics).length}/${deviceIds.length} devices have metrics`);
            return statusMap;
        }
        };
        ...
        return NextResponse.json(
            { error: 'Internal server error while fetching sysmon status' },
            { status: 500 }
        );
    }
}