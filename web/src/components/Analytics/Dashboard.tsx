...
import { useRperfBandwidth } from '@/hooks/useRperfBandwidth';
...
const Dashboard = () => {
    const { token } = useAuth();
    const { data: rperfBandwidthData = [], isLoading: isRperfLoading, error: rperfError } = useRperfBandwidth();
                    ...
                    <ChartWidget title="RPerf Bandwidth by Target (Mbps)">
                        {isRperfLoading ? (
                            ...
                            <div className="flex items-center justify-center h-full text-red-500">
                                Error loading rperf data
                            </div>
                        ) : rperfBandwidthData.length > 0 ? (
                            <SimpleBarChart data={rperfBandwidthData} />
                        ) : (
                            <NoData />
                        )}
                    </ChartWidget>
