    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h1 className="text-2xl font-bold text-white">Pollers ({filteredPollers.length})</h1>
                    ...
                    >
                        <div className="p-4 flex justify-between items-center cursor-pointer hover:bg-gray-700/30" onClick={() => toggleExpand(poller.id)}>
                            <div className="flex items-center">
                                ...
                                <h3 className="ml-3 font-semibold text-white truncate">{poller.name}</h3>
                            </div>
                            <div className="flex items-center space-x-2">
                                ...
                                {expandedPoller === poller.id ? <ChevronUp className="h-5 w-5 text-gray-400" /> : <ChevronDown className="h-5 w-5 text-gray-400" />}
                            </div>
                        </div>
                        <div className="px-4 pb-4 pt-0 grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="flex flex-col">
                                <span className="text-xs text-gray-400">Response Time Trend</span>
                            ...
                            <div className="flex flex-col">
                                <span className="text-xs text-gray-400">Services Status</span>
                                <div className="flex items-center mt-1 space-x-2">
                                    <div className="flex items-center" title="Healthy"><div className="h-3 w-3 rounded-full bg-green-500 mr-1.5"></div><span className="text-sm text-white">{poller.servicesCount.healthy}</span></div>
                                    <div className="flex items-center" title="Warning"><div className="h-3 w-3 rounded-full bg-yellow-500 mr-1.5"></div><span className="text-sm text-white">{poller.servicesCount.warning}</span></div>
                                    <div className="flex items-center" title="Critical"><div className="h-3 w-3 rounded-full bg-red-500 mr-1.5"></div><span className="text-sm text-white">{poller.servicesCount.critical}</span></div>
                                </div>
                            </div>
                            <div className="flex flex-col">
                                ...
                                <div className="flex items-center mt-1">
                                    ...
                                    <span className="text-xs text-gray-300">{poller.lastUpdate}</span>
                                </div>
                            </div>
                        </div>
                        <div className="px-4 pb-4">
                            <div className="flex flex-wrap gap-2">
                                {poller.tags.map((tag: string) => (
                                    <span key={tag} className="px-2 py-1 text-xs rounded-full bg-gray-700 text-gray-300">{tag}</span>
                                ))}
                            </div>
                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                            {poller.serviceGroups.map((group) => (
                                                <div key={group.name} className="bg-[#1C1B22] p-3 rounded-lg">
                                                    <div className="flex justify-between items-center mb-2">
                                                        <h5 className="font-medium text-gray-100">{group.name}</h5>
                                                        <span className="text-sm text-gray-400">{group.healthy} / {group.count} healthy</span>
                                                    </div>
                                                    ...
                                                    <div className="flex flex-wrap gap-1 mt-2">
                                                        {group.services.map((service: Service) => (
                                                            <div
                                                                key={service.name}
                                                                onClick={(e) => { e.stopPropagation(); handleServiceClick(poller.id, service.name); }}
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                                        {poller.services.map((service: Service) => (
                                            <div
                                                key={service.name}
                                                onClick={(e) => { e.stopPropagation(); handleServiceClick(poller.id, service.name); }}
                                                className="bg-[#1C1B22] p-2 rounded cursor-pointer hover:bg-gray-600/50 transition-colors"
                                            >
                                                <div className="flex items-center justify-between mb-1">
                                                    <span className="font-medium text-sm text-gray-100 truncate">{service.name}</span>
            ...
            </div> {filteredPollers.length === 0 && (
            <div className="text-center py-12 bg-[#25252e] border border-gray-700 rounded-lg shadow-lg">
                <Server className="h-12 w-12 mx-auto text-gray-400" />
                <h3 className="mt-2 text-lg font-medium text-white">No pollers found</h3>
