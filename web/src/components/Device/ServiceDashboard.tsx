...
type ServiceDashboardData = RegistryDetails | unknown[] | null;
    ...
    const renderRegistryDetails = () => {
        ...
        return (
            ...
            <div className="bg-gray-800 rounded-lg p-6 grid gap-4 md:grid-cols-2">
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">Component</h3>
                    <p className="text-lg font-semibold text-white">{registryInfo.device_type || 'Unknown'}</p>
                </div>
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">Status</h3>
                    <p className="text-lg font-semibold text-white">{registryInfo.status || 'Unknown'}</p>
                </div>
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">Registration Source</h3>
                    <p className="text-white">{registryInfo.registration_source || 'Unavailable'}</p>
                </div>
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">SPIFFE Identity</h3>
                    <p className="text-white break-all">{registryInfo.spiffe_identity || '—'}</p>
                </div>
                {registryInfo.parent_id && (
                    <div>
                        <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">Parent ID</h3>
                        <p className="text-white break-all">{registryInfo.parent_id}</p>
                    </div>
                )}
                {registryInfo.component_id && (
                    <div>
                        <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">Component ID</h3>
                        <p className="text-white break-all">{registryInfo.component_id}</p>
                    </div>
                )}
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-6 grid gap-4 md:grid-cols-3">
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">First Registered</h3>
                    <p className="text-white">{formatTimestamp(registryInfo.first_registered)}</p>
                </div>
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">First Seen</h3>
                    <p className="text-white">{formatTimestamp(registryInfo.first_seen)}</p>
                </div>
                <div>
                    <h3 className="text-sm uppercase tracking-wide text-gray-400 mb-2">Last Seen</h3>
                    <p className="text-white">{formatTimestamp(registryInfo.last_seen)}</p>
                </div>
            </div>
                        ...
                        {Object.entries(registryInfo.metadata)
                            ...
                            .map(([key, value]) => (
                                <div key={key} className="border border-gray-700 rounded-lg p-4">
                                    <dt className="text-sm text-gray-400 uppercase tracking-wide">{key}</dt>
                                    <dd className="mt-1 text-white break-all">{value || '—'}</dd>
                                </div>
                            ))}
                    </dl>
                </div>
                    ...
                    {isRegistryService ? (
                        loading ? (
                            <div className="bg-gray-800 rounded-lg p-6 text-center">
                                ...
                                <p className="mt-2 text-gray-400">Loading registry details...</p>
                            </div>
                        ) : (
