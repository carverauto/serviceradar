...
Docker Compose now runs a CNPG major-version preflight before Postgres starts.
If your existing data volume is still PostgreSQL 16, the stack will stop with
an explicit error instead of attempting an unsupported in-place start on PG18.

To migrate an existing local PG16 Compose volume to PG18:
...
`cnpg-credentials` volume, provide them when running the migration:
