...
targets AS (
  ...
  SELECT
    ...
    (
      SELECT sum(COALESCE(f.bytes_total, 0) * GREATEST(COALESCE(f.sampling_rate, 1), 1))
      FROM platform.ocsf_network_activity f, params p
      WHERE f."time" >= p.observed_at - p.short_window
    )::numeric AS logical_volume_1h
)
SELECT
  ...
  t.fix_scope,
  t.rows_1h,
  round(t.rows_1h::numeric, 2) AS rows_per_hour_1h,
  t.rows_24h,
  round(t.rows_24h::numeric / 24.0, 2) AS rows_per_hour_24h,
