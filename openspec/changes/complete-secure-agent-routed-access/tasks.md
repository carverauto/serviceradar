...
- [ ] 4.5 Create and approve `add-automation-callback-grants`, registering only `remote_access.ssh_ca.bundle.read` as a bounded read-only public-data action.
- [ ] 4.6 Require the logged-in profile to hold both `ansible.runs.launch` and the callback action's mapped permission before dispatch, and recheck the issuance ceiling plus current permissions/policy/approval on activation and use.
