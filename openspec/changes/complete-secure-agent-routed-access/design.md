...
- The intended Windows guest `farm01/pve01/qemu/155` (`sr-win-test01`) is stopped and has no current IP. The only configured RDP target, `192.168.2.126:3389`, is unlinked to that guest, carries a stale gateway ID, has accumulated more than 26,000 sweep failures, and is unreachable from both the workstation and selected edge agent. There is no current RDP target proof.
