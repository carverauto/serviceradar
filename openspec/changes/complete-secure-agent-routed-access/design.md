...
- Live AWX currently has one Proxmox inventory/source representing the Farm hosts; no distinct `tonka01` AWX inventory/source is configured. Its `pve01` through `pve04` host rows omit `ansible_host`. From the AWX execution environment, those Farm inventory names resolve to Tonka addresses (`pve01` to `192.168.10.182`, `pve02` to `192.168.10.136`, and `pve03` to `192.168.10.235`) while `pve04` does not resolve. Any PVE-targeted job can therefore reach the wrong cluster and is prohibited until provider-scoped aliases and explicit management addresses are in place. Most AWX and Proxmox twins also remain separate canonical rows.
- The intended Windows guest `farm01/pve01/qemu/155` (`sr-win-test01`) returned to `running` on 2026-07-13. `192.168.2.126:3389` is reachable from both the workstation and selected `agent-sr-test-pve04`; Farm PVE01 resolves that IP to `BC:24:11:DE:2B:7D`, exactly VM 155's `net0` MAC. The QEMU guest agent is not running, so the Proxmox source does not publish the IP. RDP negotiation identifies `DESKTOP-9Q0P9N0`, advertises CredSSP/NLA, Early User Auth, and RDSTLS, and the existing isolated IronRDP live probes reach the TLS/CredSSP boundary without cleartext-password exposure. Demo still stores `.126` as a separate manual/sweep device and its RDP target retains that stale device UID and gateway ID with `user_present` custody and no credential rule. Reachability and the pre-auth protocol boundary are now proven, but canonical guest/IP linkage, production CA/server-name verification, NLA authentication, helper readiness, media, cleanup, and end-to-end RDP remain unproven.
- The selected agent has a signed/staged RDP add-on `0.1.0` under `/var/lib/serviceradar/agent/addons/rdp/current`, but applied config points `remote_access_rdp_adapter_path` at a different legacy `/usr/local/bin/serviceradar-rdp-adapter`; the two binaries have different SHA-256 digests, and the manifest's declared `/usr/local/lib/serviceradar/addons/rdp/current` path is absent. Both binaries report `connector_ready: true`, the agent advertises RDP, and there is no per-session runtime proof. This provenance/path mismatch must fail readiness rather than treating the capability probe alone as sufficient.
- 24 Proxmox console sessions, all permanently `requested`; no generic SSH sessions, host keys, or recordings.
- Proxmox guest and network sightings are not reliably reconciled to one canonical device, and active provider-reference collisions are unsafe for console routing.
- The two five-minute Proxmox assignments are actively overwriting the same canonical device UIDs. During the read-only audit, a Tonka run changed the observed distribution from 55 Farm-scoped, 8 Tonka-scoped, and 29 legacy records to 40 Farm-scoped, 22 Tonka-scoped, and 30 legacy records. Examples included a Farm `pve01` UID rewritten with a Tonka provider ref while retaining Farm IP `192.168.2.10`, Tonka LXC 102 retaining a Farm IP and another VMID's payload, and Tonka VM 106 carrying a correct ref with stale VMID 115. This is live cross-provider data corruption, not eventual-consistency staleness.
- The Proxmox integration reaches both provider instances, but normalized host/guest refs such as `proxmox:node:pve02` and `proxmox:guest:pve02:qemu:<vmid>` omit the immutable provider instance until too late in reconciliation. Same-named Farm/Tonka nodes and guests therefore collide in device and virtualization tables. Console routing, PVE automation, and fleet selection MUST fail closed until provider-instance identity is applied before every reconciliation key, alias, relationship, and persistence decision.
...
The public repository contains no deployment-specific CA. Integrated enrollment retrieves the bundle through the delegated automation callback contract described below. The initiating profile/service principal must grant both `ansible.runs.launch` and exact permission `devices.remote_access.ssh.ca_bundle.read`, plus target policy and any required approval. Neither permission implies the other. The control plane, not the playbook form, chooses the callback action, CA trust policy, and target-keyed principal mapping.
...
Integrated automation does not require an operator-created API key or CLI. Any future external CLI export/mint workflow requires its own exact permission, tenant/target/policy, response, and audit contract; it is not authorized by the callback child. Unattended automation uses only the explicit fixed-ceiling service-principal contract above.

The catalog UX shows `ansible.runs.launch`, `devices.remote_access.ssh.ca_bundle.read`, the named action, why it is needed, internal response sensitivity, target scope, policy/approval requirements, TTL, and budget before confirmation. Each action registry entry declares security tier, effect, sensitivity, target binding, bearer/PoP rule, principal types, approval, limits, schema, retry, and audit. This program permits only `remote_access.ssh_ca.bundle.read`; secret-returning, mutating, signing, credential-minting, or reusable actions require a separate OpenSpec security review and stronger proof-of-possession where appropriate.
