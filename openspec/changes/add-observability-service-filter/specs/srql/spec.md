...
SRQL SHALL expose the OTel service catalog as `in:otel_services`, returning `service_name`, `signals`, `first_seen`, `last_seen`, `logs_last_seen`, `traces_last_seen` and `metrics_last_seen`.
...
- **Time.** `time:` SHALL apply to the last-seen of the requested signals, or to `last_seen` when no signal is given.
...
Access to `in:otel_services` SHALL require the observability view permission for each signal named in `signal:`. A query without `signal:` SHALL require at least one of `observability.logs.view`, `observability.traces.view` or `observability.metrics.view`, and SHALL be restricted to the signals the caller may view.
