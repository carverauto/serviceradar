## Context
The current demo environment shows two independent observability failures:
...
Retention on the raw Timescale hypertables is still configured and running, so this proposal focuses on the specific regressions in the query path and in trace summary maintenance.
...
## Migration Plan
1. Add CNPG migrations for the effective log timestamp index and the `traces_stats_5m` CAGG plus refresh policy.
...
4. In demo, clear orphaned trace summary worker state, run the repaired maintenance flow, and verify log query plans and trace freshness.
