# Change: Restore observability query performance and trace summary maintenance

## Why
The demo `/observability` page regressed after the effective log timestamp change and the CNPG 18 upgrade window. In the running `web-ng` release, the exact logs SRQL query `in:logs time:last_7d sort:timestamp:desc` takes about 5.3 seconds because it orders by `COALESCE(observed_timestamp, timestamp)` without a matching index, forcing a full scan and sort across millions of recent log rows. At the same time, trace summary maintenance is stale: `otel_trace_summaries` contains only data from February 14, 2026 through February 21, 2026, the `RefreshTraceSummariesWorker` has orphaned `executing` jobs stuck since February 11, 2026, and the `traces_stats_5m` rollup queried by the traces UI does not exist in demo.
...
- Affected code: `rust/srql` logs planner, Elixir trace summary and alert retention workers, cron uniqueness, CNPG migrations for logs/traces indexes and CAGGs, demo remediation/runbooks
