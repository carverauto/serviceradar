...
- [ ] 1.2 Record exact Teleport v14 files consulted, if any, with tag, commit, headers, and dependency scan results.
- [ ] 1.3 Define the session scoping mechanism for SSH and provider-console adapters, preferring cgroup-scoped monitoring.

## 2. Agent Collector
- [ ] 2.1 Add Linux-only BPF build plumbing and keep non-BPF builds working by default.
- [ ] 2.2 Implement command exec probes and user-space normalization.
- [ ] 2.3 Implement file open/access probes and user-space normalization.
- [ ] 2.4 Implement network connect probes and user-space normalization.
- [ ] 2.5 Implement loss counters for kernel drops, parser failures, and user-space backpressure.
- [ ] 2.6 Bind collector lifecycle to remote-access session open/close/timeout paths.
