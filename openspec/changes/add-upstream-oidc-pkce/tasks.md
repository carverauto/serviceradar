...
- [ ] 1.1 Add `ServiceRadarWebNG.Pkce` with `generate_verifier/0` (32-byte
...
- [ ] 1.2 Point `ServiceRadarWebNG.Mcp.OAuth.Pkce.challenge_s256/1` at the
      shared helper (wrapper is fine). MCP authorize/token behavior stays
      unchanged.
- [ ] 1.3 Unit-test verifier charset/length and that `challenge_s256/1`
...
- [ ] 2.1 Read `SERVICERADAR_OIDC_PKCE_MODE` in `runtime.exs` (`auto` |
      `required` | `disabled`, default `auto`) onto
      `config :serviceradar_web_ng, :oidc_pkce_mode`.
- [ ] 2.2 Teach `OIDCClient.authorize_url/1` to decide PKCE from mode +
      ...
      - `disabled` → omit PKCE
- [ ] 2.3 Return the verifier (and whether PKCE was used) with `state` and
      `nonce`. Do not put the verifier on the authorize URL.
- [ ] 2.4 Teach `OIDCClient.exchange_code/2` to send `code_verifier` in the
      form body when provided, **and** keep `client_secret`. Omit
      `code_verifier` when PKCE was not used.
- [ ] 2.5 Do not log the verifier or the token-request form body.

## 3. Callback session binding

- [ ] 3.1 `OIDCController.request/2` stores `:oidc_state`, `:oidc_nonce`,
      `:oidc_code_verifier`, and `:oidc_pkce` in session.
- [ ] 3.2 Every callback clause deletes those four keys **before**
      `exchange_code/2` (success, IdP error, and invalid-state paths).
- [ ] 3.3 After consume: invalid `state` still fails as today. If
      `:oidc_pkce` was true and the verifier is missing/empty, fail with
      a generic auth error and **do not** call the token endpoint.
- [ ] 3.4 Pass the consumed verifier into `exchange_code/2` only on the
      PKCE path. Leave `nonce` verification unchanged.
- [ ] 3.5 Update the Endpoint session comment so `:oidc_code_verifier` is
      listed next to `:oidc_state` / `:oidc_nonce` (SameSite=Lax still
      required for the IdP top-level GET).

## 4. Tests

- [ ] 4.1 `oidc_client_test.exs`: authorize URL includes S256 challenge
...
- [ ] 4.2 `oidc_client_test.exs`: token exchange form includes
...
- [ ] 4.3 `oidc_controller_test.exs`: missing session still rejects;
...
- [ ] 4.4 Existing OIDC and MCP PKCE tests still pass.

## 5. Docs and Entra check

- [ ] 5.1 Document upstream PKCE in `docs/docs/auth-configuration.md`
      (S256, confidential client still uses the secret, mode env,
      `disabled` escape hatch). Explicitly distinguish MCP OAuth PKCE
      (`/oauth/authorize`) from this login-client PKCE.
