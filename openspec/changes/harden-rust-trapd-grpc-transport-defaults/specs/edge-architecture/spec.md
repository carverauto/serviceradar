...
#### Scenario: Trapd rejects plaintext gRPC status serving
- **GIVEN** `rust/trapd` is configured with `grpc_listen_addr`
- **WHEN** `grpc_security.mode` is `none`
- **THEN** configuration validation SHALL fail
- **AND** trapd SHALL NOT start its gRPC status server without authenticated transport
