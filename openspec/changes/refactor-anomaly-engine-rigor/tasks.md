...
- [~] 0.4 **Core half — NIF/kernels validated through the real path**: the `causal_disposition` NIF was rebuilt with the capacity-PI + seasonal-default Rust changes and exercised via `mix test` (capacity parity 1e-9 + seasonal source + series_key + disposition — all green through the actual cdylib, not just the Rust kernel). The full seasonal/capacity WORKER orchestration (DB-backed, Ash upsert, VerdictEmitter) against the `srql-fixtures` CNPG scratch DB remains (localhost PG is down; needs the skill's scratch-DB setup).
...
- [~] 1.1 Preparation landed; the producer value-rename itself is BLOCKED on a coordinated rule-update migration (verified analysis): consumers all dual-read (1.f3) and the alert RULE DEFS now dual-match `signal_type: ["causal","prediction"]` (`rule_seeder.ex` ×4, primary + recovery — match-any semantics verified via `match_value/2`'s `Enum.any?`). BUT `seed_rule_if_missing` is INSERT-IF-ABSENT (never updates an already-seeded rule), so renaming the producers (`verdict.rs:54` + seasonal/capacity `verdict_emitter.ex`) would silently ORPHAN the already-seeded `causal_*_health_finding` rules in existing deployments → lost health incidents. The rename needs (a) a migration/seeder-upsert to update EXISTING rules, (b) the BMP/topology producers in lockstep (1.f2), (c) end-to-end runtime verification that incidents still raise. A focused, runtime-verified session.
