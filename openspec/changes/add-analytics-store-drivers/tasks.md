...
- [x] 6.4 Flip the table to pg_duckdb; confirm EventWriter no longer inserts
      ...
      `[00:00, 08:09:49Z)` range-COPYed 2,407,018 rows (exact match).
      `analyticsStore.driver=pg_duckdb` `tables=[timeseries_metrics]`
      `dualWrite=[]`. Hypertable `n_tup_ins` held at 49,855,810 and
      `max(timestamp)` at 09:29:37Z while EventWriter kept publishing
      Parquet (`core-elx-*.parquet` through 09:41Z+). CAGG refresh
      removed for `timeseries_metrics_hourly`,
