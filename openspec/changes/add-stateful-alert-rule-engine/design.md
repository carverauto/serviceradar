...
- Decision: Evaluate stateful rules against OCSF events
  - Logs are promoted to OCSF events before rule evaluation so thresholds
    operate on the event stream rather than raw log records.
