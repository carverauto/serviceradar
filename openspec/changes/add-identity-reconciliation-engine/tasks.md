...
- Later resets with sync/poller paused, promotion off: device_updates contained all 50k Armis IDs, but unified_devices landed at ~49,98x due to registry batch dedupe collapsing strong-ID updates that shared an IP (DHCP reuse). Registry dedupe was updated to skip strong-identity updates (only merge IP-only sightings). Built/pushed `ghcr.io/carverauto/serviceradar-core:sha-c54786c4-ire-dedupe-fix` (digest `sha256:e75d0c88b19f5a8618e75e25eefa94776bf7d911a39aebc687dcacee5d571cd6`), flipped the demo Helm release to that tag with `core.identity.sightingsOnlyMode=false`, truncated identity/device tables, and reran sync/poller. Demo now reports 50,002 devices (50k faker + 2 internal).
