# Ash Cluster Infrastructure Spec Delta
...
## FUTURE Requirements (Gossip Protocol)

### Requirement: Gossip-Based Discovery (Future)
The system SHALL support gossip protocol for large-scale deployments.

#### Scenario: Multicast discovery
- **WHEN** using gossip strategy
- **THEN** nodes SHALL discover peers via UDP multicast
- **AND** no central registry SHALL be required
- **AND** nodes SHALL handle network partitions gracefully

#### Scenario: Gossip scalability
- **GIVEN** 50+ poller nodes
- **WHEN** using gossip strategy
- **THEN** cluster formation time SHALL be logarithmic
- **AND** network overhead SHALL remain bounded
