...
#### Scenario: Zen rejects missing or insecure gRPC transport
- **GIVEN** `rust/consumers/zen` is configured to expose its gRPC status server
- **WHEN** `grpc_security` is missing or `grpc_security.mode` is `none`
- **THEN** configuration or startup SHALL reject the server before it binds a listening socket
- **AND** zen SHALL NOT serve the status interface over plaintext
