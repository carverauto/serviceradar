...
- Adopt the viable issue #3029 follow-up analyzers that fit the managed workspace contract today: `mix_audit` (`mix deps.audit`), `ex_slop`, and `ex_dna` through Credo integration, `Styler` through the repo-owned formatter stack, and `Boundary` through a root-namespace compiler rollout across the first-party Mix projects
- Keep `rustywind` deferred because it belongs to Tailwind/frontend formatting rather than the shared Mix analyzer contract
