...
**Status: SUPERSEDED** (as of 2026-01-16)

This proposal has been superseded by `2286-break-out-tenant-control-plane` and
`remove-tenant-awareness-from-instance`. The architecture evolved to:
- Single-tenant-per-deployment model (each tenant gets isolated deployment)
- CNPG credentials set PostgreSQL `search_path` for schema isolation
- No cross-tenant queries possible at instance level
- Ash.Scope pattern implemented for all LiveViews and controllers
