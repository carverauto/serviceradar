...
- Add RPM/DEB packaging under `//build/packaging/web-ng` with systemd unit, config defaults, and versioning aligned to the repo `VERSION` file.
...
- Affected code: `web-ng/**` (build tooling only), `docker/images/**`, `build/packaging/**`, `MODULE.bazel` (new toolchains), release/push scripts.
