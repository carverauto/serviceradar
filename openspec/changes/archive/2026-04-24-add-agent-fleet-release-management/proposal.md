...
- Extend operator workflows so the releases page auto-discovers recent repository releases, supports one-click import of signed release metadata from repository-hosted assets, and still keeps the manual publish path available for developer and local test workflows.
- Extend the release publishing pipeline so repository-hosted releases attach the managed agent runtime archive plus the signed manifest/signature assets expected by repository import.
