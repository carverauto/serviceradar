...
- Secret generator jobs validate `cloak-key`, generate it only when missing, and fail fast on empty/invalid values (unless an explicit override is provided).
...
## Migration Plan
- On upgrade, generator jobs only change `cloak-key` when it is missing; valid keys are preserved and invalid values cause a hard failure so operators can fix them.
