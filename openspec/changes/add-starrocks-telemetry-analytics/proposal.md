...
- Introduce optional StarRocks analytics as the warehouse for flows, scalar metrics, logs and event history. Keep NetFlow collection independently gated in Helm/Compose, with no dependency in either direction. When StarRocks is off, all telemetry including NetFlow is still written to CNPG hypertables. Keep CNPG as the control-plane/current-state database. Serving follows per-dataset cutover, except for flows: flow reads are warehouse-only and are refused until the dataset is cut over, rather than falling back to CNPG.
