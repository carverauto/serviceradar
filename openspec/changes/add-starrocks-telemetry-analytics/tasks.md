...
- [ ] 5.3 Implement time-bucket MVs/aggregate routing, freshness detection, disjoint raw edges and raw fallback for unsupported filters/classification; prove EXPLAIN selection and exact parity.
  ...
  - Deferred: MV freshness detection and EXPLAIN-based selection evidence are NOT implemented -- nothing inspects StarRocks refresh state before routing a read to an async MV, so an unrefreshed view returns short counts. Deferred by decision to a direct implementation pass after this change lands; re-check this box then. The spec scenario "Rewrite is unavailable" (`specs/netflow-analytics/spec.md`) deliberately keeps the fresh-view requirement as the target rather than being weakened to match the code.
