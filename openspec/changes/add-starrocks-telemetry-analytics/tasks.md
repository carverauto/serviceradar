...
- [x] 5.3 Implement time-bucket MVs/aggregate routing, freshness detection, disjoint raw edges and raw fallback for unsupported filters/classification; prove EXPLAIN selection and exact parity.
  ...
  - Landed: MV freshness detection. The views are `REFRESH ASYNC` with no schedule, so readers compare `MAX(bucket)` against the wall clock (`RollupFreshness.fresh?/2`, stale after `rollup_stale_after_seconds`, default 7200) and fail closed to the StarRocks raw tables on any error, empty view, or unparseable timestamp -- never CNPG. The Rust compiler exposes `translate_raw` behind a `starrocks_raw` mode (selected by `ServiceRadarWebNG.SRQL` when the flows/metrics view is stale); the events reader branches in `event_window_source/2`. Covered by `stale_rollups_compile_from_the_raw_table`, `RollupFreshnessTest` (7 cases incl. fail-closed), the stale-rollup web-ng mode tests, and fail-before proofs for each.
