...
- [x] 5.3 Implement time-bucket MVs/aggregate routing, freshness detection, disjoint raw edges and raw fallback for unsupported filters/classification; prove EXPLAIN selection and exact parity.
  - Landed: whole-hour bucketed reads route to `ocsf_network_activity_hourly` / `timeseries_metrics_hourly` / `events_hourly` only when bucket, filters, series and aggregation re-aggregate exactly; everything else reads the raw table. Edges are NOT disjoint raw merges: per decision, an hour overlapping the window is returned whole from the warehouse and never completed from CNPG (`hourly_rollup` in `rust/srql/src/query/starrocks.rs`, `event_window_source/1` in `log_event_consumers.ex`).
  - Landed: MV freshness detection. The views are `REFRESH ASYNC` with no schedule, so readers measure how far a view trails the table it aggregates -- `MAX(raw.<time column>) - MAX(mv.bucket)` (`RollupFreshness.fresh?/2`, stale after `rollup_stale_after_seconds`, default 7200, wired through `SERVICERADAR_STARROCKS_ROLLUP_STALE_AFTER_SECONDS` and Helm `analytics.starrocks.rollupStaleAfterSeconds`). The signal is lag, not age: an idle dataset keeps its rollup, a source table with no rows reads as fresh, and any error, empty result or non-timestamp high-water mark fails closed to the StarRocks raw tables -- never CNPG. The Rust compiler exposes `translate_raw` behind a `starrocks_raw` mode; `ServiceRadarWebNG.SRQL` probes only after compilation, when the compiled SQL actually names an `_hourly` view, so a row listing or stats query costs no round trip. The events reader branches in `event_window_source/2` through the same `:query` seam every other statement uses. Covered by `stale_rollups_compile_from_the_raw_table`, `RollupFreshnessTest`, the fresh/stale/idle web-ng and events reader tests, and fail-before proofs for each.
  - Deferred: EXPLAIN-based selection evidence is NOT recorded. The spec scenario "Rewrite is unavailable" (`specs/netflow-analytics/spec.md`) deliberately keeps the fresh-view requirement as the target rather than being weakened to match the code.
