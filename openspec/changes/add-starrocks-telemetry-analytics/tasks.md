...
- [x] 3.3 Add opt-in Helm/operator integration and Compose profile, internal transport authentication, scoped roles/secrets, redirect policy and dedicated object storage configuration.
  - `analytics.starrocks.enabled` warehouses flows/metrics/logs/events. `flowCollector.enabled` is independent and fails closed without StarRocks. Compose: `--profile starrocks` warehouse only; `--profile flows` warehouse + collector. Off StarRocks means remaining telemetry on CNPG hypertables. `cutoverDatasets` stays empty.
