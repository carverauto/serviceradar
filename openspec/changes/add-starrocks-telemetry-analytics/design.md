...
Cache residency is not retention. In shared-data mode remote storage holds durable table data and local disk caches it; a 30-day cache objective cannot guarantee every recent row remains local. Retention is per dataset and resolution. `specs/telemetry-analytics/spec.md` owns the defaults (365 days for hosted logs, events and alert history; 90 days for raw flows and metrics, mirroring the CNPG raw policy); permit longer configured values, and specify scalar-metric/raw/rollup policy explicitly before enabling that dataset. Keep raw and aggregate coverage discoverable. Expiry cannot race backfill, rollback protection or approved holds. Never reuse the database backup bucket for analytics.
...
6. Give each reader a warehouse implementation after its parity acceptance, keeping its CNPG implementation; enabling StarRocks stops CNPG telemetry writes at once, so a reader without a warehouse implementation reports unavailable rather than reading a frozen CNPG table. There is no per-dataset rollback to CNPG; disabling StarRocks resumes CNPG writes with a gap covering the warehouse period, and history is repaired by backfill, never by blindly redirecting reads.
7. Stop the old writer only after all consumers pass. CNPG telemetry writers, tables, CAGGs and policies are kept as the backend for installations without StarRocks and are never retired; only legacy archive resources (pg_duckdb #488 heads, buckets, manifests, checkpoints) may be retired, in a separately reviewed operation after re-querying coverage and restore evidence. Never remove the old head, bucket, manifests or checkpoints merely because #488 closed.
