...
StarRocks is optional for the platform and is the warehouse for every historical telemetry dataset EventWriter already owns (flows, scalar metrics, logs, event history), not a NetFlow-only sidecar. Installs without NetFlow may leave it off and keep remaining telemetry on CNPG. Enabling NetFlow collection requires `analytics.starrocks.enabled`: Helm fails closed rather than serving IPFIX/NetFlow history from CNPG. Other datasets still cut over independently; enabling the warehouse does not by itself switch sysmon/SNMP/logs/events serving off CNPG.
