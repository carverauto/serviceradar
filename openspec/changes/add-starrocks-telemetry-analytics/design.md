...
StarRocks is optional for the platform and is a single warehouse switch. Enabling `analytics.starrocks.enabled` deploys NetFlow collection and shadows every EventWriter telemetry dataset (flows, scalar metrics, logs, event history) into StarRocks. Disabling it leaves NetFlow undeployed and keeps remaining telemetry on CNPG hypertables. Helm fails closed if NetFlow is requested without StarRocks. Serving still follows per-dataset `cutoverDatasets`; enabling the warehouse does not by itself switch readers off CNPG.
