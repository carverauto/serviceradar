...
The system SHALL expose configurable raw and aggregate retention by dataset, default every warehouse dataset (logs, events, alert history, raw flows, metrics and MTR) to 365 days, allow longer configured retention per dataset, and distinguish durable retention from local cache residency.
