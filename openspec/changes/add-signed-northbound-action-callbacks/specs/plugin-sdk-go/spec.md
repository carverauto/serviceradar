...
- **GIVEN** a Go Wasm plugin launches an external task and receives an external task ID
...
- **WHEN** the external system reports completion
- **THEN** the SDK SHALL allow returning a final succeeded or failed action result with redaction hints preserved
...
- **GIVEN** a Go Wasm plugin receives HMAC callback metadata on the selected target
