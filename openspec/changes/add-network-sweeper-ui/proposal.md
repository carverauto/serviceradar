...
Key gaps:
1. No UI for creating/editing sweep jobs targeting tag-based device subsets
...
- Device bulk edit: reuse existing bulk editor for tag application only
- Job status dashboard: last run time, operational status, error tracking
- Device selection DSL: tag-based targeting with optional static targets (IPs/CIDRs/ranges), partition, and agent constraints

### 2.1 Device Tagging (Schema + UI)
- Add `ocsf_devices.tags` as a flexible map for user-applied labels
- Bulk edit applies tags to selected devices
...
- **New specs**:
  ...
  - `device-inventory`: Device tags storage and bulk tag application
- **Modified specs**:
...
- **Affected code**:
  ...
  - `elixir/serviceradar_core/lib/serviceradar/inventory/` (ocsf_devices tags)
