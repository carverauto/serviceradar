...
- [ ] 5.1 Withdraw `add-delta-metrics-lakehouse` and `add-rust-tdengine-analytics` change dirs; remove `rust/metrics-delta-writer` skeleton; carry the write-throughput decision-gate forward as the documented trigger for a future ingest-side writer committing through the manifest contract
