...
- The isolated probe and Bazel connector-link adapter target now prove registered PEM and DER CA bundle material can build a normal Rustls client verifier, reject empty or malformed bundles, and disable TLS resumption for CredSSP compatibility. This is the production trust shape for `verify` with a registered bundle and `pinned_ca`; system root loading and live verified TLS remain follow-up work.
