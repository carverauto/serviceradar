...
- `rust/rdp-connector-probe` is a review-only isolated Cargo workspace with its own `Cargo.lock`. It proves the connector graph, a ServiceRadar helper-open payload parser, a minimal ServiceRadar-to-IronRDP config mapping, and the first IronRDP connector state-machine steps can compile without perturbing the root ServiceRadar Rust lockfile. The negotiation smoke tests decode the emitted X.224 request, verify that NLA/CredSSP is advertised without plain TLS fallback, prove a HYBRID_EX server confirm reaches the TLS upgrade boundary before CredSSP, reject a server-selected TLS-only downgrade, and confirm the pre-TLS X.224 request carries the mstshash username cookie without the cleartext password.
