...
- The isolated probe also links `ironrdp-blocking = 0.8.0` and drives its upstream `connect_begin` wrapper with a scripted server confirm. This proves the production helper should reuse IronRDP's connector loop through the TLS upgrade boundary rather than hand-rolling the connection-initiation sequence. TLS verification, server public-key extraction for CredSSP, and the active graphics stage remain production-helper work and must stay behind the isolated dependency boundary.
