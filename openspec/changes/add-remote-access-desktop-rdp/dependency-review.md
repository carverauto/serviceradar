...
- `rust/rdp-connector-probe` is a review-only isolated Cargo workspace with its own `Cargo.lock`. It proves the connector graph, a minimal ServiceRadar-to-IronRDP config mapping, and the first IronRDP connector state-machine step can compile without perturbing the root ServiceRadar Rust lockfile.
