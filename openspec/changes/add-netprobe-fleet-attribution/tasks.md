...
- [ ] 7.35 Close the remaining worker CPU gap after the full-fleet canary: 30s `/proc` CPU samples after the canary were `1.07%`, `1.41%`, `0.40%`, `1.81%`, `1.04%`, `2.84%`, and `0.03%`; `k8s-cp3-worker3` remains above target, and most k8s workers still consume the configured one procfs metadata cold read per second for cmdline/container enrichment.
