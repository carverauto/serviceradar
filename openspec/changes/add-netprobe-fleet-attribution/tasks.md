...
- [ ] 7.31 Replace the append-only raw attribution staging table with a current-state/upsert model plus durable matched investigation records so high-cardinality agent-up observations do not blow up CNPG while delayed NetFlow joins still work.
