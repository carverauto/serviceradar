...
## 9. Deferred workload context enrichment
...
- [ ] 9.2 Include non-Kubernetes container context in the same workload enrichment design: map container IDs to Docker container name, image, labels, Compose project/service/container number, swarm/task metadata where present, runtime socket/source, and host path hints, while keeping Docker socket access optional, least-privileged, and separately auditable from netprobe flow attribution.
