...
- **ADD** a generic `ServiceRadarWebNGWeb.Plugs.RateLimit` plug backed by an ETS limiter (`ServiceRadar.Security.RateLimiter`) so non-auth pipelines (CLI auth, OAuth/OIDC, SAML, dashboard publish, plugin upload, webhook ingest) can opt in by name with per-bucket config from `config.exs`. The existing `ServiceRadarWebNGWeb.Auth.RateLimiter` becomes a thin shim that delegates to the shared limiter. Buckets are deployment-wide; tenant separation is provided by the platform (per-tenant k8s namespace + CNPG schema + NATS account), not by app-layer scoping.
