...
The capacity kernel SHALL report the uncapped projected crossing time alongside the capped exhaustion ETA, and the forecasting worker SHALL record a series whose only reason for lacking an ETA is the history-relative extrapolation cap with skip reason `exhaustion_beyond_history_cap`, carrying the raw crossing time, the observed history span, and the cap in its diagnostics. Series with no crossing within the horizon SHALL keep the `no_projected_exhaustion` reason.
