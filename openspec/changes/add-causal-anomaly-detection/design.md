...
| 5 | **Inconsistent ingress** — OTLP enters two ways (collector vs core-relay), sysmon via gRPC-to-CNPG, core self-publishes some subjects. | **One ingress publisher per telemetry type; core-elx never publishes-then-reads-back.** OTLP (direct + relayed) → `rust/otel` collector; flows → `flow-collector`; SNMP traps → `trapd`; agent gRPC status (sysmon) → **agent-gateway publishes `metrics.*`** (agents are NATS-denied, so the gateway is the boundary). core-elx is a pure consumer. |
...
- **OTLP relay loop** — today agent → agent-gateway (gRPC) → **core re-publishes the OTLP chunks to NATS** (`status_handler.ex:281`) → consumed back. **Fix: funnel the relay into the existing `rust/otel` collector**, which is already the canonical OTLP→NATS publisher (tonic gRPC receiver → `nats/chunker.rs` → `nats/publish.rs` emits `otel.*`). The gateway forwards the relayed `Export*ServiceRequest` (gRPC) to the collector; the collector publishes. This deletes core's `status_handler` republish, gives **one OTLP code path** for direct + relayed, and needs **no bespoke NATS publisher** in the gateway or core. (`rust/otel` is an ingress *producer*, not a redundant DB-writing consumer — keeping it is consistent with deleting `db-event-writer`/`serviceradar-zen`.)
