...
- **Agent telemetry (OTLP) round-trips through core:** agent → agent-gateway (gRPC) → **core publishes the OTLP chunk to NATS** (`status_processor.ex:180,246`) → consumed back.
...
| 5 | **No single ingress.** | **agent-gateway is the single ingress** publishing all telemetry raw to JetStream; everything downstream consumes from there. |
...
- **OTLP relay loop** — today agent → agent-gateway (gRPC) → **core publishes OTLP to NATS** → consumed back. Fix: agent-gateway publishes the OTLP chunk **directly to JetStream at ingress** (single-ingress principle); core no longer publishes-then-consumes its own relay.
