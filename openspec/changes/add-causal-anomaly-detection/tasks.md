...
- [ ] 0.6 **Fold the ZEN rules engine into core-elx as a Rustler NIF** (normalization runs in-process); **delete the standalone `serviceradar-zen` consumer** (`rust/consumers/zen`). **Port the existing zen rule set into the NIF** — including `build/packaging/zen/rules/falco_normalize.json` (added by #3599) and the `logs.*` normalization rules. Coordinate `add-event-writer-processor-contributions` passthrough engines (don't duplicate).
