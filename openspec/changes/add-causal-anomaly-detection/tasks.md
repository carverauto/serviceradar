## 0. Ingestion uniformity — dedicated metrics stream (prerequisite)
- [ ] 0.1 Stand up a dedicated `metrics` JetStream stream (subjects `metrics.>`), **`limits` retention — never `workqueue`** (so DB-sync and detector consumers fan out independently), with retention/limits tuned for high-rate host metrics. Register in `event_writer/config.ex` stream registry.
- [ ] 0.2 Agent-side publisher for cpu/mem/disk/process sysmon metrics → `metrics.sysmon.*` (`go/pkg/agent/push_loop_status.go` `pushSysmonStatus`), behind a cutover flag; keep the gRPC `StreamStatus` write running in shadow.
- [ ] 0.3 Route SNMP interface telemetry (ifHCInOctets/ifHCOutOctets + `if_index`) → `metrics.snmp.*` (it is not durable on JetStream today).
- [ ] 0.4 New `event_writer` DB-sync processor + **durable** consumer (`deliver_policy: all`, `ack_policy: explicit`, `max_deliver: -1` — at-least-once) consuming the metrics stream into cpu/disk/memory/process + interface tables (mirror `processors/telemetry.ex`).
- [ ] 0.5 Switch core ingestion to the JetStream consumer; verify parity with the gRPC path; then retire the direct CNPG write in `results_router.ex:241` / `observability/sysmon_metrics_ingestor.ex:216`.
- [ ] 0.6 Coordinate with `update-sysmon-downsampling` so agent-side windowed aggregation lands on the publish path, not the gRPC path.
- [ ] 0.7 Update BUILD.bazel for any new Go/Elixir files.
...
- [ ] 1.5 Detector consumer wiring: a **separate** consumer (durable + `deliver_policy: new` + `inactive_threshold`, or ephemeral) — independent of the DB-sync consumer (ack only advances own cursor; `limits` retention) — over `metrics.sysmon.*`, `metrics.snmp.*`, `otel.metrics.>`, `flows.raw.netflow|sflow`, `flow.attributed.>`; per-subject enable flag.
...
- [ ] 1.7 Restart survival: persist a compact per-series state snapshot (last N samples + counters) to CNPG/JetStream-KV on a timer + graceful shutdown; restore on boot; suppress findings until re-warmed (min-samples guard).
- [ ] 1.8 Tests: synthetic flood that self-masks under naive baseline but stays anomalous under withhold rule; spike-vs-sustained discrimination via M-slot gate; cold-start seeding; restart restores state without replay storm; ack-independence (detector ack does not affect DB-sync consumer).
