...
- Security impact: keeps encrypted-at-rest and broker-grant boundaries, removes provider code from the browser path, and broadens the UI surface that can create credential records; requires strict descriptor validation, redaction, and host-only credential use.
