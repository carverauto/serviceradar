...
## What Changes
- Run Compose services with KV-backed configuration (set `CONFIG_SOURCE=kv` plus `KV_*` env) so bootstrap writes defaults into datasvc and publishes watcher snapshots.
- Ensure Compose variants seed poller/agent/core templates on first boot without clobbering existing KV data.
- Document/verify the expected KV keys and watcher telemetry for the Compose stack.
