...
(xnih/satori, GPLv2 data corpus) on the DHCP axis fed by a new DHCP
...
- [x] 32.3 Vendor the Satori DHCP/DHCPv6 XML corpus under `rust/netprobe/satori-corpus/`. Pull from the maintained `https://github.com/xnih/satori` upstream at commit `73fa88fe6549995c68760be10631382df4ec1d1c`. Record source URL, commit SHA, sha256 of `dhcp.xml` and `dhcpv6.xml`, and preserve the upstream GPLv2 LICENSE and README. Vendor only the fingerprint XML data; do not vendor or copy the Python runtime, pcap integration, or SSL/JA4 code.
...
- [ ] 32.5 Implement the Satori DHCP parser + matcher at `rust/netprobe/src/satori.rs`. Build-time: parse `dhcp.xml` and `dhcpv6.xml` into compile-time lookup tables keyed on DHCP option-list canonical form. Runtime: `fn match_satori(options: &DhcpOptionList) -> Option<SatoriLabel>`. Unit tests with at least 5 device-class fixtures (printer, IP phone, Windows laptop, macOS laptop, Linux server).
...
- [ ] 32.8 Add a Phase-2 DHCP DPI dissector at `rust/netprobe/src/dpi/dhcp.rs`. The dissector recognises UDP traffic on ports 67/68 and DHCPv6 traffic on ports 546/547, parses the DHCP option list out of DISCOVER / REQUEST / INFORM packets per RFC 2132 and DHCPv6 messages per RFC 8415, and emits the canonical option-list signature to a per-flow callback. Add the new dissector to the §16 dissector pack; toggleable via `VisibilityProfile.dpi.protocols = ["dhcp"]`. The dissector MUST NOT emit DHCP option *values* (which can include hostname, client identifier, requested options) — only the ordering and presence of options, which is what Satori matches against. Privacy posture mirrors the §16 default-redaction stance.
...
- [ ] 32.13 Extend the §31.13 CI license-lint to *positively assert* that all newly-added fingerprint corpora trace to one of: LGPL-2.1 for the separately replaceable upstream p0f corpus, GPLv2 for separately replaceable Satori XML data, public domain / CC0 for ServiceRadar-authored additions, MIT, BSD-2-Clause / BSD-2-Clause-Views, BSD-3-Clause, or Apache-2.0. Any other license fails the lint. Catches accidental introduction of incompatible corpora (PRADS code, Nmap NPSL, fingerbank API-restricted, etc.) and FoxIO-1.1-encumbered methods.
