...
- [ ] 20.15 [B-5 #3, forward-looking] Producer-side validation in the attribution bridge: `redacted_cmdline` is already joined and capped at 256 bytes in `rust/netprobe/src/attribution.rs`; the remaining partition_id suffix / depth-2 subject guard must land with the §21.3 publisher before anything publishes onto `flow.attributed.<partition>`.
...
- [x] 21.2 Annotate matched 5-tuples with PID / process / cmdline / uid / container-id; drop unmatched. Implemented by `rust/netprobe/src/attribution.rs` (`flow_to_pid` + `process_info` map readers with `/proc` enrichment), `rust/netprobe/src/external_flow.rs` (canonical 5-tuple matcher with unmatched/invalid drop paths), and `rust/netprobe/src/server.rs` (external-flow IPC ack + matched `FlowAttributionEvent` fan-out). Focused coverage: `external_flow`, `flow_attribution_event_caps_redacted_cmdline_to_contract`, and `ingest_external_flow_record_emits_matched_via_broadcast_and_bumps_counter`.
- [x] 21.3 [Option A] Replaced the Go bridge with a core-elx 5-tuple join. New modules in `elixir/serviceradar_core/lib/serviceradar/event_writer/`: `attributed_flow_joiner.ex` (ETS-backed TTL cache, direction-agnostic 5-tuple key, configurable TTL default 60 s, soft cap 250 000 entries, oldest-eviction); `host_slice_subscriber.ex` (core-NATS subscriber on `flow.host-slice.>` decoding `Flowpb.AttributedFlowMessage` and forwarding host-slice halves to the joiner). `lib/serviceradar/status_handler.ex` routes `source == "flow-attribution"` envelopes through `Netprobepb.FlowAttributionEventBatch` decoding and into the joiner with the cert-derived `partition_id`. **Security invariant:** the published `flow.attributed.<P>` subject always uses the server's own `self_partition_id` (from `SERVICERADAR_OTX_PARTITION` or the joiner config), never a value sourced from the agent payload — preserving the B-4 boundary. Joiner wired into `ServiceRadar.EventWriter.Supervisor`; host-slice subscriber gated by `:host_slice_subscriber_enabled` (default `false` until partition-core NATS creds are minted with the required `subscribe_allow flow.host-slice.>` / `publish_allow flow.attributed.>` grants). Telemetry: `[:serviceradar, :event_writer, :attributed_flow, host_slice_received | attribution_received | joined_and_published | orphan_timeout_drops | batch_received | batch_decode_failed | publish_failed | evicted_oversize]`. Coverage: `test/serviceradar/event_writer/attributed_flow_joiner_test.exs` (both arrival orders, TTL eviction, partition spoof rejection) and `test/serviceradar/status_handler_test.exs` (FlowAttributionEventBatch decode path).
- [x] 21.4 Add the "Attributed Flows" view to the Flows dashboard. Implemented as `/observability/flows/attributed` with fixture-backed attributed and unmatched rows until persisted `attributed_flow` records are available.
