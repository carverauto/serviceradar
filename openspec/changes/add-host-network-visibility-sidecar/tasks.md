...
- [~] 20.16 [B-5 #4] ExUnit coverage for `ProvisionAgentWorker.perform/1`. Oban.Testing harness is now stood up: `config/test.exs` swapped `Oban, false` for `testing: :manual` (with `queues: false, plugins: false, peer: false, notifier: Oban.Notifiers.PG`) so resource after-actions that enqueue (e.g. `OnboardingPackage`'s `:create` for `:agent` rows) no longer crash and `perform_job/2` / `assert_enqueued/1` are available. `test/serviceradar/edge/workers/provision_agent_worker_test.exs` covers every `:discard` branch end-to-end against a real `ServiceRadar.Repo`: `:package_not_found`, `:not_agent_package`, `:invalid_agent_id` (both unsafe-char and empty-string forms), `:nats_not_configured` (missing + empty `nats_account_name`), `:account_seed_not_found` (missing + empty `nats_account_seed`). Uses `SystemActor.system(:test)` (no `authorize?: false`). The remaining branches — happy path, `{:grpc_error, _}`, `:not_connected`, catch-all `{:error, _}` — are gated on `mint_credentials/3` (worker line 189) growing an `:account_client` injection seam mirroring `ControllerHealthWorker`'s `:awx_client`; tracked as follow-up since the worker source is out of scope for §20.16.
