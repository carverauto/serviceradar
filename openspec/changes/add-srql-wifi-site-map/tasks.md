...
- [x] 6.7.1 Create the initial external `~/src/wifi-dashboard` dashboard package workspace with a TinyGo `dashboard-wasm-v1` renderer, manifest build, sample frames, and harness instructions.
...
- [ ] 7.0 Create the customer-owned Example agent plugin package outside the ServiceRadar OSS repository using `serviceradar-sdk-go`; it must support CSV seed mode first and emit `serviceradar.wifi_map.batch.v1` through `serviceradar.plugin_result.v1`.
