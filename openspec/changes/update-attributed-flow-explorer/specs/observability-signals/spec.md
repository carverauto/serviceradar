...
The Observability logs pane SHALL default to standard paginated browsing with live
updates disabled. The pane SHALL expose a header control that lets the operator
explicitly enable live log streaming for the current view. Other high-volume
observability panes, including attributed flows, SHALL follow the same browse-first
and opt-in Live interaction model.
...
#### Scenario: Attributed flows follow live mode contract
- **WHEN** a user opens `/observability/flows/attributed`
- **THEN** the attributed flows pane SHALL load as a normal paginated view
- **AND** live updates SHALL remain disabled until the user enables Live mode
