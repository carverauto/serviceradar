# Change: Add a package-declared external inventory Wasm plugin contract
...
OpenText Network Automation is the first external implementation of this contract. Its current inventory export is manual, so an edge plugin will collect a bounded switch inventory once per day and on demand.
...
- Build a runtime integration catalog exclusively from approved package descriptors, reject duplicate claims, and reserve built-in providers from external replacement.
...
- Keep provider-specific protocol code, configuration schema, documentation, display metadata, fixtures, and end-to-end host test in the external plugin repository.
...
- Build the first Go/TinyGo plugin in `serviceradar-plugin-opentext-network-automation`. It executes a fixed bounded `list device` operation, uses brokered credentials, emits complete `serviceradar.device_discovery.v1` snapshots, and never writes ServiceRadar storage directly.
...
- Affected code: external inventory plugin repositories; package manifest/import validation; generic credential reconciliation; producer schedules and action-result ingestion; device source observations; DIRE; source-inventory API; web-ng credentials and provenance UI; protected Forgejo workflow
