...
- [ ] 3.4 Verify OTX and Dusk host-function failures distinguish external connectivity or allowlist denial from agent runtime bugs.
- [x] 3.5 Verify UniFi Protect scheduled and streaming assignments do not interfere with each other and report current state independently.

## 4. UI and Diagnostics
- [x] 4.1 Keep failures sorted first and newest-first within each status class.
- [x] 4.2 Preserve detailed plugin failure messages for operator diagnostics while keeping cards compact.
- [ ] 4.3 Ensure the service detail link resolves from a `service_state` row to the latest historical `service_status` details.

## 5. Tests and Validation
- [x] 5.1 Add Ash/resource tests for `ServiceStateRegistry` placeholder protection and history-to-state repair.
- [ ] 5.2 Add `PluginResultIngestor` tests proving current-state upsert happens for OK and failure plugin results.
- [ ] 5.3 Add LiveView tests proving `/services` reload renders persisted OK/FAIL states without waiting for PubSub.
- [x] 5.4 Add Go plugin runtime tests for first-party plugin config fixtures and Proxmox panic regression.
- [x] 5.5 Run focused Elixir and Go tests for touched modules.
