...
The system SHALL create and use the `platform_graph` AGE graph for topology projections in a dedicated schema, and the application role SHALL have USAGE/ALL privileges on the `platform_graph` schema.
...
- **THEN** the `serviceradar` role has USAGE and ALL on schema `platform_graph`
