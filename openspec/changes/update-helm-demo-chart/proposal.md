...
- Blockers: Demo core is healthy, but agent still crashlooping (KV resolver returns zero addresses), db-event-writer fails CNPG SPIFFE TLS (`x509svid: could not verify leaf certificate`), and flowgger still crashlooping due to OpenSSL 3.2 dependency mismatch. Pending validation of recent fixes: agent now uses `dnsPolicy: ClusterFirstWithHostNet` on hostNetwork to resolve service DNS, db-event-writer now mounts the CNPG CA secret and uses `/etc/serviceradar/cnpg/ca.crt` by default for TLS, and a Helm hook reseeds the KV entry from the corrected template on install/upgrade. App database now separated from SPIRE/postgres (defaults to `serviceradar` user/DB/secret); need to finish idempotent CNPG init so installs provision the DB/user automatically.
