...
- [ ] 2.4 Review the installed Plugin Packages table for bounded display/pagination and avoid presenting object-store history as if every retained blob is a recommended active version.
...
- [ ] 3.6 Normalize plugin result statuses from SDK/sample plugins so `failed` is accepted or mapped to the canonical failed/critical status instead of producing `plugin status invalid`.
...
- [ ] 8.1 Document the release/plugin inventory limits and object-store retention behavior.
- [ ] 8.2 Document the Wasm plugin service visibility contract for plugin authors/operators.
...
- [ ] 8.7 Run `openspec validate update-plugin-release-inventory-visibility --strict`.
