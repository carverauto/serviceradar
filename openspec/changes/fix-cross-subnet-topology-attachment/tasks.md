...
- [ ] 1.1 [A] Remove the same-/24 gate from the SNMP-L2 MAC→IP join: allow an FDB
...
- [ ] 1.2 [B] Add a Q-BRIDGE FDB walk (`dot1qTpFdbPort`) alongside `dot1dTpFdbPort`,
  tagging entries with their VLAN (`snmp_l2_bridge.go`).
- [ ] 1.3 [B] Add credential-gated per-VLAN community (`community@vlan`) FDB
  enumeration for switches that expose FDB only per VLAN.
- [ ] 1.4 [C] Feed L3 router ARP (`ipNetToMediaPhysAddress`) into the shared
...
- [ ] 1.5 [D] Scope the known-IP-without-identity veto (`snmp_l2_query.go:210`) so
...
- [ ] 1.6 Confirm the tonka01 job's recursive expansion walks both the ARP owner
  (`tonka01`) and the FDB owner (aruba) in one run so their evidence can join.

## 2. UniFi wired-client attachment (Go, `go/pkg/mapper`)

- [ ] 2.1 [E] Stop discarding wired clients at fetch (`ubnt_api.go:112-119`); carry
  wired clients through the pipeline.
- [ ] 2.2 [E] Emit endpoint-attachment evidence for wired clients using the
...
- [ ] 2.3 [E] When per-port detail is absent, emit a switch-level attachment at
...
- [ ] 3.1 [F2] Extend topology neighbor resolution to consult
...
- [ ] 3.2 [G] Add direct-physical LLDP/CDP endpoint sightings to the promotable
...
- [ ] 3.3 Ensure a resolved endpoint whose device record is IP-only (no MAC
  identity) binds to that existing device rather than minting a provisional.

## 4. Validation

- [ ] 4.1 Go unit/fixture tests: cross-subnet FDB join, dot1q/per-VLAN walk,
  router-ARP shared map, recursive-pass veto scoping, UniFi wired attachment.
- [ ] 4.2 Elixir tests: identifier-aware resolution binds a secondary-MAC sighting;
...
- [ ] 5.1 Sequence after / coordinate with `improve-mapper-topology-fidelity`
  (adjacent `go/pkg/mapper` + `mapper_results_ingestor.ex` diffs).
- [ ] 5.2 Confirm `fix-topology-evidence-pipeline-resilience` promotion binds the
