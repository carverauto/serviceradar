...
| `MaxPrincipalBytes` | string bytes, post-decode; **THREE sites** -- record producer context, edge publication slot, service publication slot | `ValidateAuthenticatedPrincipal` from `validateProducerContext` and both slots / `PublicationIdentity` slots ONLY -- **no record-level check** | LANDED: FOUR controls at EVERY site -- 0 refused, 1 ACCEPTED, 128 accepted, 129 refused -- literal-pinned from the manifest rather than built from the constant (the prior `principal_test.go` rows were SELF-ADJUSTING and moved with any drift). PER-SITE ROWS ARE NOT REDUNDANT WITH A HELPER ROW, measured across the FIVE LIVE CARRIER SITES -- three Go, two Elixir: removing a single carrier's CALL to the shared helper fails exactly that carrier's row and no other. The SIXTH runtime-side cell is the delegated `n/a` and carries NO mutation result, because this runtime has no record-level principal gate to remove -- an absent gate cannot be mutation-tested, and counting it among the five would report a proof that does not exist. The Go producer-context row re-points the matching PRODUCTION CAPABILITY CLAIM and reseals the semantic envelope -- without that it is refused for producer identity, another rule's outcome standing in for the ceiling's | COMPLETE in Go. The Elixir record-level site stays with **1.5-n** and is recorded `n/a` with that owner |
| `MaxTraceStrBytes` at `abort_reason` | string bytes, post-decode; LIFECYCLE, not MTR; **TWO PREDICATES, kind-conditional** -- ABORTED requires a bounded non-empty reason, every other kind requires NONE | `ValidateSweepExecutionEvent` / **none** (1.6-c owns the peer) | LANDED (GO ONLY): **SIX controls**, not three -- the ABORTED arm carries 0 refused / 1 accepted / 256 accepted / 257 refused, and the second arm its own accepted and refused pair. The second arm is independently removable and deleting it now fails the site; an ABORTED-only trio left it untested. The non-ABORTED arm needs an ACCEPTED control too, or a predicate refusing every non-aborted event would satisfy the negative row. Its fixture uses START, not COMPLETED, whose completion-proof rules would refuse it for another reason | delegated to **1.6-c**, recorded `n/a` with that owner. **1.6-c's obligation is all SIX controls** -- on the ABORTED arm length 0 refused, length 1 ACCEPTED, at accepted and over refused, plus BOTH non-ABORTED controls -- not the upper bound alone. The length-1 acceptance is not optional: a zero refusal alone leaves the frozen minimum free to move up. The FOUR MTR predicates stay 1.4-a |
...
| `MaxManifestPages` **(recovery, RAW)** | supplied page-list length, pre-decode | `ValidateManifestChainFromRaw` -- correctly staged / `RecoveryValidate.bound_received` -- **now bounded** | LANDED: bounded traversal, call-site improper-tail row, and the SHARED N/N+1 corpus (`recovery_raw`) with a STAGE WITNESS each side -- Go a malformed final page only a decode could object to, Elixir a traced proof the decoder is never entered plus a live-decoder control | UPPER + STAGE COMPLETE; LOWER OUTSTANDING. Kept as its own site because the plain N+1 verdict is shared with the decoded gate. The empty-list arm is unproven here -- see the deferred lower-arm partition under 1.5-h in `tasks.md`; this raw check falls through to the decoded refusal if removed, so it is SHADOWED rather than independently killable |
| `MaxManifestPages` **(recovery, DECODED)** | page-list length on decoded pages | `ValidateManifestChain` / `RecoveryValidate.manifest_chain` -- **both now count first** | LANDED: precedence rows in both runtimes, mutation-verified, plus the shared N/N+1 corpus (`recovery_decoded`) | UPPER COMPLETE; LOWER OUTSTANDING -- the empty-manifest refusal is load-bearing here and has no control; see the deferred partition, which assigns the recovery page-list MINIMUM to 1.5-h as new normative text |
| `MaxManifestPages` **(plan, RAW)** | supplied page-list length, pre-decode | `ValidatePlanFromRaw` -- correctly staged / `AssignmentValidate.bound_page_count/2` -- **now bounded** | LANDED: bounded traversal, call-site row, and the SHARED N/N+1 corpus (`plan_raw`) with a STAGE WITNESS each side -- Go an oversized final page (`ErrPlanPageTooLarge` wraps `ErrPlanBounds` and is separately testable), Elixir a traced proof plus a live-decoder control. Both accept at N through their PUBLIC boundary: Go's `ValidatePlanFromRaw` takes header bytes and raw pages, while this runtime's `validate_bytes_against_plan_bytes/3` also validates a RECORD, so its at-ceiling row carries a valid assignment bound to the generated plan rather than an empty one | UPPER + STAGE COMPLETE; LOWER OUTSTANDING -- SHADOWED: the raw empty check falls through to the decoded refusal if removed |
| `MaxManifestPages` **(plan, DECODED)** | page-list length in `ValidatePlanPages` | both runtimes now count AFTER header validation and BEFORE the walk | LANDED: precedence rows both runtimes; the declared page count stays a header RELATION below the walk; plus the shared N/N+1 corpus (`plan_decoded`) | UPPER COMPLETE; LOWER OUTSTANDING -- also backed by the header page-count RELATION, so the local `== 0` arm is not independently killable |
| `MaxRangesPerPage` | range count, post-decode per page | `plan.go` / `PlanValidate` -- **both now precede descent** | LANDED: precedence rows both runtimes, plus the shared N/N+1 corpus (`plan_ranges`). The page-size ceiling returns the SAME refusal tag, so it is excluded against the LIVE bound rather than a copied number: Go runs the REFUSED pages through the exported `MaxPlanPageBytes`, and this runtime -- whose ceiling is private -- adds an ACCEPTED N-range control built from canonical full-length IPv6 that encodes LARGER than the refused N+1 page, so any ceiling low enough to have refused N+1 for size refuses the control too. The discriminating mutation is a ceiling STRICTLY BETWEEN the two encoded sizes; both runtimes fail it | UPPER COMPLETE; LOWER OUTSTANDING -- the range-list empty arm is GENUINELY LOAD-BEARING and currently has no control |
| `MaxSpansPerPage` **(chain)** | span count, post-decode per page | `ValidateManifestChain` / `RecoveryValidate` -- **both now precede descent** | LANDED: precedence rows both runtimes, plus the shared N/N+1 corpus (`recovery_spans_chain`) | UPPER COMPLETE; LOWER OUTSTANDING -- span fallbacks shadow the early nonempty gate; the follow-up either centralizes them or records boundary behaviour without claiming each arm is proven. This runtime already carries empty-page controls |
| `MaxSpansPerPage` **(single page)** | span count on ONE page | `validateSingleManifestPage`, reached from the recovery-control path / **no Elixir peer** | LANDED (GO ONLY): a signed recovery-control wrapper around the shared span pair -- page sealed first, scope digest taken from it, payload/claim/envelope/both signatures built after, nothing mutated post-seal; both artifacts pass `ValidateRecordSigned` before any production claim. Removing ONLY this ceiling ADMITS the over-limit wrapper | UPPER COMPLETE; LOWER OUTSTANDING. Single-runtime row: the peer is **1.6-d**'s, recorded as `n/a` with that owner in the corpus so an absent peer cannot become a silent exemption. The lower arm's proof topology is deferred with the other six |
| `MaxManifestPages` **(tombstone)** | supplied page-list length, against a DECLARED count | `ValidateTombstone` / `RecoveryValidate.tombstone/2` -- **both now bound BEFORE comparing the declared count** | LANDED: bounds-before-relation parity rows in both runtimes, mutation-verified, plus the shared N/N+1 corpus (`tombstone`). The N+1 tombstone RESEALS its declaration to 1025 and asserts so -- declaring 1024 would be refused by the count RELATION and the row would pass with the ceiling deleted | COMPLETE for the page-LIST bound. An INDEPENDENTLY REMOVABLE eighth site. SEPARATE AND OUTSTANDING: the signed tombstone's `manifest_page_count` rule bounds a DECLARED SCALAR, not a supplied list, and has NO DEDICATED COUNT-SPECIFIC PAIR in either runtime -- Go's signed reason rows exercise a count of 1 INCIDENTALLY, since every wrapper they build declares one page, but nothing varies the count or asserts a verdict on it, so a deleted or re-aimed gate would go uncaught. The CURRENT Go gate is also LOWER-ONLY -- `ValidateTombstone` bounds a supplied list and reconciles the declaration against it, but `recoveryControlBody`, the only path a SIGNED tombstone reaches, receives no list and checks `== 0` alone, so a declaration of 2^32-1 is admitted. DECIDED: freeze 1..`MaxManifestPages` there, and the follow-up owns ALL FOUR Go controls -- 0 refused, 1 accepted, at accepted, over refused -- since it authors the ceiling in the same work; owing only the lower pair would leave its own evidence narrower than the rule it freezes. Defining it lower-only and deferring upper enforcement to chain assembly was REJECTED -- the scope digest is taken over the tombstone AS SIGNED, so an unbounded declaration is committed and carried before assembly could reject it. A RUNTIME CHANGE owned by the follow-up; the signed Elixir peer is **1.6-d**'s and MIRRORS ALL FOUR; what is delegated is the TARGET bound rather than today's `> 0` arm -- handing over only what is implemented would freeze the peer narrower than the rule |
