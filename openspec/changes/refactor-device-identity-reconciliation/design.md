...
- **D6 — Versioned, stable integration identifiers.** `integration_id` values carry a format version; Proxmox uses cluster-scoped stable keys (`proxmox:v2:<cluster>:<kind>:<vmid>` — survives renames and NIC changes); a migration table maps both legacy formats to v2 so connector resume reconciles instead of duplicating.
