...
- [x] 2.2 Add legacy-format mapping (name-keyed and MAC-keyed generations → v2) consulted during reconciliation (`IntegrationIdentity.legacy_candidates/2` + `lookup_values/1`; consulted in the hypervisor enrichment resolution path, and exposed to update-time reconciliation via `metadata["legacy_integration_ids"]` — SyncIngestor/Resolver wiring lands with 5.1).
