...
Create `build/packaging/core-elx/` with:
...
Mirror the existing `build/packaging/web-ng/` structure exactly.

**Validate:** `ls build/packaging/core-elx/{BUILD.bazel,config/core-elx.env,systemd/serviceradar-core-elx.service,scripts/postinstall.sh,scripts/preremove.sh}`

## 2. Add agent-gateway packaging directory

Create `build/packaging/agent-gateway/` with the same structure as core-elx but for agent-gateway:
...
**Validate:** `ls build/packaging/agent-gateway/{BUILD.bazel,config/agent-gateway.env,systemd/serviceradar-agent-gateway.service,scripts/postinstall.sh,scripts/preremove.sh}`

## 3. Add both services to packages.bzl

Add `"core-elx"` and `"agent-gateway"` entries to the `PACKAGES` dict in `build/packaging/packages.bzl`. Each entry references:
...
**Validate:** `bazel query //build/packaging/core-elx:all` and `bazel query //build/packaging/agent-gateway:all` show deb and rpm targets.
...
```bash
bazel build //build/release:package_manifest
cat bazel-bin/build/release/package_manifest.txt | grep -E "core-elx|agent-gateway"
```
