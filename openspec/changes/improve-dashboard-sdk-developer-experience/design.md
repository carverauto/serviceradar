...
**Risk: Templates drift from the canonical reference dashboard (UAL).** UAL's pattern evolves; templates need to keep up. *Mitigation*: the `react-map` template is kept structurally identical to a slimmed-down UAL `src/app/`. A unit test loads the template, runs it through the same shape projections, and asserts the dashboard mounts. When Example diverges, the test fails and the template gets updated.
