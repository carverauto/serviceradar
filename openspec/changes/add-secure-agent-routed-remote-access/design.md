...
- `elixir/web-ng/lib/serviceradar_web_ng_web/controllers/api/remote_access_session_controller.ex` exposes authenticated generic session create/show/close routes under `/api/remote-access/sessions` using the existing `:api_auth` pipeline. Responses include the one-time browser attach ticket only on create, never include attach-ticket hashes, and keep reusable credential material out of returned JSON.
- `elixir/web-ng/lib/serviceradar_web_ng_web/controllers/api/remote_access_stream_controller.ex` and `channels/remote_access_stream_handler.ex` expose the authenticated generic browser stream at `/v1/remote-access/sessions/:id/stream`. The handler consumes the one-time attach ticket, starts `RemoteAccessBroker`, forwards base64 terminal bytes and resize frames, handles ready/close/timeout events, and tests prove attach tickets and credential metadata are not echoed to the browser.
...
- `elixir/web-ng/assets/component/src/RemoteAccessSSHConsole.jsx`, `assets/js/hooks/RemoteAccessSSHConsole.js`, `ServiceRadarWebNGWeb.RemoteAccessLive.SSH`, and `/devices/:uid/remote-access/ssh` provide the first browser SSH console. The React form accepts pasted or uploaded per-session private keys, optional passphrases, displays a browser-local SHA-256 digest of the normalized key text, and can remember the key only in browser local storage when deployment policy explicitly enables that affordance. The key is not posted to session creation and is sent only in the authenticated WebSocket attach frame as a user-present credential envelope.
...
- `elixir/serviceradar_core/lib/serviceradar/edge/remote_access_ssh_certificate_policy.ex` is the pure policy boundary for generic SSH certificate requests. It checks the dedicated SSH remote-access permission, intersects requested principals with a target-approved principal policy, bounds TTL, and emits the session key ID/audit shape before any CA signs.
...
- `elixir/serviceradar_core/lib/serviceradar/edge/remote_access_ssh_ca_command_signer.ex` implements that signer behaviour by invoking an external JSON stdin/stdout command. It is the local bridge to a signer process and must not log request payloads or place CA private-key bytes in application state.
...
- `ServiceRadar.Edge.RemoteAccessSessions` enforces the first approval boundary before creating a generic session. Centrally brokered custody and any request or recording policy that explicitly marks approval as required must carry an approval ID, and an optional approval-checker module can reject stale or unauthorized approval IDs. Missing approval writes a denial audit with `rbac_decision: "approval_required"` and no attach ticket is issued.
