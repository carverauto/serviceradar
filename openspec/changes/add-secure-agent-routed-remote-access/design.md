...
- `elixir/web-ng/lib/serviceradar_web_ng_web/controllers/api/remote_access_session_controller.ex` exposes authenticated generic session create/show/close routes under `/api/remote-access/sessions` using the existing `:api_auth` pipeline. The current public create endpoint is the browser SSH API protected by `devices.remote_access.ssh.open`: it defaults requests to `protocol: "ssh"`, `adapter: "ssh"`, and `target_kind: "inventory_device"`, and rejects non-SSH protocols, non-SSH adapters, or provider-console target kinds until those surfaces get dedicated endpoints or permission checks. Responses include the one-time browser attach ticket only on create, never include attach-ticket hashes, and keep reusable credential material out of returned JSON. The public create API strips client-controlled SSH certificate policy metadata such as allowed principals, principal mappings, certificate envelopes, SSH auth objects, and credential mode overrides; those values must come from trusted target policy or session credential issuance paths. Browser-supplied `target_host` overrides are rejected by default and require an explicit deployment switch.
...
- `ServiceRadar.Edge.RemoteAccessSessions` enforces the first approval boundary before creating a generic session. Centrally brokered custody and any request or recording policy that explicitly marks approval as required must carry an approval ID and must be verified by a configured approval-checker module. Missing or unverified approval writes a denial audit and no attach ticket is issued.
