...
- `go/pkg/agent/remoteaccess/manager.go` is the agent-side generic frame boundary. It validates open/resize terminal dimensions and terminal data-frame size before touching the PTY, so the agent does not rely only on browser/web-ng validation.
