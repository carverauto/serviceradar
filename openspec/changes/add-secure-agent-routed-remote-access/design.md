...
- `elixir/serviceradar_core/lib/serviceradar/edge/remote_access_ssh_certificates.ex` orchestrates policy plus an injected signer module. The signer is intentionally replaceable so the CA private key can live in a dedicated signer service, OpenBao/KMS path, or native port instead of web-ng or agent-gateway.
- `elixir/serviceradar_core/lib/serviceradar/edge/remote_access_ssh_session_credentials.ex` combines a user-present private key with a signed certificate envelope for one broker open. The private key appears only in returned broker options and must not be copied into the certificate envelope, audit details, persisted session metadata, or logs.
