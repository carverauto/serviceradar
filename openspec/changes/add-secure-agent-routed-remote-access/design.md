...
- `elixir/web-ng/assets/component/src/RemoteAccessSSHConsole.jsx`, `assets/js/hooks/RemoteAccessSSHConsole.js`, `ServiceRadarWebNGWeb.RemoteAccessLive.SSH`, and `/devices/:uid/remote-access/ssh` provide the first browser SSH console. The React form accepts pasted or uploaded per-session private keys, optional passphrases, displays a browser-local SHA-256 digest of the normalized key text, and can remember the key only in browser local storage. The key is not posted to session creation and is sent only in the authenticated WebSocket attach frame as a user-present credential envelope.
