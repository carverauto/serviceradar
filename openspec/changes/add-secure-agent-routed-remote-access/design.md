...
- The certificate issuer returns the signed certificate, selected Unix login, credential mode, target, and audit metadata, but never returns or persists the matching private key. The remote-access broker merges that issuer envelope with the user-present session key only when emitting the one-time agent `open` frame.
