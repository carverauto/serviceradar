...
- [ ] 1.7 Generalize the frame and lane handshake before treating v1 as a public
  producer ABI. Add `EdgeOutputContractRef`, authenticated
  `EdgeProducerContext`, producer-local idempotency key semantics, generic
  assignment/run authorization, and finite platform route profiles. Retain
  payload kinds only as platform routing families; a package contract MUST NOT
  allocate a subject, stream, consumer, connection, or traffic class.
...
- [ ] 2.7 Add byte-based fair interleaving across network/site scopes, agents,
  producer assignments, runs/executions, and scheduler-attested traffic classes
  ...
  plugin, or integration run cannot monopolize the result window; callers
...
- [ ] 2.10 Implement the agent-owned producer sink and run API for in-process
  ...
  explicit retryable backpressure without accepting-and-dropping.
  ...
  lookup after uncertain outcomes, retain the binding for the producer retry
  horizon after spool reclamation, and reject same-key/different-body conflicts.
...
- [ ] 2.14 Keep bounded check/status summaries compatible with
  `GatewayServiceStatus` during migration, but move persistent metrics, traces,
...
- [ ] 3.8 Load the same signed output-contract/route registry epoch as the agent
  ...
  or downstream-not-ready contracts before PubAck; stamp host-attested producer
  provenance and never trust a producer-supplied subject, agent, scope, class,
  cost, or database destination.
...
- [ ] 5.10 Add registry-driven dispatch that verifies the exact output contract,
  complete bundle digest, cost model, route, and approved platform-owned projector before
  decode. A registry epoch is ready only when every gateway route and required
  EventWriter projector is installed; unknown/revoked contracts pause or DLQ by
  declared policy and never fall back to dynamic code or generic SQL.
...
- [ ] 6.6 Compile imported Wasm/native package output requests into explicit
  ...
  readiness approval. Revoke grants monotonically, stop new output immediately,
  and retain delivery-only authority for already-spooled immutable records.
...
- [ ] 6.10 Reconcile dependent active proposals before implementation:
  amend `add-external-inventory-wasm-plugin-contract` from one complete
  `plugin_result.v1` snapshot to bounded typed pages/terminal activation, and
  amend `add-event-writer-processor-contributions` so edge dispatch uses trusted
  contract headers on fixed subjects rather than package-selected subject
  filters. Validate each change independently and document archive order.
...
- [ ] 8.6 Add Go/Elixir/Wasm golden and adversarial tests for output-contract
  ...
  acceptance, and EventWriter rechecks body claims before side effects.
- [ ] 8.7 Prove uncertain local receipts return the same durable identity,
  native reconnect resumes without loss, and a complete consistency-proven
  inventory terminal applies absence once while partial/invalid/stale terminals
  apply none. Benchmark removal of Wasm base64/JSON allocation and Armis
  whole-run coalescing. Show a maximum-size plugin/inventory run cannot starve
  interactive or recovery traffic and that thousands of approved package
  contracts do not create proportional lanes, streams, consumers, connections,
