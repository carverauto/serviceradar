...
SHALL select v1 only when that version gate and the complete installation-local
  gateway, stream, registry, and consumer path are compatible. Agents below the gate SHALL
  receive no new affected durable work; the installation SHALL NOT create an
...
permissions. The scheduler MAY issue a short-lived delivery capability for
