...
- **AND** the telemetry SHALL remain owned by the terminator or common agent
  spool until the corresponding local or edge durability acknowledgement
...
during its contract migration; unsupported agents SHALL receive no new affected
work.
...
canonical fixed JetStream edge-record subjects and trusted output-contract
headers for persistent observations rather
