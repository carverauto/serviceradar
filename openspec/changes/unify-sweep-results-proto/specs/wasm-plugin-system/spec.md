...
- **THEN** the legacy checker status path SHALL continue unchanged during its
  declared compatibility window
- **AND** it SHALL NOT become an alternate durable ingestion path for new
  output contracts
...
`EdgeResultFrame`, spool coordinates, NATS subjects, route/partition selection,
...
filesystem, and scanning host-function capabilities.
