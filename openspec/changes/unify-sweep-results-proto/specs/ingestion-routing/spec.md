...
### Requirement: Edge observations use layered transport and durability
Agent-originated sweep observations, execution events, and MTR traces SHALL use
...
JetStream ingress publisher for these agent-originated observation types.
...
#### Scenario: Edge observation reaches durable ingress
- **GIVEN** an authenticated agent has a completed observation frame
- **WHEN** it sends the frame over the edge result stream
- **THEN** the gateway SHALL publish the canonical protobuf payload to its
  installation-local subject selected by payload kind, immutable traffic class,
...
#### Scenario: Broker-free sync remains on its existing path
- **GIVEN** an agent sends a large sync snapshot
- **WHEN** it uses the existing chunked gRPC results pipeline
- **THEN** the snapshot SHALL continue to use the sync-specific core ingestor
- **AND** this change SHALL NOT require JetStream or edge observation framing
  for sync snapshots
...
- **GIVEN** one long-lived result RPC carries frames for multiple executions
...
### Requirement: Edge observation frames are byte-bounded and versioned
Every edge observation frame SHALL declare its payload kind, schema version,
compression, encoded and uncompressed sizes, checksum, stable event ID,
persistent spool identity, monotonic spool sequence, and projected database-row
cost, projected database-write bytes, and cost-model version. It SHALL also
declare exactly one authoritative `network_scope_id`, immutable signed
`traffic_class`, authorization kind/context, and any execution/shard/epoch/range
ID/digest covered by its signed capability. The equivalent complete metadata
SHALL persist in canonical broker headers bound to the exact payload by an
immutable semantic-envelope digest plus a full persisted delivery/placement-
envelope digest. The semantic digest SHALL bind network scope, authenticated
agent, traffic class, payload/schema/cost contract, authorization, execution,
assignment, range, observation identity, and content. Spool ID/sequence,
logical/physical partition, gateway receipt/identity, stream-map placement, and
renewable delivery proof SHALL be excluded from the domain-semantic digest and
bound by the full envelope instead. Broker publication identity SHALL bind
trusted network scope/agent, lane, spool ID/sequence, and the semantic digest.
The initial target encoded size SHALL be 256 KiB and the hard
encoded size SHALL be 512 KiB plus bounded transport headers.

#### Scenario: Broker envelope is persisted
- **WHEN** the gateway durably publishes a valid frame
- **THEN** canonical headers SHALL include network scope, authenticated agent,
  traffic class, lane/spool coordinates, semantic identity, payload and cost
  metadata, authorization context, execution fence, logical partition, physical
  stream, and stream-map version
- **AND** delivery-only reauthorization MAY change only the delivery envelope,
  never the semantic digest, traffic class, or original collection proof
...
The agent SHALL retain each result frame in a crash-safe local spool until the
gateway reports a disposition covering that frame. The gateway SHALL report an
accepted disposition only after validating a JetStream PubAck for the exact
stable event ID and expected authoritative stream. Sweep-bulk,
sweep-interactive, MTR-bulk, MTR-interactive, and recovery SHALL use independent
spool/credit sequences. Spool-loss recovery SHALL use separately reserved
...
- **THEN** the agent SHALL stop new affected scan admission and page operators
...
#### Scenario: MTR lane is blocked
- **GIVEN** an MTR frame cannot be resolved
- **WHEN** later sweep observations are ready
- **THEN** the independent sweep spool and credit lane SHALL continue to make
  progress within its own limits
- **AND** no acknowledgement SHALL skip an unresolved sequence within either
  lane
...
- **THEN** operators SHALL be paged and new result admission SHALL stop before
  capacity eviction
- **AND** expiry beyond the declared durability SLO SHALL leave affected
  executions partial and emit a critical incident

### Requirement: Traffic-class routing is immutable and physically disjoint
Every result SHALL carry exactly one scheduler-attested `traffic_class` of
`bulk` or `interactive`. Sweep and MTR SHALL publish to the fixed
installation-local subjects `telemetry.sweep.v1.bulk.pNN`,
`telemetry.sweep.v1.interactive.pNN`, `telemetry.mtr.v1.bulk.pNN`, or
`telemetry.mtr.v1.interactive.pNN`. The signed capability, edge frame, semantic
digest, broker header, subject, and stream-map entry SHALL agree on the class.
...
claim queues SHALL use weighted fairness across site or `network_scope_id`,
authenticated agent, execution, and immutable traffic class.
Interactive work SHALL receive bounded reserved progress without bypassing
durability or starving bulk work. Per-scope, per-agent, per-execution, per-class,
...
#### Scenario: Consumer falls behind during an active sweep
...
- **THEN** bounded pull and publish windows SHALL propagate pressure toward the
  scanner
- **AND** lower-priority or overlapping scans SHALL be deferred before any
  unacknowledged result is overwritten
...
- **THEN** the agent SHALL stop opening target windows and SHALL NOT release the
  affected result as durable
- **AND** the execution SHALL pause or abort from reserved metadata capacity with
...
- **THEN** a redundant journal and scheduler capability SHALL permit a fresh
  ...
  corrupt sequence
- **AND** failure of both journal copies SHALL stop all scan admission and leave
  affected work to authoritative scheduler fencing rather than claim recovery
...
#### Scenario: Large and small executions share a gateway
- **GIVEN** one million-target execution and smaller interactive executions are
  active concurrently
...
- **AND** the large execution SHALL remain subject to per-scope, per-agent,
  per-execution, per-class, and installation byte/rate/backlog limits
...
permitted `network_scope_id`, traffic class, authorization context, execution,
assignment, and target range from the canonical edge mTLS session plus a locally
verifiable scheduler-signed collection or delivery capability. Deployment CA
...
#### Scenario: Opaque MTR records have mixed authorization contexts
- **GIVEN** an MTR frame declares one authorization context
- **WHEN** decoding reveals a trace from a different sweep, check, command,
  traffic class, assignment epoch, or target range
- **THEN** the consumer SHALL reject the entire frame as poison before side
  effects
- **AND** producers SHALL flush a batch whenever authorization context changes
...
retain exact trusted source headers, exact bounded collection-capability bytes
and their digest, semantic digest, immutable traffic class, original source
stream/sequence, body checksum and bytes, and bounded error metadata. DLQ
...
- **THEN** the consumer SHALL commit its domain and derived side effects at most
  once
- **AND** execution counts SHALL be reconciled from unique observations rather
  than incremented per delivery
...
- **WHEN** a committed event ID is received with a different body checksum or
  immutable schema/size/cost/traffic-class/execution/epoch/authorization/range
  envelope
...
- **THEN** the system SHALL republish through the current stream map with the
  original trusted network scope, authenticated agent, collection provenance,
  semantic digest, content, and traffic class
...
### Requirement: Ordinary observations remain stream records
Sweep observations and bounded MTR traces SHALL remain independently consumable
JetStream records. This change SHALL NOT add an object-reference fallback for an
oversize observation.
...
#### Scenario: Observation exceeds the event contract
- **WHEN** a sweep or MTR record exceeds its byte or projected-row budget
- **THEN** it SHALL be rejected or quarantined according to the result protocol
- **AND** it SHALL NOT be silently diverted into Object Store

### Requirement: Migration requires a minimum dual-path version
Before cutover, the scheduler and gateways SHALL enforce a minimum agent/gateway
version that supports both independently acknowledged bounded legacy JSON and
versioned protobuf result frames. Older binaries SHALL stop receiving new work
and SHALL be upgraded; this change SHALL NOT create an unpatched-agent bridge or
compatibility stream. Both supported formats SHALL enter JetStream through the
gateway and be persisted only by EventWriter. New v1 emission SHALL require both
the `edge-results:v1` gateway capability and explicit agent/cohort configuration;
a configuration hash SHALL NOT select a result format.
...
- **WHEN** the scheduler or gateway observes an agent that cannot emit bounded,
  stable-ID legacy frames or read retained v1 spool records
- **THEN** it SHALL stop assigning new result-producing work to that binary
- **AND** cutover readiness SHALL fail rather than route it through an unpatched
  compatibility bridge

#### Scenario: Patched mixed-format fleet operates during migration
- **WHEN** minimum-version legacy and enabled v1 agents report concurrently
- **THEN** each payload SHALL route by an explicit version or format marker
  without content sniffing
- **AND** every legacy frame SHALL be independently acknowledged, no larger than
  512 KiB plus bounded headers, and carry stable ID/checksum plus signed network-
  scope/range/class authority rather than one cumulative request
- **AND** both paths SHALL persist equivalent supported domain results through
  EventWriter
- **AND** one execution SHALL select exactly one authoritative format for its
  lifetime rather than dual-writing both
...
- **WHEN** configuration selects patched legacy format for new executions during
  rollback
- **THEN** existing v1 spool lanes SHALL continue through a compatible sender
  until drained
...
- **WHEN** all registered agents meet the minimum version, the supported offline
  upgrade window has elapsed, and all stream/spool backlog is drained
