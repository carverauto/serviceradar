...
MAY continue to process declared legacy sync compatibility payloads
through bounded streaming gRPC chunking during their migration window. Durable
...
- **GIVEN** an explicitly unmigrated legacy sync source remains inside its
  compatibility window
- **WHEN** its bounded sync payload exceeds a single compatibility message
...
### Requirement: Edge durable record frames are byte-bounded and versioned
Every edge durable record frame SHALL declare its platform payload family,
exact output-contract ID/version/bundle digest and registry epoch, authenticated
producer/package/assignment/run context, platform route profile, schema version,
...
epoch/range ID/digest covered by its signed capability. Trusted projected cost,
route, traffic class, and provenance SHALL be derived by the agent sink from the
approved contract/grant rather than accepted from a producer. The equivalent complete metadata
...
- **THEN** canonical headers SHALL include network scope, authenticated agent,
  ...
  metadata, output contract/registry, producer provenance, authorization
  context, run/execution fence, route profile, logical partition, physical
...
stable event ID and expected authoritative stream. The finite platform-owned
