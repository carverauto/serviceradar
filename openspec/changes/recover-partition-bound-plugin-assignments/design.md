...
The original row remains disabled and unbound. A retry is idempotent: a completed recovery returns its recorded replacement rather than creating a second enabled assignment. If a different enabled assignment now owns the same `(partition, agent, plugin)` tuple, the recovery fails with an actionable conflict and does not overwrite it. The immutable audit row is not a generic plugin-manager read model: only a named internal audit lookup can read it, and only after the recovery boundary has authorized the exact legacy row. Package detail projects the completed manual result only as `reapproved`.
...
suggests that `default` is an acceptable inferred partition. Candidate discovery
is connected-socket work, not a disconnected-mount database query, and is
bounded in keyset-backed pages rather than arbitrary offset scans. A completed
manual audit projects only `reapproved`; a completed policy request projects
only `reconciled` and a replacement count. Those historical rows leave the
actionable queue and their package-detail controls become non-actionable,
without disclosing audit/request payloads or replacement identifiers.
...
The detailed audit rows are readable only by the named internal recovery lookup;
the user-facing recovery read model first authorizes the exact legacy row and
then projects only allowlisted status.
