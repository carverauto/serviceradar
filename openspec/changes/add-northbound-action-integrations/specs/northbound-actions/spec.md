...
The system SHALL maintain a provider-neutral catalog of executable northbound actions exposed by configured integrations or approved Wasm plugins.
...
The system SHALL determine action eligibility from target scope, required context, provider health, approved capabilities, configured credentials, and actor permissions.
...
- **THEN** the Run Task control is disabled
...
The system SHALL support launching actions from selected devices and selected interfaces using the same provider-neutral invocation model.
