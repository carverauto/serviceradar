...
- [ ] 1.1 Land the pending dialect fixes found by the first cutover: counter `rate`/`rate_sum`, tag-split series (`core_id`, `tags.<key>`), newest-N `sort:desc limit`.
...
- [ ] 2.4 `other:true` top-N with an "Other" tail for flows and metrics.
- [ ] 2.5 Route the direct CNPG readers through `Readers`: dashboard throughput sparklines, device Flows-tab presence probes, and any logs/events stat card that bypasses SRQL.
- [ ] 2.6 Measure log search on the deployed profile: which index types shared-data supports, and latency of a substring search over 1, 30 and 365 days; document the supported behaviour.
- [ ] 2.7 Run the parity harness for `logs` and `events`; cut each over in demo values only after it passes; verify cards and charts against ground truth after the rollout completes.
...
- [ ] 4.3 `SET LOCAL statement_timeout = 0` and `lock_timeout = 0` on the migration lock transaction.
...
- [ ] 5.2 Per-dataset `cnpgWrites` switch in Helm/Compose, default on; EventWriter honours it; turning it off while the dataset is not cut over is refused at startup.
...
- [ ] 5.5 Separate reviewed migration(s) dropping each retired hypertable, its continuous aggregates, retention and compression policies and jobs; only after 5.4 has held for the dataset.
