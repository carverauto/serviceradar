...
- **GIVEN** a device whose metadata includes `armis_access_switch` = `switch01.example.com:gi1/0/7`
- **AND** no other present source reports a disagreeing switch-port attachment
- **WHEN** fact promotion runs
- **THEN** `ocsf_devices.switch_port_attachment.switch_hostname` SHALL be `switch01.example.com`
- **AND** `ocsf_devices.switch_port_attachment.port` SHALL be `gi1/0/7`
- **AND** `ocsf_devices.switch_port_attachment.source` SHALL be `armis`
- **AND** `metadata.armis_access_switch` SHALL still equal `switch01.example.com:gi1/0/7`
...
- **GIVEN** a device whose metadata includes `armis_vlans` = `[200]`
- **AND** no other present source reports a disagreeing VLAN identifier
- **WHEN** fact promotion runs
- **THEN** `ocsf_devices.vlan_uid` SHALL be `200`
- **AND** `metadata.armis_vlans` SHALL still equal `[200]`
