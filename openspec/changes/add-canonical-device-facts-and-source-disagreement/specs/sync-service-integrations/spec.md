...
- **GIVEN** an Armis payload maps to metadata `armis_access_switch` = `switch02.example.com:1/1/41`
- **WHEN** the sync update is ingested
- **THEN** metadata SHALL still contain `armis_access_switch`
- **AND** a `switch_port_attachment` fact SHALL be recorded with hostname `switch02.example.com` and port `1/1/41`

#### Scenario: Existing Armis VLAN metadata becomes a vlan_uid fact
- **GIVEN** an Armis payload maps to metadata `armis_vlans` = `[200]`
- **WHEN** the sync update is ingested
- **THEN** metadata SHALL still contain `armis_vlans`
- **AND** a `vlan_uid` fact SHALL be recorded with value `200`
