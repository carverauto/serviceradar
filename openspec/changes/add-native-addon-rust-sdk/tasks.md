...
- [ ] 1.1 Generate Rust gRPC stubs from `proto/agent/addon/v1/` (tonic or equivalent).
- [ ] 1.2 Implement the go-plugin handshake (magic cookie, app protocol version) and
...
- [ ] 2.1 Build a Rust reference add-on against the helper that the agent supervises as
  an `agent-sidecar`, proving Go↔Rust interop end to end. (§6b.3)
- [ ] 2.2 Add it to `build/native_addons/addon_inventory.bzl` so it builds/bundles/signs
  like a first-party add-on.

## 3. Reference consumer manifest
- [ ] 3.1 Author `addons/fingerprintd/addon.yaml` (`pushed-artifact` / `agent-sidecar`,
  `language: rust`) and confirm the contract expresses it without gaps. (§9.1)

## 4. Validation
- [ ] 4.1 `openspec validate add-native-addon-rust-sdk --strict` passes.
- [ ] 4.2 An integration test launches the Rust reference add-on via the agent's
