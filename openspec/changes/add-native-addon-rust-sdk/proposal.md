...
already exist. The planned `fingerprintd` capability is Rust. To unblock Rust add-ons
we need a Rust handshake/gRPC helper (or a precise documented contract) and a Rust
reference add-on proving end-to-end interop with the agent's go-plugin client.
...
- Author a `fingerprintd` `addon.yaml` (`pushed-artifact` / `agent-sidecar`,
...
- **Affected code:** a Rust crate under `rust/` implementing the handshake/contract;
  ...
  `addons/fingerprintd/` manifest package.
