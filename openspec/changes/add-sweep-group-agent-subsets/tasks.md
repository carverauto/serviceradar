...
- [ ] 1.1 Add normalized `SweepGroup.agent_ids`, accept it on create/update,
...
- [ ] 1.2 Author a narrowly scoped expand Ecto migration: add/backfill/index
...
- [ ] 1.3 Make new code read only `agent_ids` behaviorally and enable subset
...
- [ ] 1.4 Keep the old-writer trigger/scalar/index through the rollback window;
...
- [ ] 1.6 Replace a superseded agent UID wherever it occurs in an explicit
...
- [ ] 1.7 Update `:by_agent`, `:for_agent_partition`, documentation, summaries,
  and fixtures to use empty-array or nonblank membership semantics.
- [ ] 1.8 Add resource/read-action, query-plan, migration, and Helm migration
...
- [ ] 2.1 Update sweep config loading so every selected agent receives the
  group, unselected/nil agent identities do not receive selected groups, and
  empty assignments remain partition-wide.
- [ ] 2.2 Preserve fleet-wide sweep dependency invalidation on membership edits
  so deselected agents drop stale groups and newly selected agents receive them.
- [ ] 2.3 Update composite-check sweep coverage and assignment labels to use
  `agent_ids` membership.
- [ ] 2.4 Add compiler/distribution, `SweepContext`, and direct
...
- [ ] 3.1 Extend `AgentCommandBus.run_sweep_group/2` to dispatch a non-empty
  selection to each selected agent's live control-session partition.
- [ ] 3.2 Return successful command IDs and per-agent failures for partial
...
- [ ] 3.3 Key command status by group plus agent/command ID and derive a group
  aggregate so later ACK/progress/completion events cannot overwrite another
  selected agent's state.
- [ ] 3.4 Update the sweep-group LiveView event and status presentation to
  distinguish complete, partial, and zero-success dispatch and retain member
  failure details.
- [ ] 3.5 Add command-bus and LiveView tests for all selected online, mixed
...
- [ ] 4.1 Replace scalar conflict detection with expected-assignment logic for
  empty, one, and many selections.
- [ ] 4.2 Suppress conflict warnings for expected selected reporters, warn for a
...
- [ ] 4.3 Integrate canonical availability with
...
- [ ] 4.4 Update missed-sweep payloads and operator summaries from `agent_id` to
  `agent_ids`; document/test that `last_run_at` is the latest report from any
  member and does not prove every member reported.
- [ ] 4.5 Add result-ingestion tests for expected multi-agent reporting,
...
- [ ] 5.1 Add a scoped `Agent.:agent_picker` Ash read with case-insensitive
  ...
  page consumers.
- [ ] 5.2 Add focused picker state/events for open, search, next/previous page,
  ...
  query changes.
- [ ] 5.3 Replace the scalar select with explicit All/Selected modes, a concise
...
- [ ] 5.4 Build the modal with `ui_modal`, stable debounced autofocus, checkbox
...
- [ ] 5.5 Bound summary loading: fetch one Agent only for a one-UID summary,
...
- [ ] 5.6 Update sweep-group list/detail summaries and render grandfathered
  stale selected IDs as unavailable without blocking unrelated edits.
- [ ] 5.7 Add LiveView/component tests for stable ordering, forward/back cursor
...
- [ ] 6.1 Run `mix format` and the focused `serviceradar_core` resource,
  compiler, command-bus, result-ingestion, and composite-check tests.
- [ ] 6.2 Run the focused web-ng LiveView/component tests and verify each new
  test is actually selected by its Bazel shard.
- [ ] 6.3 Run the affected Bazel test targets, then the repository-required
  `make test` unit sweep before opening a pull request.
- [ ] 6.4 Verify expand/backfill, old-writer mirroring, and fail-narrow scalar
  projection against nil, single-agent, and multi-agent rows; leave scalar
  cleanup to the later deprecation release.
- [ ] 6.5 Run `openspec validate add-sweep-group-agent-subsets --strict` and
