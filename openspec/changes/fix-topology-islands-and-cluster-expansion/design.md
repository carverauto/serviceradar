...
- Non-Goals:
  ...
  - Switching to a force-directed or ELK-primary layout; ELK stays as fallback only.
...
- **D2: Attachment-satellite placement on the client.** Devices whose only edges are attachment-plane rows get placed on a ring/spiral around their anchor node (reusing the hosted-island ring mechanics), instead of falling into unplaced residual lanes. The backend emits `attachment_anchor_id` in node metadata so the client needs no identity fusion.
...
- **D4: Spiral expanded-cluster geometry.** Replace the rigid grid with an Archimedean spiral around the anchor (`r(θ) = r0 + bθ`), two interleaved arms so consecutive members alternate sides; ordering groups members by parent port before spiraling. Candidate placements (right/left/down/up start angles) are scored by node-overlap count AND segment-intersection count between member edges and non-member edges; the lowest score wins. Grid layout remains the degenerate fallback for N=1..2.
