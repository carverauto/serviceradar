...
- Anchor devices whose only connectivity is an endpoint attachment next to their attachment anchor in the client layout (ring/spiral satellite placement) instead of routing them to unplaced residual lanes.
...
- Replace the expanded-cluster rigid grid placement with a collision-free spiral arrangement around the anchor, selected by scoring both node overlap and edge crossings.
