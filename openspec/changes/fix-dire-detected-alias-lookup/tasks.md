...
- [ ] 2.1 Add `lookup_detected_aliases/2` function to `DeviceLookup` that returns detected aliases as fallback candidates
- [ ] 2.2 Modify `batch_lookup_by_ip/2` to accept `include_detected: true` option
- [ ] 2.3 Add unit tests for detected alias fallback behavior

## 3. Sweep Ingestor Integration

- [ ] 3.1 Update `process_batch/3` to check detected aliases before creating new devices
- [ ] 3.2 When using a detected alias, call `confirm_from_sweep` action to promote state
- [ ] 3.3 When creating a new sweep device, create an IP alias record (state: detected)
- [ ] 3.4 Add integration tests for sweep-detected alias resolution

## 4. DeviceAliasState Enhancement

- [ ] 4.1 Add `confirm_from_sweep` action that promotes detected → confirmed immediately
- [ ] 4.2 Record metadata about the sweep that triggered confirmation
- [ ] 4.3 Add tests for the new confirmation action

## 5. IdentityReconciler Enhancement

- [ ] 5.1 Update `lookup_alias_device_id/3` to check detected aliases when no confirmed alias found
