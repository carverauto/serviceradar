...
`//:buildbuddy_setup_fixture_env` and the Forgejo materializer obtain PEM content as follows:

1. **kubectl**, when `kubectl` is on PATH and `can-i get secrets` in `srql-fixtures` succeeds.
   Read `srql-fixture-server-ca` key `ca.crt`. This is the workstation and any future
   in-cluster runner with RBAC.
2. **HTTPS GET** of `SRQL_FIXTURE_CA_URL`, default
   `https://srql-fixture-ca.serviceradar.cloud/ca.crt`. The URL is terminated by
   `letsencrypt-dns`, so the Firecracker VM can fetch it with the public trust store and no
   kubeconfig.
3. **Fail closed.** Do not use an ambient `SRQL_TEST_DATABASE_CA_CERT` from the BuildBuddy or
   Forgejo secret store. That is the snapshot this change deletes.
...
A one-file publisher in `srql-fixtures` (or a Gateway HTTPRoute if that is less new surface)
serves only `ca.crt` from the cert-manager Secret / a projected ConfigMap. ExternalDNS already
knows how to create `srql-fixture.serviceradar.cloud`; add
`srql-fixture-ca.serviceradar.cloud` the same way. TLS on that hostname uses `letsencrypt-dns`,
not the fixture CA — otherwise fetching the CA would require the CA.
