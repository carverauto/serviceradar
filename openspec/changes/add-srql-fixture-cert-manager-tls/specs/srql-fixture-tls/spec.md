...
BuildBuddy and Forgejo fixture setup SHALL obtain `SRQL_TEST_DATABASE_CA_CERT` at the start
of each database job from a live source: the Kubernetes CA Secret when the caller has
`get secrets` in `srql-fixtures`, otherwise the published HTTPS CA bundle. Setup SHALL still
emit PEM **content** (not a caller-owned path) into the per-run environment consumed by
...
#### Scenario: Forgejo uses the same live-source rules
- **GIVEN** a Forgejo database job is configuring the fixture
