...
- [x] 1.4 Identify must-have parity items for v1 (documented in `proposal.md`)
- [x] 1.5 Identify deferred items (documented in `proposal.md`) and mark as Phase F/G
...
- [x] 4.4 Repo checks are green (`make lint`, `make test`). Note: `make test` skips DB-backed `web-ng` tests unless `SERVICERADAR_REQUIRE_DB_TESTS=1`.
