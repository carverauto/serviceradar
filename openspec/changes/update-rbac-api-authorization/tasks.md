...
- [x] 1.1 Update `elixir/web-ng/lib/serviceradar_web_ng_web/plugs/api_auth.ex` to verify Guardian bearer tokens with explicit `token_type` and accept only `access` and `api`.
- [x] 1.2 Ensure OAuth2 client credentials tokens are minted with a consistent API token type (and claims) that matches API auth expectations (`elixir/web-ng/lib/serviceradar_web_ng_web/controllers/oauth_controller.ex`).
...
- [x] 2.1 Remove SystemActor execution from `elixir/web-ng/lib/serviceradar_web_ng_web/controllers/api/collector_controller.ex` for admin operations and enforce an explicit RBAC permission.
- [x] 2.2 Pass the authenticated actor through `elixir/web-ng/lib/serviceradar_web_ng_web/controllers/api/edge_controller.ex` list/get/events paths and enforce `settings.edge.manage`.
- [x] 2.3 Audit `elixir/web-ng/lib/serviceradar_web_ng_web/controllers/api/*.ex` under `:api_key_auth` to ensure each endpoint either:
...
- [x] 2.4 Fix stale router action mapping for `CollectorController.account_status/2` (`elixir/web-ng/lib/serviceradar_web_ng_web/router.ex`).

## 3. Edge Onboarding Context Modules
- [x] 3.1 Remove implicit SystemActor fallback from `elixir/web-ng/lib/serviceradar_web_ng/edge/onboarding_packages.ex` for user-facing operations (`list/2`, `get/2`, `delete/2`, `revoke/2`, `deliver/3`), or require an explicit opt-in to system mode.
- [x] 3.2 Remove implicit SystemActor fallback from `elixir/web-ng/lib/serviceradar_web_ng/edge/onboarding_events.ex` for user-facing reads (`list_for_package/2`, `recent/1`).
...
- [x] 4.2 Gate `/admin/jobs*` LiveViews behind `settings.jobs.manage` and enforce permission checks in `handle_event/3` (especially `trigger_job` in `elixir/web-ng/lib/serviceradar_web_ng_web/live/admin/job_live/show.ex`).
...
- [x] 8.2 Run `cd elixir/web-ng && mix test` and any targeted `elixir/serviceradar_core` tests affected by policy hardening.
