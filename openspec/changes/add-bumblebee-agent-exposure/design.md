...
- On Linux, package the scanner as a systemd unit/timer pair such as `serviceradar-bumblebee-scan.service` and `serviceradar-bumblebee-scan.timer`. The timer provides the normal cadence; future on-demand runs should prefer systemd activation rather than a custom privileged RPC protocol.
