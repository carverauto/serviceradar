...
generate_cert "log-collector" "serviceradar-log-collector" "DNS:serviceradar-log-collector,DNS:log-collector,DNS:log-collector.serviceradar,DNS:serviceradar-log-collector.{{ .Release.Namespace }}.svc.cluster.local,DNS:localhost,IP:127.0.0.1"
...
if [ -f "$CERT_DIR/tools.pem" ] && { [ ! -f "$CERT_DIR/client.pem" ] || [ "$FORCE_REGENERATE" = "true" ]; }; then
  cp "$CERT_DIR/tools.pem" "$CERT_DIR/client.pem"
  chmod 644 "$CERT_DIR/client.pem"
fi
if [ -f "$CERT_DIR/tools-key.pem" ] && { [ ! -f "$CERT_DIR/client-key.pem" ] || [ "$FORCE_REGENERATE" = "true" ]; }; then
