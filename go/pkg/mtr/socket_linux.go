...
// OpenTCPFlow crafts SYNs on a raw TCP socket when the process may open one,
// and otherwise falls back to kernel connect() probes (reach detection only).
func (s *linuxRawSocket) OpenTCPFlow(dst net.IP, dstPort int, timeout time.Duration) (TCPFlow, error) {
	if s.sendFD >= 0 {
		flow, err := openRawTCPFlow(dst, dstPort, s.ipv6)
		if err == nil {
			return flow, nil
		}

		if !errors.Is(err, errRawTCPPermission) {
			return nil, err
		}
	}
