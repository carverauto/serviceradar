...
func TestMarshalSweepMetricEnvelope(t *testing.T) {
	...
	require.Equal(t, batch.EmittedAtUnixNano, metrics["sweep.total_hosts"].Points[0].ObservedAtUnixNano)
	require.Equal(t, "1780000000", entry(metrics["sweep.total_hosts"].Points[0].Metadata, "last_sweep"))
	require.NotEqual(t, uint64(1_780_000_000)*uint64(time.Second), metrics["sweep.total_hosts"].Points[0].ObservedAtUnixNano)
