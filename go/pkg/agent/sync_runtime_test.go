...
func TestArmisAccessTokenUsesSecretKeyCredential(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if payload["api_key"] != "key-1" {
			t.Fatalf("api_key = %q, want key-1", payload["api_key"])
		}
		if payload["secret_key"] != "secret-1" {
			t.Fatalf("secret_key = %q, want secret-1", payload["secret_key"])
		}
		if _, ok := payload["api_secret"]; ok {
			t.Fatalf("api_secret should not be sent to Armis token endpoint: %#v", payload)
		}
