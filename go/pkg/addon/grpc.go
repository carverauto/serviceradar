...
func (c *grpcClient) StreamTelemetry(ctx context.Context) (<-chan *addonpb.TelemetryBatch, error) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				c.emitStreamDiagnostic(ctx, "telemetry", err)
				return
			}
			select {
			case out <- batch:
...
func (c *grpcClient) StreamArtifacts(ctx context.Context) (<-chan *addonpb.ArtifactUploadChunk, error) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				c.emitStreamDiagnostic(ctx, "artifacts", err)
				return
			}
			select {
			case out <- chunk:
...
func (c *grpcClient) StreamMetricFeed(ctx context.Context) (chan<- *addonpb.MetricFeedFrame, <-chan uint64, error) {
	...
	go func() {
		for {
			...
			case frame, ok := <-frames:
				...
				if err := stream.Send(frame); err != nil {
					c.emitStreamDiagnostic(streamCtx, "metric_feed_send", err)
					cancel()
					return
				}
			}
	...
	go func() {
		...
		for {
			ack, err := stream.Recv()
			if err != nil {
				c.emitStreamDiagnostic(streamCtx, "metric_feed_ack", err)
				return
			}
			select {
