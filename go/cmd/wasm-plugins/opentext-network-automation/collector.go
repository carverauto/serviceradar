...
func (c *Collector) Collect(ctx context.Context, cfg Config) (Snapshot, error) {
	if c == nil || c.HTTP == nil {
		return Snapshot{}, runError("network_automation_transport_unavailable")
	}
	if err := cfg.Validate(); err != nil {
		return Snapshot{}, runError("network_automation_config_invalid")
	}
	...
	for _, query := range cfg.Queries {
		...
		for {
			...
			if status == http.StatusUnauthorized {
				return Snapshot{}, runError("network_automation_auth_failed")
			}
			if status == http.StatusForbidden {
				return Snapshot{}, runError("network_automation_forbidden")
			}
			if status != http.StatusOK {
				return Snapshot{}, runError("network_automation_api_unavailable")
			}

			pages++
			receivedRows += len(rows)
			if receivedRows > cfg.MaxRows {
				return Snapshot{}, runError("network_automation_row_limit_exceeded")
			}
			...
			for _, raw := range rows {
				...
				if !ok || row.DeviceID == "" {
					return Snapshot{}, runError("network_automation_device_invalid")
				}
				...
				if previous, exists := devices[row.DeviceID]; exists {
					if !sameInventoryDevice(previous, device) {
						return Snapshot{}, runError("network_automation_duplicate_conflict")
					}
					duplicateRows++
					continue
				}
			...
			if maxID < startID {
				return Snapshot{}, runError("network_automation_pagination_invalid")
			}
			nextStartID := maxID + 1
			if nextStartID <= startID {
				return Snapshot{}, runError("network_automation_pagination_stalled")
			}
			startID = nextStartID
		}
	}
	...
	if err != nil {
		return Snapshot{}, runError("network_automation_snapshot_hash_failed")
	}
...
func (c *Collector) fetchPage(
	...
	if err != nil {
		return nil, 0, runError("network_automation_request_invalid")
	}
	...
	if err != nil {
		return nil, response.Status, runError("network_automation_response_invalid")
	}
...
func (c *Collector) doWithRetry(ctx context.Context, cfg Config, request HTTPRequest) (HTTPResponse, error) {
	for attempt := 0; attempt <= cfg.MaxRetries; attempt++ {
		...
		if attempt == cfg.MaxRetries {
			return HTTPResponse{}, runError("network_automation_upstream_unavailable")
		}
		if err := c.sleep(ctx, time.Duration(1<<attempt)*100*time.Millisecond); err != nil {
			return HTTPResponse{}, runError("network_automation_request_canceled")
		}
	}
	return HTTPResponse{}, runError("network_automation_upstream_unavailable")
}
...
func normalizeDeviceRow(raw map[string]any) (networkAutomationDeviceRow, bool) {
	row := networkAutomationDeviceRow{
		...
		Serial:           valueString(raw, "serialNumber", "serial", "chassisSerial"),
		...
		ManagementStatus: valueString(raw, "managementStatus", "status"),
...
func inventoryDevice(instanceID string, row networkAutomationDeviceRow) InventoryDevice {
	integrationID := fmt.Sprintf(
		"opentext-network-automation:v1:%s:device:%s",
		instanceID,
		row.DeviceID,
	)
	...
	return InventoryDevice{
		...
		Metadata: map[string]any{
			...
			"integration_type": "opentext-network-automation",
