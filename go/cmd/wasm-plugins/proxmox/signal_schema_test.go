...
func TestProxmoxSignalSchemaRefMatchesShippedPackageContract(t *testing.T) {
	...
	producerID, producerVersion, signal := readManifestSignal(t, manifestPath, proxmoxSignalSchemaID)
...
func signalSchemaTestFile(t *testing.T, relative string) string {
	...
	if testSrcDir := os.Getenv("TEST_SRCDIR"); testSrcDir != "" {
		packagePath := filepath.Join("go", "cmd", "wasm-plugins", "proxmox", relative)
