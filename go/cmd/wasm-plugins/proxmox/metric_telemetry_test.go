...
func TestEncodeMetricBatchProducesCanonicalMetricEnvelope(t *testing.T) {
	payload := encodeMetricBatch(metricBatch{
		...
		Samples: []metricTelemetrySample{{
			Name:       "proxmox_node_cpu_ratio_max",
			Value:      0.91,
			Unit:       "ratio",
			Thresholds: sdk.Thresholds(0.80, 0.90),
		}},
	})
	...
	if got := protoString(t, resource, 4); got != proxmoxSignalSchemaProducerID {
		t.Fatalf("resource.service_name = %q", got)
	}
	metric := parseSingleMessage(t, root, 20)
	if got := protoString(t, metric, 1); got != "proxmox_node_cpu_ratio_max" {
		t.Fatalf("metric name = %q", got)
	}
	point := parseSingleMessage(t, metric, 20)
	if got := protoDouble(t, point, 1); got != 0.91 {
		t.Fatalf("point value = %v", got)
	}
	thresholds := metric[32]
	if len(thresholds) != 2 {
		t.Fatalf("threshold count = %d, want warn+crit", len(thresholds))
	}
	warn := parseProtoFields(t, thresholds[0].data)
	if got := protoString(t, warn, 1); got != "warn" {
		t.Fatalf("threshold key = %q", got)
	}
	if got := protoString(t, warn, 2); got != "0.8" {
		t.Fatalf("warn threshold = %q", got)
	}
}

func TestProxmoxMetricSamplesPreserveInventorySummary(t *testing.T) {
	samples := proxmoxMetricSamples(proxmoxDetails{
		Summary: checkSummary{Targets: 1, Nodes: 2, Guests: 3},
	...
	for _, sample := range samples {
		names[sample.Name] = sample.Value
	}
	if names["proxmox_targets"] != 1 || names["proxmox_nodes"] != 2 || names["proxmox_guests"] != 3 {
		t.Fatalf("summary metrics missing: %#v", names)
	}
	if names["proxmox_node_cpu_ratio_max"] != 0.7 || names["proxmox_ceph_warn_nodes"] != 1 {
		t.Fatalf("resource metrics missing: %#v", names)
	}
}
