...
func emitAxisMetricTelemetry(sourceInstance string, metrics map[string]metricPoint) {
	if len(metrics) == 0 {
		return
	}
	...
	samples := make([]metricTelemetrySample, 0, len(keys))
	for _, key := range keys {
		point := metrics[key]
		samples = append(samples, metricTelemetrySample{
			Name:  "axis_" + key,
			Value: point.Value,
			Unit:  point.Unit,
		})
	...
	payload := encodeMetricBatch(metricBatch{
		Resource: metricResource{
			...
			Attributes: []metricStringMapEntry{
				{Key: "camera_host", Value: sourceInstance},
			},
		},
	...
	err := sdk.EmitTelemetry(sdk.TelemetryBatch{
		...
		Records: []sdk.TelemetryRecord{{
			EventID:              "axis-metrics-" + strconv.FormatUint(now, 10),
	...
	if err != nil {
		sdk.Log.Warn("failed to emit axis metric telemetry: " + err.Error())
	}
}
