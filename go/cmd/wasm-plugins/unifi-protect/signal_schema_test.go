...
func TestProtectSignalSchemaRefMatchesShippedPackageContract(t *testing.T) {
	manifestPath := signalSchemaTestFile(t, "plugin.yaml")
	producerID, producerVersion, signal := readManifestSignal(t, manifestPath, protectSignalSchemaID)
...
func signalSchemaTestFile(t *testing.T, relative string) string {
	...
	if testSrcDir := os.Getenv("TEST_SRCDIR"); testSrcDir != "" {
		packagePath := filepath.Join("go", "cmd", "wasm-plugins", "unifi-protect", relative)
