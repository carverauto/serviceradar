...
// pluginInputItem is a single resolved SRQL target. The controller host is
// derived from ip -> device_ip -> host -> hostname -> name (ip preferred).
...
// (for example rtsp://10.0.0.5:7447/abcd). Streaming plugins use this so media
// works when the agent injects only the relay object and no host envelope.
