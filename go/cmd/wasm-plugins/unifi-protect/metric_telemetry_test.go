...
func TestEncodeMetricBatchProducesCanonicalMetricEnvelope(t *testing.T) {
	payload := encodeMetricBatch(metricBatch{
		...
		Samples: []metricTelemetrySample{{
			Name:  "unifi_protect_camera_total",
			Value: 2,
			Unit:  "count",
		}},
	})
	...
	if got := protoString(t, resource, 4); got != protectSignalSchemaProducerID {
	...
	if got := protoString(t, metric, 1); got != "unifi_protect_camera_total" {
		t.Fatalf("metric name = %q", got)
	}
	point := parseSingleMessage(t, metric, 20)
	if got := protoDouble(t, point, 1); got != 2 {
