...
```bash
# Build the image
docker build -f go/cmd/faker/Dockerfile -t serviceradar-faker .
...
Enable by setting `simulation.bgp.enabled=true` in `go/cmd/faker/config.json` (or your deployed config).
