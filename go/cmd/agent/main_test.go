...
func TestLoadConfigRejectsUnexpectedUnknownFields(t *testing.T) {
	t.Parallel()

	path := writeAgentConfig(t, `{
		"agent_id": "k8s-agent",
		"checkers_dir": "/var/lib/serviceradar/checkers",
		"definitely_not_a_real_field": true
	}`)

	_, err := loadConfig(path)
	if err == nil {
		t.Fatal("loadConfig accepted an unexpected unknown field")
	}

	if !strings.Contains(err.Error(), `unknown field "definitely_not_a_real_field"`) {
