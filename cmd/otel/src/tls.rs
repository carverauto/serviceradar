...
pub fn setup_grpc_tls(config: &Config) -> Result<Option<ServerTlsConfig>, Box<dyn std::error::Error>> {
    ...
    let cert = fs::read(&tls.cert_file)
        .map_err(|e| format!("Failed to read gRPC TLS certificate file '{}': {e}", tls.cert_file))?;
    
    debug!("Loading gRPC TLS private key from: {}", tls.key_file);
    let key = fs::read(&tls.key_file)
        .map_err(|e| format!("Failed to read gRPC TLS private key file '{}': {e}", tls.key_file))?;
...
pub fn log_tls_info(config: &Config) {
    if let Some(ref tls) = config.grpc_tls {
        info!("gRPC TLS enabled - cert: {}, key: {}", tls.cert_file, tls.key_file);
