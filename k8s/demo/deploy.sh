    ...
    if [ "\${WAIT_FOR_PROTON:-true}" = "true" ]; then
        ...
        for i in {1..30}; do
            if command -v openssl >/dev/null 2>&1; then
                ...
                if timeout 5 bash -lc "exec 3<>/dev/tcp/\${PROTON_TLS_HOST}/\${PROTON_TLS_PORT}" > /dev/null 2>&1; then
                    TLS_READY=true
                fi
            fi
        ...
        if [ "\$TLS_READY" != "true" ]; then
            echo "Warning: Proton TLS port did not respond after 30 attempts; proceeding anyway" >&2
        fi
    fi
