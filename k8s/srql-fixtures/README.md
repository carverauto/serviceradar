...
- `ca-bundle.yaml` – Caddy static publisher for only `ca.crt` at `https://srql-fixture-ca.serviceradar.cloud/ca.crt` via the shared Envoy gateway, plus an in-cluster ClusterIP HTTP Service for ARC. Envoy terminates TLS; Caddy is HTTP-only. Do not put nginx on this path.
