...
- **Causal / statistical / streaming-anomaly reasoning**: use the **DeepCausality** library (`deep_causality_core` Flow API plus `deep_causality_data_structures` `SlidingWindow`; source at `~/src/deep_causality`, shipped through `causal_reasoner_nif`). Authored by Marvin Hansen, who guides ServiceRadar's anomaly-engine design. **Do not hand-roll a parallel detector** for rolling z-score, running mean/variance, sliding windows, CSM, or equivalent anomaly decisions in Elixir or Go when DeepCausality already provides the primitive. A second implementation must be kept in numeric parity by hand and can drift. One reasoner, in the NIF, is the source of truth. Performance is not a reason to fork: O(1) Welford inside DeepCausality benchmarks around 100M evals/sec/core versus around 2.8M/sec for the O(window) recompute, and per-sample FFI cost is solved by batching (`reason_batch`), not by a separate Elixir engine. If DeepCausality lacks a primitive, add it upstream or wrap it behind a project-owned module instead of creating a divergent reimplementation. See `openspec/changes/refactor-anomaly-reasoner-deepcausality`.
