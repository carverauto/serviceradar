...
3. **Version-bump gate** — register `<name>` in `scripts/check-native-addon-version-bumps.sh` (`addon_ids`, `manifest_path`, `path_belongs_to_addon`, plus `cargo_version_path` for Rust add-ons). Any change to the add-on's source/config/unit/bundle inventory requires bumping `addons/<name>/addon.yaml` `version`; for Rust add-ons the `Cargo.toml` `[package] version` must match. Rust add-ons also need `MODULE.bazel.lock` to record the current `Cargo.lock` hash — run `bazel --batch mod deps --lockfile_mode=update` (twice if the first pass rewrites `Cargo.lock` mid-run, since the recorded hash lags one pass).

Run `bash scripts/check-native-addon-version-bumps.sh origin/staging <your-commit-sha>` locally to verify (note: with jj, git `HEAD` is the parent — pass the real commit SHA, e.g. `jj log -r @ --no-graph -T commit_id`).
