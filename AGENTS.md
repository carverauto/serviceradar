...
- **Two system crates are patched and pinned: `openssl-src` and `pq-src`** (patches in `//third_party/rust_patches/`, pins `openssl-sys = "=0.9.116"` / `pq-sys = "=0.7.5"` in the root `Cargo.toml`). They build OpenSSL and libpq from source, which is what makes the build portable. Their patches are declared as annotation `patches`, so a version bump that invalidates one fails the fetch loudly — **do not paper over that**: `pq-src`'s patch is macOS-only, so a skipped patch leaves Linux CI green and breaks a developer's machine later. Bumping one is a deliberate act: re-pin, regenerate the patch, update the hardcoded `openssl-src` labels in `third_party/BUILD.bazel`, verify on macOS **and** Linux.
