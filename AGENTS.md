...
- **Causal / statistical / streaming-anomaly reasoning**: use the **DeepCausality** library (`deep_causality_core` Flow API + `deep_causality_data_structures` `SlidingWindow`; source at `~/src/deep_causality`, shipped via the `causal_reasoner_nif`). Authored by Marvin Hansen, who guides ServiceRadar's anomaly-engine design. **Do not hand-roll a parallel detector** (rolling z-score, running mean/variance, sliding window, CSM, etc.) in Elixir or Go when DeepCausality already provides the primitive — a second implementation must be kept in numeric parity by hand and drifts (this caused the PR #3792 variance bug). One reasoner, in the NIF, is the source of truth. **Performance is not a reason to fork**: O(1) Welford inside DeepCausality benchmarks at ~100M evals/sec/core (vs ~2.8M/s for the O(window) recompute), and per-sample FFI cost is solved by batching (`reason_batch`), not by a separate Elixir engine. If DeepCausality lacks a primitive, add it upstream or wrap it behind a project-owned module — never a divergent reimplementation. See `openspec/changes/refactor-anomaly-reasoner-deepcausality`.
