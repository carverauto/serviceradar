defmodule ServiceRadarAgentGateway.MtrMetricsPublisherTest do
  ...
  setup do
    previous_config = Application.get_env(:serviceradar_agent_gateway, :mtr_metrics_publisher)
    previous_pid = Application.get_env(:serviceradar_agent_gateway, :mtr_metrics_publisher_test_pid)

    on_exit(fn ->
      restore_env(:mtr_metrics_publisher, previous_config)
      restore_env(:mtr_metrics_publisher_test_pid, previous_pid)
    end)

    Application.put_env(:serviceradar_agent_gateway, :mtr_metrics_publisher_test_pid, self())

    :ok
  end

  test "publishes protobuf MTR metric batch with gateway-attested identity" do
    Application.put_env(:serviceradar_agent_gateway, :mtr_metrics_publisher,
      enabled: true,
    ...
    assert :ok = MtrMetricsPublisher.publish_mtr(mtr_status(payload))

    assert_receive {:published, "metrics.mtr.mtr.mtr_hop_avg_us", published_payload, opts}
  ...
  test "rejects legacy JSON MTR metric payloads" do
    ...
    assert {:error, :invalid_metric_batch_payload} =
             MtrMetricsPublisher.publish_mtr(mtr_status(Jason.encode!(%{"results" => [%{"target" => "example.net"}]})))
  ...
  test "is disabled by default" do
    ...
    assert :disabled = MtrMetricsPublisher.publish_mtr(mtr_status(metric_batch_payload()))
    refute_receive {:published, _subject, _payload, _opts}
  end
