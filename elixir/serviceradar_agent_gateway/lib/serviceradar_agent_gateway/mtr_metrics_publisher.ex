defmodule ServiceRadarAgentGateway.MtrMetricsPublisher do
  @moduledoc """
  Publishes protobuf MTR scalar metric batches to the high-rate metrics stream.
  """
  ...
  @config_key :mtr_metrics_publisher
  @default_subject_prefix "metrics.mtr"
  @metric_schema "serviceradar.metric.v1"

  @type publish_result :: :ok | :disabled | {:error, term()}

  @spec publish_mtr(map()) :: publish_result()
  ...
  defp decode_metric_batch(_message), do: {:error, :missing_mtr_message}
  ...
  defp log_publish_error(reason, status, error) do
    Logger.warning("Failed to publish MTR metrics",
