defmodule ServiceRadarAgentGateway.IcmpMetricsPublisher do
  @moduledoc """
  Publishes protobuf ICMP check metric batches to the high-rate metrics stream.
  """
  ...
  @config_key :icmp_metrics_publisher
  @default_subject_prefix "metrics.icmp"
  @metric_schema "serviceradar.metric.v1"

  @type publish_result :: :ok | :disabled | {:error, term()}

  @spec publish_icmp(map()) :: publish_result()
  ...
  defp decode_metric_batch(_message), do: {:error, :missing_icmp_message}
  ...
  defp log_publish_error(reason, status, error) do
    Logger.warning("Failed to publish ICMP metrics",
