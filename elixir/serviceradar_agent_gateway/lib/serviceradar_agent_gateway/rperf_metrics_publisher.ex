defmodule ServiceRadarAgentGateway.RperfMetricsPublisher do
  @moduledoc """
  Publishes protobuf rperf metric batches to the high-rate metrics stream.
  """
  ...
  @config_key :rperf_metrics_publisher
  @default_subject_prefix "metrics.rperf"
  @metric_schema "serviceradar.metric.v1"

  @type publish_result :: :ok | :disabled | {:error, term()}

  @spec publish_rperf(map()) :: publish_result()
  ...
  defp decode_metric_batch(_message), do: {:error, :missing_rperf_message}
  ...
  defp log_publish_error(reason, status, error) do
    Logger.warning("Failed to publish rperf metrics",
