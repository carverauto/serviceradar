defmodule ServiceRadarAgentGateway.SweepMetricsPublisher do
  @moduledoc """
  Publishes protobuf sweep scalar metric batches to the high-rate metrics stream.
  """
  ...
  @config_key :sweep_metrics_publisher
  @default_subject_prefix "metrics.sweep"
  @metric_schema "serviceradar.metric.v1"

  @type publish_result :: :ok | :disabled | {:error, term()}

  @spec publish_sweep(map()) :: publish_result()
  ...
  defp decode_metric_batch(_message), do: {:error, :missing_sweep_message}
  ...
  defp log_publish_error(reason, status, error) do
    Logger.warning("Failed to publish sweep metrics",
