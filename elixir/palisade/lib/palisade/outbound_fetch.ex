defmodule Palisade.OutboundFetch do
  @moduledoc """
  HTTP fetch helper that binds requests to the resolved IP that
  `Palisade.OutboundURLPolicy.resolve_https_public_url/1`
  approved. TLS hostname verification + the `Host:` header stay
  tied to the original hostname, so cert validation still works.

  This is the second half of the SSRF defense — the policy
  module validates "is this URL acceptable to fetch?", and
  `OutboundFetch` makes sure the actual HTTP request lands at
  that pre-validated IP (defeats DNS rebinding where a
  public-looking host returns a private IP between validation
  and fetch).

  Ported from
  `ServiceRadarWebNGWeb.Auth.OutboundFetch` (~/src/serviceradar).
  Palisade is the canonical home.

  ## Usage

      Palisade.OutboundFetch.get("https://accounts.example.com/.well-known/openid-configuration")
      # => {:ok, %Req.Response{}} | {:error, _}

  Pass `resolved_address: address` if you've already resolved
  the host once and want to skip a second lookup:

      Palisade.OutboundFetch.get(url, resolved_address: address)
  """
  ...
  @doc """
  Conservative defaults for OIDC discovery / JWKS / metadata
  fetches. Short connect + receive timeouts; redirects disabled
  so an attacker can't steer the request away from the validated
  host.
  """
