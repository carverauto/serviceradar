defmodule ServiceRadarWebNGWeb.Api.SourceInventoryControllerTest do
  ...
  test "requires bearer or API-key authentication", %{conn: conn} do
    response =
      ...
      |> get(~p"/api/v1/source-inventory?instance=example-prod")
      |> json_response(401)
  ...
  test "returns a bounded collection response to an authorized bearer", %{conn: conn} do
    ...
    response =
      conn
      |> get(~p"/api/v1/source-inventory?instance=example-prod&limit=100")
      |> json_response(200)

    assert_received {:source_inventory_params, %{"instance" => "example-prod", "limit" => "100"}}
  ...
  test "maps collection changes and unsafe queries to stable errors", %{conn: conn} do
    ...
    invalid =
      ...
      |> get(~p"/api/v1/source-inventory?instance=example-prod&sql=select")
      |> json_response(400)
