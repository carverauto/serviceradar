...
describe("timeseries hover geometry", () => {
  ...
  it("maps point indices back to rendered local x positions", () => {
    expect(timeseriesPointIndexToLocalX(0, rect, 5)).toBe(4)
    expect(timeseriesPointIndexToLocalX(2, rect, 5)).toBe(200)
    expect(timeseriesPointIndexToLocalX(4, rect, 5)).toBe(396)
  })
})
