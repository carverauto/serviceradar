defmodule ServiceRadarWebNGWeb.Settings.NetworkCredentialRulesLive do
  ...
  defp normalize_rule_params(params) do
    with {:ok, provider} <- enum_param(params, "provider", @providers, "provider"),
         ...
         {:ok, credential_use_policy} <- credential_use_policy_param(params, purposes) do
      {:ok,
       %{
         ...
         metadata: rule_metadata(params, provider, purposes, credential_use_policy)
       }}
    end
  rescue
  ...
  defp rule_metadata(params, provider, purposes, credential_use_policy) do
    metadata =
    ...
    if is_map(credential_use_policy),
      do: Map.put(metadata, "credential_use_policy", credential_use_policy),
      else: metadata
  end
