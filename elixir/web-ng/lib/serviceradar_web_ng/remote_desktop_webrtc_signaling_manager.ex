defmodule ServiceRadarWebNG.RemoteDesktopWebRTCSignalingManager do
  @moduledoc """
  ERTS client for desktop WebRTC signaling owned by `core-elx`.
  """

  @default_rpc_timeout 5_000

  def create_session(session_id, opts) when is_binary(session_id) do
    rpc(:create_session, [session_id, opts], opts)
  end
  ...
  def submit_answer(session_id, viewer_session_id, answer_sdp, opts)
    ...
    rpc(:submit_answer, [session_id, viewer_session_id, answer_sdp, opts], opts)
  end
  ...
  def add_ice_candidate(session_id, viewer_session_id, candidate, opts)
    ...
    rpc(:add_ice_candidate, [session_id, viewer_session_id, candidate, opts], opts)
  end
  ...
  defp rpc(function, args, opts) do
    ...
    case core_elx_node(module, timeout) do
      nil ->
        {:error, "desktop webrtc signaling unavailable"}

      node ->
        case :rpc.call(node, module, function, args, timeout) do
          {:badrpc, reason} -> {:error, "desktop webrtc signaling unavailable: #{inspect(reason)}"}
          result -> result
        end
    end
  end
  ...
  defp remote_manager_module do
    Application.get_env(
      :serviceradar_web_ng,
      :remote_access_desktop_webrtc_remote_manager,
      Module.concat([ServiceRadarCoreElx, RemoteDesktop, WebRTCSignalingManager])
    )
  end

  defp rpc_nodes do
    Application.get_env(:serviceradar_web_ng, :remote_access_desktop_webrtc_rpc_nodes, [
      Node.self() | Node.list()
    ])
  end
end
