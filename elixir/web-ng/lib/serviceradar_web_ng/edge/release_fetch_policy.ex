defmodule ServiceRadarWebNG.Edge.ReleaseFetchPolicy do
  ...
  defp validate_scheme(_uri), do: {:error, :disallowed_scheme}
  ...
  defp private_or_loopback_ip?({_, _, _, _, _, _, _, _} = ip), do: private_or_loopback_ipv6?(ip)
  defp private_or_loopback_ip?(_ip), do: true
  ...
  defp private_or_loopback_ipv6?({w1, _, _, _, _, _, _, _}) when band(w1, 0xFE00) == 0xFC00, do: true
