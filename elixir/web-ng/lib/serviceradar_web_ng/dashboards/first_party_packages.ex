defmodule ServiceRadarWebNG.Dashboards.FirstPartyPackages do
  ...
  def fetch_renderer(%DashboardPackage{source_type: :first_party, dashboard_id: @dashboard_id} = package, opts) do
  ...
  defp ensure_route_instance(%DashboardPackage{} = package, actor) do
    case fetch_route_instance(actor) do
      ...
      {:ok, %DashboardInstance{dashboard_package: %DashboardPackage{dashboard_id: @dashboard_id}} = instance} ->
  ...
  defp verify_package_renderer(%DashboardPackage{content_hash: hash}, renderer) when is_binary(hash) do
    actual =
      :sha256
