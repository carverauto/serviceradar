...
// The three FieldSurvey raw streams. Each WebSocket frame is one Arrow IPC RecordBatch
// produced by //rust/fieldsurvey-sidekick (or the Swift Sidekick app); decoding here rather
// than in Elixir keeps the per-row work off the BEAM scheduler, and returning NifMap structs
// hands the handler exactly the shape the Ash :bulk_insert actions accept.
#[rustler::nif(schedule = "DirtyCpu")]
