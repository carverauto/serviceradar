...
function escapeSrqlValue(value) {
  return String(value ?? "").trim().replace(/\s+/g, "\\ ");
}
function srqlList(values) {
...
function createSrqlClient(api) {
  ...
  const update = (query2, frameQueries = {}) => {
    if (srqlUpdate) srqlUpdate(query2, frameQueries);
    if (legacyUpdate && fallback.update !== api?.setSrqlQuery) legacyUpdate(query2, frameQueries);
  };
  return {
    query,
    update,
...
function createDashboardQueryState(options = {}) {
  const {
    initialState = {},
    baseQuery = "",
    buildQuery,
    buildFrameQueries: buildFrameQueries2,
    serializeFilters,
    hydrateFilters,
    debounceMs = 0,
    onBeforeApply,
  ...
  function computeQuery(nextState) {
    const query = typeof buildQuery === "function" ? normalizeQuery(buildQuery(nextState)) : normalizeQuery(baseQuery);
    const frameQueries = typeof buildFrameQueries2 === "function" ? sanitizeFrameQueries(buildFrameQueries2(nextState)) : {};
...
async function importApacheArrow() {
  ...
  try {
    ...
    const error = new Error(
      "ServiceRadar dashboard SDK could not load apache-arrow. Install apache-arrow as a dependency of your dashboard package, or call setArrowDecoder() to provide a custom decoder."
    );
    error.cause = cause;
    throw error;
  }
...
const dynamicImport = (name) => import(
  /* @vite-ignore */
  /* webpackIgnore: true */
  name
);
...
const DashboardContext = reactExports.createContext(null);
function DashboardProvider({ host, api, lifecycle, children }) {
  const value = reactExports.useMemo(() => ({ host, api, lifecycle }), [host, api, lifecycle]);
  return React.createElement(DashboardContext.Provider, { value }, children);
}
function useDashboardFrames() {
  const { api } = useDashboardContext();
  const cacheRef = reactExports.useRef({ frames: [], digests: [] });
  const reconcile = reactExports.useCallback((candidate) => {
  ...
  const [frames, setFrames] = reactExports.useState(() => reconcile(safeFrames(api)));
...
function useDashboardFrame(frameId) {
  const frames = useDashboardFrames();
  const target = String(frameId);
  return reactExports.useMemo(() => frames.find((frame) => String(frame?.id) === target), [frames, target]);
}
function useFrameRows(frameId, options) {
  const frame = useDashboardFrame(frameId);
  return useDecodedFrameRows(frame, options);
}
function useDashboardSrql() {
  const { api } = useDashboardContext();
  return reactExports.useMemo(() => createSrqlClient(api), [api]);
}
function useDashboardQueryState(options = {}) {
  ...
  const [snapshot, setSnapshot] = reactExports.useState(() => controllerRef.current.getSnapshot());
  reactExports.useEffect(() => {
  ...
  const apply = reactExports.useCallback((patch, opts) => controllerRef.current?.apply(patch, opts), []);
  const reset = reactExports.useCallback((opts) => controllerRef.current?.reset(opts), []);
  const flush = reactExports.useCallback(() => controllerRef.current?.flush(), []);
  const hydrate = reactExports.useCallback((input) => controllerRef.current?.hydrate(input), []);
...
function mountReactDashboard(Component, options = {}) {
  return async function mountDashboard2(root, host, api) {
...
const decodedRowsCache = /* @__PURE__ */ new WeakMap();
...
function useDecodedFrameRows(frame, options) {
  ...
  reactExports.useEffect(() => {
    ...
    let cancelled = false;
    decodeArrowFrame(frame).then(({ rows: decoded }) => {
      if (cancelled) return;
...
function Dashboard() {
  ...
  const statusCounts = reactExports.useMemo(() => normalizeAvailability(availability), [availability]);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "noc-shell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "noc-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "Service Availability NOC" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { children: [
          group.label,
          " service checks, SLO budget pressure, and active target state."
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "noc-toolbar", "aria-label": "Dashboard filters", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
          "Group",
          /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: queryState.state.group, onChange: (event) => queryState.apply({ group: event.target.value }, { immediate: true }), children: GROUPS.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: option.value, children: option.label }, option.value)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
          "Kind",
          /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: queryState.state.kind, onChange: (event) => queryState.apply({ kind: event.target.value }, { immediate: true }), children: KINDS.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: option.value, children: option.label }, option.value)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
          "Status",
          /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: queryState.state.status, onChange: (event) => queryState.apply({ status: event.target.value }, { immediate: true }), children: STATES.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: option.value, children: option.label }, option.value)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { children: [
          "SLO owner",
          /* @__PURE__ */ jsxRuntimeExports.jsxs("select", { value: queryState.state.owner, onChange: (event) => queryState.apply({ owner: event.target.value }, { immediate: true }), children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "all", children: "All" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "noc", children: "NOC" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "database-ops", children: "Database Ops" })
          ] })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "kpi-grid", "aria-label": "Service and SLO status", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricTile, { title: "Availability", value: `${formatNumber(statusCounts.availability_pct, 2)}%`, tone: availabilityTone(statusCounts.availability_pct), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        formatCount(statusCounts.available),
        " available of ",
        formatCount(serviceTotal),
        " services"
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricTile, { title: "Attention", value: formatCount(atRiskServices), tone: atRiskServices > 0 ? "warning" : "good", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        formatCount(statusCounts.critical),
        " critical, ",
        formatCount(statusCounts.unknown),
        " unknown"
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricTile, { title: "SLO Budget", value: `${formatNumber(Number(budget.avg_budget_remaining_basis_points || 0) / 100, 1)}%`, tone: budgetTone(budget.avg_budget_remaining_basis_points), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
        formatCount(budget.warning),
        " warning, ",
        formatCount(budget.critical),
        " critical"
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(MetricTile, { title: "Max Burn", value: `${formatNumber(budget.max_burn_rate_short, 1)}x`, tone: Number(budget.max_burn_rate_short || 0) >= 6 ? "bad" : "neutral", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: budget.next_projected_exhaustion_at ? `Next exhaustion ${formatTime(budget.next_projected_exhaustion_at)}` : "No exhaustion projected" }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "split-grid", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(PanelHeader, { title: "Service State", detail: `${formatCount(attentionServices.length)} services need review` }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(StatusBars, { counts: statusCounts }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(ServiceTable, { rows: attentionServices })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(PanelHeader, { title: "SLO Pressure", detail: `${formatCount(sloEvaluations.length)} warning or critical evaluations` }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(BudgetSummary, { budget }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(SloTable, { rows: sloEvaluations })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "panel inventory-panel", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(PanelHeader, { title: "Monitored Services", detail: `${formatCount(inventory.length)} services in current selector` }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(InventoryTable, { rows: inventory.slice(0, 12) })
    ] })
  ] });
}
...
function MetricTile({ title, value, tone, children }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: `metric-tile tone-${tone || "neutral"}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "metric-title", children: title }),
...
function PanelHeader({ title, detail }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "panel-header", children: [
...
function StatusBars({ counts }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "status-bars", "aria-label": "Availability status distribution", children: segments.map(([key, count, label]) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bar-row", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: label }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bar-track", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `bar-fill ${key}`, style: { width: `${Math.max(2, Number(count || 0) / total * 100)}%` } }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: formatCount(count) })
  ] }, key)) });
}
function BudgetSummary({ budget }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "budget-summary", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "budget-meter", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { width: `${consumedPct}%` } }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("dl", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("dt", { children: "Consumed" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("dd", { children: formatCount(consumed) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("dt", { children: "Remaining" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("dd", { children: formatCount(remaining) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("dt", { children: "Short burn" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("dd", { children: [
          formatNumber(budget.max_burn_rate_short, 1),
          "x"
        ] })
      ] })
    ] })
...
function ServiceTable({ rows }) {
  if (rows.length === 0) return /* @__PURE__ */ jsxRuntimeExports.jsx(EmptyState, { label: "No services match the attention filters." });
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "table-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Service" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Capability" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Status" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Latency" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Observed" })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: rows.slice(0, 10).map((row) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: row.service_name || row.service_key }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: row.service_key })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: row.descriptor_id || row.service_kind }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(StatusPill, { status: row.status }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: row.response_time_ms == null ? "-" : `${row.response_time_ms} ms` }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: formatTime(row.last_observed_at) })
    ] }, row.uid || row.check_instance_id || row.service_key)) })
  ] }) });
}
function SloTable({ rows }) {
  if (rows.length === 0) return /* @__PURE__ */ jsxRuntimeExports.jsx(EmptyState, { label: "No SLOs are currently warning or critical." });
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "table-wrap", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "SLO" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "State" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Budget" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Burn" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Evaluated" })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: rows.slice(0, 10).map((row) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: row.slo_name || row.slo_key }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("small", { children: row.owner || "unowned" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(StatusPill, { status: row.severity || row.compliance_state }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { children: [
        formatNumber(Number(row.budget_remaining_basis_points || 0) / 100, 1),
        "%"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { children: [
        formatNumber(row.burn_rate_short, 1),
        "x"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: formatTime(row.evaluated_at) })
    ] }, row.uid || row.evaluation_key || row.slo_key)) })
  ] }) });
}
function InventoryTable({ rows }) {
  if (rows.length === 0) return /* @__PURE__ */ jsxRuntimeExports.jsx(EmptyState, { label: "No services are in the current selector." });
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "inventory-grid", children: rows.map((row) => /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: "service-card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: row.display_name || row.service_key }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
      row.service_kind || "service",
      " / ",
      row.protocol || "unknown"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("small", { children: [
      row.host || row.service_key,
      row.port ? `:${row.port}` : ""
    ] })
  ] }, row.uid || row.service_key)) });
}
function StatusPill({ status }) {
  const normalized = String(status || "unknown").toLowerCase();
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `status-pill ${normalized}`, children: normalized });
}
function EmptyState({ label }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "empty-state", children: label });
}
...
function formatNumber(value, digits = 0) {
  const numeric = Number(value || 0);
  return numeric.toLocaleString(void 0, { minimumFractionDigits: digits, maximumFractionDigits: digits });
}
function formatTime(value) {
  if (!value) return "-";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return String(value);
  return date.toLocaleString(void 0, { month: "short", day: "2-digit", hour: "2-digit", minute: "2-digit" });
}
const mountDashboard = mountReactDashboard(Dashboard);
export {
  mountDashboard as default,
  mountDashboard
};
