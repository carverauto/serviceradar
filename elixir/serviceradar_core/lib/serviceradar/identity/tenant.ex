defmodule ServiceRadar.Identity.Tenant do
  ...
  actions do
    ...
    action :generate_ca, :struct do
      ...
      run(fn input, _context ->
        ...
        # Platform actor for CA generation (cross-tenant infrastructure operation)
      ...
      change(fn changeset, _ ->
        ...
        |> Ash.Changeset.after_action(fn _changeset, tenant ->
          ...
          with {:ok, user} <-
               ...
               {:ok, final_tenant} <- Ash.update(tenant, %{owner_id: user.id}, actor: actor) do
            with {:ok, platform_tenant} <- platform_tenant,
                 {:ok, _} <- ensure_platform_owner(first_user?, platform_tenant, user, actor) do
              {:ok, final_tenant}
            end
          else
  ...
  defp first_user? do
    # Platform actor for checking if this is the first user
    actor = SystemActor.platform(:tenant_registration)
  ...
  defp fetch_platform_tenant do
    # Platform actor for finding the platform tenant
    actor = SystemActor.platform(:tenant_registration)
  ...
  defp ensure_platform_owner(_, _, _, _), do: {:ok, :ok}
