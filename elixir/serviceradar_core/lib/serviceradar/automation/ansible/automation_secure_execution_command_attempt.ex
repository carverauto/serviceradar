defmodule ServiceRadar.Automation.Ansible.AutomationSecureExecutionCommandAttempt do
  @moduledoc """
  Durable outbox and replay ledger for one hardened non-callback AWX command.

  Callback-enabled executions have a distinct attempt resource and schema.
  This row is valid only for an execution whose operation has no callback
  actions.
  """
  ...
  postgres do
    table "automation_secure_execution_command_attempts"
    repo ServiceRadar.Repo
    schema "platform"

    identity_index_names unique_command: "automation_secure_execution_attempts_command_uidx",
                         unique_stage_attempt:
                           "automation_secure_execution_attempts_stage_attempt_uidx",
                         one_active_stage:
                           "automation_secure_execution_attempts_active_stage_uidx"
    ...
    check_constraints do
      check_constraint :request_digest, "automation_secure_execution_attempts_digest_check",
        check:
      ...
      check_constraint :request_schema_version,
                       ...
                       message: "must use the supported secure execution command schema"

      check_constraint :command_type, "automation_secure_execution_attempts_stage_command_check",
        check: """
        ...
        """,
        message: "must match its bounded secure execution stage contract"

      check_constraint :attempt, "automation_secure_execution_attempts_bounds_check",
        check: """
        ...
        """,
        message: "must stay within the bounded secure execution domain"

      check_constraint :next_attempt_at, "automation_secure_execution_attempts_deadline_check",
        check: "next_attempt_at IS NULL OR next_attempt_at <= deadline_at",
        message: "must not exceed the command deadline"

      check_constraint :reconcile_after, "automation_secure_execution_attempts_reconcile_check",
        check:
      ...
      check_constraint :lease_token, "automation_secure_execution_attempts_lease_check",
        check: """
      ...
      check_constraint :processed_at, "automation_secure_execution_attempts_terminal_check",
        check: """
  ...
  policies do
    ...
    action_with_permission(
      [:read, :by_id, :by_command_id, :for_execution],
      @view_check
    )
  end

  changes do
    # Action filters describe which lifecycle transition is valid, while the
    # version guard makes ownership of that transition exclusive when multiple
    # recovery workers loaded the same row. Without this guard two callers can
    # both receive an apparently successful lease from the same stale struct.
  ...
  attributes do
    ...
    attribute :candidate_job_ids, {:array, :integer},
      ...
      default: [],
      public?: true,
      constraints: [items: [min: 1], max_length: 50]
  ...
  identities do
    ...
    identity :unique_stage_attempt, [:execution_id, :stage, :purpose, :attempt]

    identity :one_active_stage, [:execution_id, :stage, :purpose],
