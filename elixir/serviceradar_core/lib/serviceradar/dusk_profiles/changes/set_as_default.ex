defmodule ServiceRadar.DuskProfiles.Changes.SetAsDefault do
  @moduledoc """
  Ensures only one default dusk profile exists.
  ...
  def change(changeset, _opts, _context) do
    ...
    system_actor = SystemActor.system(:dusk_profile_default)
  ...
  defp unset_other_defaults(current_id, actor) do
    ...
    case Ash.read(query, actor: actor) do
      ...
      {:error, error} ->
        ...
        require Logger
        Logger.warning("Failed to query existing default dusk profiles: #{inspect(error)}")
        :ok
    end
  end
end
