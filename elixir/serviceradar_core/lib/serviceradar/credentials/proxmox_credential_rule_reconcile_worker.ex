defmodule ServiceRadar.Credentials.ProxmoxCredentialRuleReconcileWorker do
  @moduledoc """
  Periodic worker that materializes Proxmox credential rules for eligible agents.
  """
  ...
  def perform(%Oban.Job{}) do
    actor = SystemActor.system(:proxmox_credential_rule_reconcile_worker)
    ...
    case load_connected_agents(actor) do
      ...
      {:error, reason} ->
        Logger.warning("Failed to load agents for Proxmox credential reconciliation",
  ...
  defp schedule_next do
    seconds =
      Application.get_env(
        :serviceradar_core,
        :proxmox_credential_rule_reconcile_interval_seconds,
