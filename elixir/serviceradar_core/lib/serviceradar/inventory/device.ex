defmodule ServiceRadar.Inventory.Device do
  ...
  actions do
    ...
    read :by_gateway do
      ...
      filter(
        expr(gateway_id == ^arg(:gateway_id) and (is_nil(deleted_at) or ^arg(:include_deleted)))
      )
    end
    ...
    update :bump_identity_revision do
      description(
        "Record that this device's identity changed (merge, unmerge, split, alias or identifier reassignment, soft delete, restore)"
      )
  ...
  attributes do
    ...
    attribute :os, :map do
      ...
      description(
        "Operating system info (OCSF OS object), including passive_fingerprint evidence"
      )
    end
    ...
    attribute :management_device_id, :string do
      ...
      description(
        "UID of the device used to reach this device for management (e.g. SNMP polling)"
      )
    end
    ...
    attribute :availability_source_agent_id, :string do
      ...
      description(
        "Agent whose sweep results drive canonical device availability. Required for All-agents sweep groups; nil lets an assigned group's scanner write the bit."
      )
    end
    ...
    attribute :availability_source_profile_id, :uuid do
      ...
      description(
        "Availability source profile that assigned the canonical source; nil means manual override or legacy fallback"
      )
    end
  ...
  relationships do
    ...
    has_many :bumblebee_postures, ServiceRadar.Inventory.BumblebeeDevicePosture do
      ...
      description(
        "Bumblebee developer endpoint exposure posture for agents associated with this device"
      )
    end
