defmodule ServiceRadar.Inventory.AdvisoryFeeds.Acquisition do
  ...
  * **CISA** is a single public JSON streamed to disk.
  ...
  validators. A publication that rolls between a download and its revalidation
  surfaces as `{:error, {:validator_changed, _}}` and is left to Oban's normal
  backoff retry, the same disposition as any other acquisition error.

  The Smokescreen egress proxy was ruled out for this path: 412 Precondition
  Failed is generated by the origin evaluating the request's preconditions, so
  the request demonstrably reached Canonical directly and the proxy could not
  have produced it. The pair stays on `OutboundFetch`, not `EgressClient`.
  """
  def acquire_ubuntu_pair(feed, osv_url, vex_url, run_id, opts \\ []) do
    with {:ok, paths} <- Staging.prepare_run(feed, run_id) do
      result = acquire_pair_once(paths, osv_url, vex_url, opts)
