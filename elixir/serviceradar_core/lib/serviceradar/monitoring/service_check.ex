defmodule ServiceRadar.Monitoring.ServiceCheck do
  ...
  actions do
    ...
    read :due_for_check do
      ...
      filter expr(
        ...
        (is_nil(last_check_at) or
         fragment("? + interval '1 second' * ?", last_check_at, interval_seconds) < now())
      )
    ...
    create :create do
      accept [
        :name, :description, :check_type, :target, :port, :interval_seconds,
        :timeout_seconds, :retries, :config, :warning_threshold_ms,
        :critical_threshold_ms, :agent_uid, :device_uid, :metadata
      ]
    end

    update :update do
      accept [
        :name, :description, :target, :port, :interval_seconds,
        :timeout_seconds, :retries, :config, :warning_threshold_ms,
        :critical_threshold_ms, :agent_uid, :metadata
      ]
    end
  ...
  calculations do
    calculate :check_type_label, :string, expr(
      cond do
    ...
    calculate :is_overdue, :boolean, expr(
      ...
      fragment("? + interval '1 second' * ? * 2", last_check_at, interval_seconds) < now()
    )

    calculate :next_check_at, :utc_datetime, expr(
