defmodule ServiceRadar.Monitoring.PollingSchedule do
  ...
  actions do
      ...
      filter expr(
        ...
        (is_nil(last_executed_at) or
         (schedule_type == :interval and
          fragment("? + interval '1 second' * ?", last_executed_at, interval_seconds) < now()) or
         schedule_type == :cron)
      )
    ...
    create :create do
      accept [
        :name, :description, :schedule_type, :interval_seconds, :cron_expression,
        :assignment_mode, :assigned_poller_id, :assigned_partition_id,
        :priority, :max_concurrent, :timeout_seconds, :metadata
      ]
      ...
      validate fn changeset, _context ->
        ...
        cond do
          schedule_type == :interval and (is_nil(interval) or interval <= 0) ->
            {:error, field: :interval_seconds, message: "must be a positive integer for interval schedules"}
    ...
    update :update do
      accept [
        :name, :description, :interval_seconds, :cron_expression,
        :assignment_mode, :assigned_poller_id, :assigned_partition_id,
        :priority, :max_concurrent, :timeout_seconds, :metadata
      ]
    end
    ...
    update :execute do
      ...
      change fn changeset, _context ->
        ...
        # The actual poll orchestration will:
        # 1. Find all checks in this schedule
        # 2. Enqueue jobs to appropriate pollers
        # 3. Track execution via PubSub
        # For now, just record the execution attempt
    ...
    update :record_result do
      ...
      change fn changeset, _context ->
        ...
        |> Ash.Changeset.change_attribute(:last_check_count, Ash.Changeset.get_argument(changeset, :check_count))
        |> Ash.Changeset.change_attribute(:last_success_count, Ash.Changeset.get_argument(changeset, :success_count))
        |> Ash.Changeset.change_attribute(:last_failure_count, Ash.Changeset.get_argument(changeset, :failure_count))
  ...
  calculations do
    ...
    calculate :is_locked, :boolean, expr(
      not is_nil(lock_token) and
      locked_at > ago(5, :minute)
    )

    calculate :next_execution_at, :utc_datetime, expr(
