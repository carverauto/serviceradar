defmodule ServiceRadar.Edge.ProxmoxConsoleSession do
  ...
  actions do
    ...
    update :attach do
      ...
      change(set_attribute(:ticket_used_at, &DateTime.utc_now/0))
  ...
  attributes do
    ...
    attribute :ticket_expires_at, :utc_datetime_usec do
      allow_nil?(false)
      public?(true)
    end

    attribute :ticket_used_at, :utc_datetime_usec do
      public?(true)
    end
    ...
    attribute :status, :atom do
      ...
      constraints(one_of: [:requested, :attached, :opening, :active, :closing, :closed, :failed, :expired])
      default(:requested)
    end
