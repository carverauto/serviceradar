defmodule ServiceRadar.Plugins.WorkloadIdentityAddonPackageSeeder do
  @moduledoc """
  Seeds the first-party workload identity native add-on package.

  Without signed artifacts this intentionally stages a visible, non-assignable
  package. Release import later replaces it with mirrored artifacts and approval.
  """
  ...
  @config_schema_path Path.expand(
                        "../../../../../addons/workload-identity/config.schema.json",
                        __DIR__
                      )
  @external_resource @config_schema_path
  @config_schema @config_schema_path |> File.read!() |> Jason.decode!()

  @manifest_path Path.expand("../../../../../addons/workload-identity/addon.yaml", __DIR__)
  ...
  def seed_defaults(opts \\ []) do
    config =
      :serviceradar_core
      |> Application.get_env(:workload_identity_native_addon_package, [])
      |> Keyword.merge(opts)

    actor = SystemActor.system(:workload_identity_addon_package_seeder)
    ...
    case normalize_artifacts(Keyword.get(config, :artifacts, %{})) do
      ...
      {:ok, _empty} ->
        Logger.info(
          "Seeding workload identity native add-on package as staged (no signed artifacts configured yet)"
        )
  ...
  defp create_package(attrs, approve?, opts) do
    with {:ok, package} <-
      ...
      Logger.info("Seeded workload identity native add-on package",
  ...
  defp approve_if_needed(%AddonPackage{} = package, opts) do
    ...
    |> Ash.Changeset.for_update(
      ...
      %{
        ...
        approved_by: "system:workload_identity_addon_package_seeder"
      },
      opts
    )
    |> Ash.update(opts)
  ...
  defp package_attrs(config, artifacts) do
    ...
    %{
      ...
      description:
        "Standalone node-local CRI/container workload metadata collector for pod, namespace, container, image, and runtime identity enrichment.",
      kind: :native,
      ...
      source_metadata: %{
        "native_addon_inventory" => "workload_identity_addon_bundle",
        "seeded_by" => "ServiceRadar.Plugins.WorkloadIdentityAddonPackageSeeder"
      },
  ...
  defp normalize_artifacts(_artifacts), do: {:error, :invalid_workload_identity_addon_artifacts}
