defmodule ServiceRadar.Plugins.PluginAssignmentRecoveryAudit do
  ...
  policies do
    ...
    read_with_permission(@plugins_manage_check)

    # The audit is readable by plugin managers, but only the recovery's exact
    # dedicated internal writer may make immutable decisions. This deliberately
    # does not use `system_bypass/0`, so another system component cannot
    # accidentally inherit recovery-audit write authority.
