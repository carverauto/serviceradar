defmodule ServiceRadar.Plugins.PluginAssignmentRecovery do
  ...
  @type recovery_kind :: :manual_reapproval | :policy_reconciliation | :not_recoverable

  @doc """
  Lists one bounded page of disabled, partition-unbound rows that need an
  explicit recovery decision. This deliberately does not read current
  inventory metadata. Callers may pass `:limit` (maximum 100) and `:offset`.
  """
  ...
  def list_legacy(opts \\ []) when is_list(opts) do
    ...
    offset = legacy_list_offset(Keyword.get(opts, :offset))

    with {:ok, actor} <- initiating_actor(opts),
         :ok <- authorize_plugins_manage(actor),
         {:ok, assignments} <- legacy_assignments(actor, limit, offset) do
  ...
  def classify(%{source: :policy} = assignment) do
    if legacy_unbound?(assignment), do: :policy_reconciliation, else: :not_recoverable
  end
  ...
  defp create_replacement(assignment, package, params, evidence, actor) do
    ...
    case result do
      {:ok, replacement} when replacement.partition_id == evidence.partition_id ->
        audit_attrs = audit_attrs(assignment, actor, :recovered, :already_recovered, evidence)
  ...
  defp owner_projection(%{source: :policy, policy_id: policy_id}, actor) do
    case OwnerReference.parse(policy_id) do
      ...
      {:error, :invalid_policy_owner} ->
        %{
          kind: :unknown,
          label: "Unrecognized policy owner",
          reference: nil,
          state: :unavailable
        }
    end
  ...
  defp policy_recovery_projection(%{source: :policy, id: legacy_assignment_id}) do
  ...
  defp recovered_audit(legacy_assignment_id, actor) do
    PluginAssignmentRecoveryAudit
    |> Ash.Query.for_read(
  ...
  defp legacy_assignments(actor, limit, offset) do
    PluginAssignment
    |> Ash.Query.for_read(:read)
    |> Ash.Query.filter(enabled == false and is_nil(partition_id))
  ...
  defp legacy_list_offset(value) when is_integer(value) and value >= 0, do: value
  defp legacy_list_offset(_value), do: 0
