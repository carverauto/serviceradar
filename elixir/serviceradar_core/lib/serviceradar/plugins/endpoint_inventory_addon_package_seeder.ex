defmodule ServiceRadar.Plugins.EndpointInventoryAddonPackageSeeder do
  @moduledoc """
  Seeds the first-party endpoint inventory native add-on package when artifact refs exist.
  ...
  def seed_defaults(opts \\ []) do
    ...
    case normalize_artifacts(Keyword.get(config, :artifacts, %{})) do
      ...
      {:ok, _empty} ->
        Logger.debug(
          "Skipping endpoint inventory native add-on package seed: no artifacts configured"
        )
  ...
  defp ensure_package(config, artifacts, actor) do
    ...
    case find_package(attrs.addon_id, attrs.version, opts) do
      ...
      {:error, reason} ->
        Logger.warning(
          "Failed to check endpoint inventory add-on package seed: #{inspect(reason)}"
        )
  ...
  defp create_and_approve(attrs, opts) do
    with {:ok, package} <-
      ...
      Logger.info("Seeded approved endpoint inventory native add-on package",
      ...
      {:error, reason} ->
        Logger.warning("Failed to seed endpoint inventory add-on package: #{inspect(reason)}")
        {:error, reason}
    end
  end
  ...
  defp update_and_approve(%AddonPackage{} = package, attrs, opts) do
    with {:ok, package} <- restage_if_needed(package, opts),
      ...
      Logger.debug("endpoint inventory native add-on package seed is current",
        version: attrs.version
      )
      ...
      {:error, reason} ->
        Logger.warning(
          "Failed to update endpoint inventory add-on package seed: #{inspect(reason)}"
        )
  ...
  defp package_attrs(config, artifacts) do
    ...
    %{
      ...
      description:
        "Root-owned systemd-timer collector for local OS package inventory and CycloneDX SBOM output.",
      kind: :native,
      ...
      source_metadata: %{
        "native_addon_inventory" => "endpoint_inventory_addon_bundle",
