defmodule ServiceRadar.Observability.NetflowEnrichmentDatasetScheduler do
  @moduledoc """
  Ensures NetFlow enrichment dataset refresh jobs are scheduled when Oban is available.
  """
  ...
  defp ensure_scheduled(worker, label) do
    case worker.ensure_scheduled() do
      {:ok, _} -> :ok
      {:error, reason} -> Logger.debug("#{label} scheduling skipped", reason: reason)
    end
  end
