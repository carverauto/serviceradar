defmodule ServiceRadar.Observability.AnomalyAddonConfigProjector do
  ...
  @drift_modes ~w(off deseasonalized_only always)
  ...
  # `"off"` and drop anything else outside the enum so a stale settings row
  ...
  defp normalize_metric_class_value("drift_mode", value) when is_atom(value) do
    normalize_metric_class_value("drift_mode", Atom.to_string(value))
  end

  defp normalize_metric_class_value("drift_mode", value) when is_binary(value) do
    if value in @drift_modes, do: {:ok, value}, else: :drop
  end

  defp normalize_metric_class_value("drift_mode", _value), do: :drop
