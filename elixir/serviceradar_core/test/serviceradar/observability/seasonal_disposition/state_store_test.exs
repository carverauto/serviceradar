defmodule ServiceRadar.Observability.SeasonalDisposition.StateStoreTest do
  ...
  test "cleans expired rows and upserts next counters with ttl" do
    assert :ok =
    ...
    assert_received {:cleanup, cleanup_sql, []}
    assert cleanup_sql =~ "DELETE FROM platform.seasonal_disposition_states"
    assert cleanup_sql =~ "expires_at <= now()"
