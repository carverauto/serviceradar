defmodule ServiceRadar.WifiMap.BatchIngestorTest do
  ...
  test "normalizes WiFi-map batch payload into table upserts" do
    ...
    payload = %{
      ...
      "sites" => [
        %{
          ...
          "controllers" => "wlc-manager-01.example.com",
          "wlc_count" => "7",
    ...
    assert reference.site_code == "ZZA"
    assert reference.latitude == 10.0000
    assert reference.longitude == -20.0000

    assert_receive {:bulk_upsert, :wifi_sites, [site], [:source_id, :site_code], _}
    assert site.site_code == "ZZA"
    assert site.region == "REGION-1"
    ...
    assert snapshot.controller_names == ["wlc-manager-01.example.com"]
    assert snapshot.all_server_groups == ["aaa-site01-group_1_0", "aaa-site02-group-1_0"]
    ...
    assert ap_update["metadata"]["site_code"] == "ZZA"
    assert ap_update["metadata"]["serial_number"] == "SN0000000001"
