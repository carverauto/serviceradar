defmodule ServiceRadar.Plugins.PluginAssignmentRecoveryTest do
  ...
  test "manual recovery clones a fresh bound assignment, preserves linked encrypted material, and is idempotent",
    ...
    assert audit.reason == :already_recovered
  ...
  test "plugin managers cannot forge recovery audits while trusted recovery remains idempotent",
       %{
         actor: actor,
         unique_id: unique_id
       } do
  ...
  defp audits_for_legacy(legacy_assignment_id, actor) do
    PluginAssignmentRecoveryAudit
    |> Ash.Query.for_read(
