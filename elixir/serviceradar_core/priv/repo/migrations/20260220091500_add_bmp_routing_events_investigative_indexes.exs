defmodule ServiceRadar.Repo.Migrations.AddBmpRoutingEventsInvestigativeIndexes do
  ...
  def up do
    ...
    CREATE INDEX IF NOT EXISTS idx_bmp_routing_events_router_ip_time
      ON #{prefix()}.#{@table} (router_ip, time DESC)
    """)

    execute("""
    CREATE INDEX IF NOT EXISTS idx_bmp_routing_events_peer_ip_time
      ON #{prefix()}.#{@table} (peer_ip, time DESC)
    """)

    execute("""
    CREATE INDEX IF NOT EXISTS idx_bmp_routing_events_prefix_time
      ON #{prefix()}.#{@table} (prefix, time DESC)
    """)

    execute("""
    CREATE INDEX IF NOT EXISTS idx_bmp_routing_events_severity_time
      ON #{prefix()}.#{@table} (severity_id, time DESC)
    """)

    execute("""
    CREATE INDEX IF NOT EXISTS idx_bmp_routing_events_metadata_gin
      ON #{prefix()}.#{@table} USING GIN (metadata)
    """)
  end

  def down do
    execute("DROP INDEX IF EXISTS #{prefix()}.idx_bmp_routing_events_metadata_gin")
    execute("DROP INDEX IF EXISTS #{prefix()}.idx_bmp_routing_events_severity_time")
    execute("DROP INDEX IF EXISTS #{prefix()}.idx_bmp_routing_events_prefix_time")
    execute("DROP INDEX IF EXISTS #{prefix()}.idx_bmp_routing_events_peer_ip_time")
    execute("DROP INDEX IF EXISTS #{prefix()}.idx_bmp_routing_events_router_ip_time")
  end
end
