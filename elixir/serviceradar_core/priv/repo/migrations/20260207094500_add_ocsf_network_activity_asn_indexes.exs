defmodule ServiceRadar.Repo.Migrations.AddOcsfNetworkActivityAsnIndexes do
  ...
  def up do
    ...
    CREATE INDEX IF NOT EXISTS idx_ocsf_network_activity_src_asn_time
      ON #{prefix() || "platform"}.#{@table} (src_as_number, time DESC)
      WHERE src_as_number IS NOT NULL
    """)

    execute("""
    CREATE INDEX IF NOT EXISTS idx_ocsf_network_activity_dst_asn_time
      ON #{prefix() || "platform"}.#{@table} (dst_as_number, time DESC)
      WHERE dst_as_number IS NOT NULL
    """)
  end

  def down do
    execute("DROP INDEX IF EXISTS #{prefix() || "platform"}.idx_ocsf_network_activity_dst_asn_time")
    execute("DROP INDEX IF EXISTS #{prefix() || "platform"}.idx_ocsf_network_activity_src_asn_time")
  end
end
