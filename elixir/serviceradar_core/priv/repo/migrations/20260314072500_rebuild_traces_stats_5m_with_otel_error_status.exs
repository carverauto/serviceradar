defmodule ServiceRadar.Repo.Migrations.RebuildTracesStats5mWithOtelErrorStatus do
  use Ecto.Migration
  ...
  def up do
    rebuild_cagg("status_code = 2")
  end

  def down do
    rebuild_cagg("status_code IS NOT NULL AND status_code != 1")
  end

  defp rebuild_cagg(error_predicate) do
    execute(remove_policies_sql())
    execute("DROP MATERIALIZED VIEW IF EXISTS #{@view} CASCADE")
    execute(create_view_sql(error_predicate))

    execute(
      "CREATE INDEX IF NOT EXISTS idx_traces_stats_5m_bucket_service ON #{@view} (bucket DESC, service_name)"
    )

    execute(add_policies_sql())
  end

  defp create_view_sql(error_predicate) do
    """
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE #{error_predicate})::bigint AS error_count,
