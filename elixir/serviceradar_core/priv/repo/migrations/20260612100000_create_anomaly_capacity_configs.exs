defmodule ServiceRadar.Repo.Migrations.CreateAnomalyCapacityConfigs do
  ...
  def up do
    ...
    create constraint(:anomaly_detection_configs, :anomaly_detection_configs_singleton_key,
             check: "key = 'default'",
             prefix: "platform"
           )

    create constraint(:anomaly_detection_configs, :anomaly_detection_configs_n_sigma_check,
             check: "n_sigma >= 0.1 AND n_sigma <= 20.0",
             prefix: "platform"
           )

    create constraint(:anomaly_detection_configs, :anomaly_detection_configs_window_check,
             check:
               "window_size >= 2 AND window_duration_seconds >= 1 AND min_samples >= 1 AND min_samples <= window_size",
             prefix: "platform"
           )

    create constraint(:anomaly_detection_configs, :anomaly_detection_configs_confirm_slots_check,
             check: "confirm_slots >= 1",
             prefix: "platform"
           )

    execute """
    INSERT INTO platform.anomaly_detection_configs (key, inserted_at, updated_at)
    ...
    create constraint(:capacity_forecast_configs, :capacity_forecast_configs_singleton_key,
             check: "key = 'default'",
             prefix: "platform"
           )

    create constraint(:capacity_forecast_configs, :capacity_forecast_configs_horizon_check,
             check:
               "forecast_horizon_seconds >= 3600 AND warning_horizon_seconds >= 3600 AND warning_horizon_seconds <= forecast_horizon_seconds",
             prefix: "platform"
           )

    create constraint(:capacity_forecast_configs, :capacity_forecast_configs_threshold_check,
             check: "warning_threshold_percent >= 1.0 AND warning_threshold_percent <= 100.0",
             prefix: "platform"
           )

    create constraint(:capacity_forecast_configs, :capacity_forecast_configs_model_check,
             check: "model IN ('linear', 'seasonal_linear', 'holt_winters')",
             prefix: "platform"
           )

    create constraint(:capacity_forecast_configs, :capacity_forecast_configs_history_check,
             check: "minimum_history_points >= 2",
             prefix: "platform"
           )

    execute """
    INSERT INTO platform.capacity_forecast_configs (key, inserted_at, updated_at)
    VALUES ('default', now(), now())
    ON CONFLICT (key) DO NOTHING
    """
  end
