...
4. **Spot-check the canonical keys** – Helm, compose, and the demo manifests now seed the same set of global keys (`config/core.json`, `config/agent.json`, `config/gateways/<gateway-id>.json`, `config/flowgger.toml`, etc.). If the API returns `404`, exec into the `serviceradar-tools` pod (or your compose shell) and run `nats-kv get <key>` to confirm the data is actually missing before filing an Admin UI bug.
...
Bare-metal installs now behave the same way as our Kubernetes, Helm, and Compose deployments: every systemd unit (core, agent-gateway, agent, mapper, flowgger, trapd, zen, otel, db-event-writer, and the packaged checkers) exports `CONFIG_SOURCE=kv` plus the `KV_*` client settings out of the box. Services immediately seed KV from the JSON/TOML file on first boot and switch to KV overlays without any manual edits.
...
sudo systemctl restart serviceradar-core serviceradar-agent-gateway serviceradar-agent
```
