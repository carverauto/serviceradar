...
status, and a **Review** link. Treat it as a review queue, not a bulk repair
tool: it deliberately has no bulk enable action and never assumes the `default`
partition.
...
reconciliation action is disabled and identifies the missing requirement. The
control plane rechecks all authority before executing, so browser state never
grants access by itself. The recovery UI and audit history show safe state and
reasons only; they do not show request parameters, principals, owner metadata,
replacement IDs, secret values, tokens, or private material. Do not re-enable
unbound rows with direct database updates.
