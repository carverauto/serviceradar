...
Use these OpenTelemetry gauges to monitor promotion health and cardinality drift. All metrics emit on the `serviceradar.identity` meter and reflect the most recent reconciliation run:
...
- `identity_cardinality_current` – current unified device count used for drift detection.
- `identity_cardinality_baseline` – configured baseline for drift detection (0 disables drift checks).
- `identity_cardinality_drift_percent` – percentage drift vs baseline (positive is over baseline).
- `identity_cardinality_blocked` – 1 when promotion is paused due to drift, 0 otherwise.
...
- Drift view: line chart of `identity_cardinality_drift_percent` with a horizontal tolerance line and a single-stat for `identity_cardinality_blocked`.
