...
The current optional bundle includes `serviceradar-agent` and the `serviceradar-rdp-adapter` helper binary. The helper is distributed only in the RDP-enabled archive and remains fail-closed until the reviewed IronRDP backend is linked into that target. Until then, RDP bundle manifest entries are marked with `deployment_requirements.release_phase: "experimental"` and `helper_connector_ready: false`; installed agents still advertise `remote_access.rdp` only after the helper readiness probe reports `connector_ready: true`.
