...
- **MCP in-process.** Production MCP is `AshAi.Mcp.Router` on `/mcp` (not `hermes_mcp`), a read-only facade over the HTTP API. See [MCP Integration](/docs/mcp-integration).
...
- MCP endpoints in Elixir (`AshAi.Mcp`, not hermes_mcp)
