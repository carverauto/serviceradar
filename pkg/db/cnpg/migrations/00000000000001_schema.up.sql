...
-- Standard indexes for unified_devices (NO IP unique constraint!)
...
CREATE INDEX IF NOT EXISTS idx_unified_devices_hostname_trgm ON unified_devices USING gin (hostname gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_unified_devices_ip_trgm ON unified_devices USING gin (ip gin_trgm_ops);
