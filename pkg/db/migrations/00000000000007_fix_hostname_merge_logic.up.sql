...
-- Materialized view that maintains only current device state with improved hostname merge logic
...
AS SELECT
    ...
    s.ip,
    s.poller_id,
    -- Preserve quality hostnames: only update if current hostname is empty, generic, or lower confidence
    if(s.hostname IS NOT NULL AND s.hostname != '',
       if(u.hostname IS NULL OR u.hostname = '' OR u.hostname = s.ip OR 
          (s.discovery_source = 'mapper' AND u.hostname NOT LIKE '% %'),
