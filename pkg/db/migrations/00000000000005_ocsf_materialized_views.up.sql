...
CREATE MATERIALIZED VIEW ocsf_observable_device_ips_mv
TO ocsf_observable_index AS
SELECT
    ...
    -- Categorization
    CASE
        WHEN startsWith(ip, '10.') OR startsWith(ip, '192.168.') OR startsWith(ip, '172.') THEN 'private'
        WHEN startsWith(ip, '127.') THEN 'loopback'
        ELSE 'public'
...
CREATE MATERIALIZED VIEW ocsf_observable_device_hostnames_mv
TO ocsf_observable_index AS
SELECT
...
CREATE MATERIALIZED VIEW ocsf_observable_netflow_src_ips_mv
TO ocsf_observable_index AS
SELECT
    'ip_address' AS observable_type,
    ...
    [] AS threat_sources,
    CASE
        WHEN startsWith(src_endpoint_ip, '10.') OR startsWith(src_endpoint_ip, '192.168.') OR startsWith(src_endpoint_ip, '172.') THEN 'private'
        WHEN startsWith(src_endpoint_ip, '127.') THEN 'loopback'
        ELSE 'public'
    END AS observable_category,
    [] AS tags,
    metadata
...
CREATE MATERIALIZED VIEW ocsf_observable_netflow_dst_ips_mv
TO ocsf_observable_index AS
SELECT
    'ip_address' AS observable_type,
    ...
    [] AS threat_sources,
    CASE
        WHEN startsWith(dst_endpoint_ip, '10.') OR startsWith(dst_endpoint_ip, '192.168.') OR startsWith(dst_endpoint_ip, '172.') THEN 'private'
        WHEN startsWith(dst_endpoint_ip, '127.') THEN 'loopback'
        ELSE 'public'
    END AS observable_category,
    [] AS tags,
    metadata
...
CREATE MATERIALIZED VIEW ocsf_observable_stats_mv
TO ocsf_observable_statistics AS
SELECT
    observable_type,
    observable_value,

    -- Time Window (hourly buckets)
    to_start_of_hour(time) AS window_start,
    to_start_of_hour(time) + INTERVAL 1 HOUR AS window_end,
