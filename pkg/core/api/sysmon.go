...
func (s *APIServer) getSysmonMetrics(
	...
	r *http.Request,
	fetchMetrics func(ctx context.Context, provider db.SysmonMetricsProvider, pollerID string, startTime, endTime time.Time) (interface{}, error),
	metricType string,
) {
	ctx := r.Context()
	vars := mux.Vars(r)
...
func (s *APIServer) getSysmonCPUMetrics(w http.ResponseWriter, r *http.Request) {
	fetch := func(ctx context.Context, provider db.SysmonMetricsProvider, pollerID string, startTime, endTime time.Time) (interface{}, error) {
		metrics, err := provider.GetAllCPUMetrics(ctx, pollerID, startTime, endTime)
		if err != nil {
...
func (s *APIServer) getSysmonMemoryMetrics(w http.ResponseWriter, r *http.Request) {
	fetch := func(ctx context.Context, provider db.SysmonMetricsProvider, pollerID string, startTime, endTime time.Time) (interface{}, error) {
		metrics, err := provider.GetMemoryMetricsGrouped(ctx, pollerID, startTime, endTime)
		if err != nil {
			return nil, &httpError{"Internal server error", http.StatusInternalServerError}
		}
...
func (*APIServer) fetchDiskMetrics(
	...
	if mountPoint != "" {
		metrics, err := provider.GetDiskMetrics(ctx, pollerID, mountPoint, startTime, endTime)
		if err != nil {
	...
	allMetrics, err := provider.GetAllDiskMetricsGrouped(ctx, pollerID, startTime, endTime)
	if err != nil {
...
func (s *APIServer) getSysmonDiskMetrics(w http.ResponseWriter, r *http.Request) {
	...
	response, err := s.fetchDiskMetrics(ctx, metricsProvider, pollerID, mountPoint, startTime, endTime)
	if err != nil {
		var httpErr *httpError
		if errors.As(err, &httpErr) {
