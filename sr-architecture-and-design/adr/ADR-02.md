...
| Status   | Proposed |
| Tags     | serviceradar, relationships, graphdb, arangodb, netflow, snmp, device-management, correlation, streaming-analytics |
...
ServiceRadar currently collects diverse raw data from edge services (e.g., sweep, ICMP, SNMP, NetFlow) via agents and pollers, storing it in CNPG/Timescale streams. A materialized view derives a unified `devices` stream, enabling a centralized inventory with `agent_id` and `poller_id` traceability. However, the system lacks a sophisticated mechanism to:
...
- **PRD-03: Real-Time Monitoring with CNPG**:
...
- **Existing Codebase** (`pkg/db/db.go`, `pkg/core/server.go`):
    - Defines CNPG streams for raw data.
...
We propose a hybrid architecture that leverages CNPG/Timescale for real-time data ingestion and stream processing, and ArangoDB for graph-based relationship modeling, device mastering, and complex correlation queries. This approach maintains the existing one-way data flow (edge → poller → core) and minimizes agent complexity.

### 1. Data Organization

#### CNPG/Timescale Streams (Real-time Data & Source of Truth for Events)

CNPG will continue to be the primary ingestion point for all raw time-series and event data.
...
- **Device-Interface Ownership:**
    ...
    - Sourced from the `discovered_interfaces` CNPG stream.
...
- **Network Topology:**
    ...
    - Raw data first lands in `topology_discovery_events` CNPG stream.
...
4. **Core Service & CNPG:**
    - Receives data and writes to respective CNPG streams.
...
5. **ArangoDB Sync Service (New):**
    - Subscribes to relevant CNPG streams:
...
4. **Fetch Relevant SNMP Metrics from CNPG:**
   ```sql
...
1. **Adopt Hybrid CNPG & ArangoDB Architecture:**
    - CNPG for all real-time event/metric ingestion, basic device status (`devices` stream), and historical interface discovery data (`discovered_interfaces` regular stream).
...
4. **Create ArangoDB Sync Service:**
    - Develop `serviceradar-arangodb-sync` to populate ArangoDB from CNPG streams.
...
6. **Implement Phased Approach:**
    - Start with data enrichment, then CNPG stream enhancements, followed by ArangoDB integration, and finally SRQL/API extensions.
...
- **Scalability:** CNPG handles high-volume streaming data while ArangoDB manages complex relationships.
...
- **Synchronization Overhead:** Real-time sync between CNPG and ArangoDB may introduce latency.
- **Learning Curve:** AQL and graph queries require developer training.
- **Storage Overhead:** Some data duplication between CNPG and ArangoDB.
...
- **Hybrid Querying:** Some queries may require both CNPG and ArangoDB, adding complexity but enabling powerful correlations.

## Implementation Plan

### 1. Data Foundation (4 weeks)
- Define and implement schema for `discovered_interfaces` (as a regular append stream, not versioned KV) and `topology_discovery_events` streams in CNPG.
...
- Test data flow into these CNPG streams.
...
- Develop and deploy the `serviceradar-arangodb-sync` service:
    - First: Sync `devices` (CNPG) → `Devices` (ArangoDB).
    - Next: Sync `discovered_interfaces` (CNPG) → `Interfaces` & `DeviceHasInterface` (ArangoDB).
    - Then: Sync `topology_discovery_events` (CNPG) → `DeviceToDevice` (ArangoDB).
- Test synchronization between CNPG and ArangoDB.
...
- Optimize CNPG streams and query performance.
...
- **Integration Tests:**
    - CNPG to ArangoDB synchronization.
...
- Update `docs/docs/architecture.md` with hybrid CNPG-ArangoDB model.
...
### 1. CNPG-Only with Enhanced Materialized Views
- **Pros**:
    - Simplifies architecture by using only one database system.
    - Leverages existing CNPG expertise and infrastructure.
...
### 3. Custom Relationship Tables in CNPG
- **Pros**:
...
- **Reason Rejected**: Our hybrid approach with CNPG for time-series and ArangoDB for relationships provides better performance characteristics while minimizing duplication.
